...
router.get('/vk/auth-url', protect, (req, res) => {
  if (!VK_APP_ID) return res.status(500).json({ success: false, error: 'VK_APP_ID not configured' });
...
router.get('/vk/callback', async (req, res) => {
  try {
    ...
    const tokenRes = await axios.get('https://oauth.vk.com/access_token', {
      params: { client_id: VK_APP_ID, client_secret: VK_SECURE_KEY, redirect_uri: REDIRECT_URI, code }
    });
