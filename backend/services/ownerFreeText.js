...
// [BOTS-FIX] + жалобы/багрепорты («не работают кнопки», «сломалось») — это вопрос о проекте,
// а НЕ задача в очередь Омеги (инцидент 2026-09-05: жалоба владельца ушла в очередь → пост в канал)
const QUESTION_RE = /(что с |когда |сколько |почему |как идёт|как идет|что по |есть ли |кто |запуск|реклам|клиент|подписк|оплат|тикет|пользовател|не работа|не приход|не дош|не отвеча|сломал|баг|ошибк|кнопк|вопрос|проблем)/i
