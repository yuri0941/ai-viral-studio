...
export async function analyzeOwnerStyle(ownerId, messages = []) {
  ...
  const analysis = await chatWithAI(prompt, { system: 'You are a linguistic analyst. Return ONLY valid JSON.', maxTokens: 800 });
  let profile;
  try { profile = JSON.parse(analysis); } catch(e) { profile = {}; }
...
export async function generateInOwnerStyle(text, ownerId, profile = null) {
  ...
  return await chatWithAI(prompt, { temperature: 0.7, maxTokens: 1500 });
}
...
export async function shouldChallengeOwner(decision, context) {
  ...
  const response = await chatWithAI(prompt, { system: 'You are a cautious but constructive advisor. Return ONLY valid JSON.', maxTokens: 600 });
  try { return JSON.parse(response); } catch(e) { return { shouldChallenge: false }; }
}
