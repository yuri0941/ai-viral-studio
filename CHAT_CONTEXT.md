...
### v9.9.19.15.17-VK-KILL-SWITCH
- Переменная `VK_PUBLISHING_ENABLED=true` включает VK-публикацию; по умолчанию — выключено.
- В выключенном состоянии нет вызовов VK API, нет логов `[vk:*]`, нет алертов; UI показывает «Отключено» и скрывает VK из планировщика.
