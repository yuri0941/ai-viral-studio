...
2. ✅ 19.14.5 выполнен: на странице Подписки табы способов оплаты переключаются, кнопка = выбранный метод, RUB → ЮKassa, ненастроенные методы disabled с причиной
3. Ждать завершения 19.6 (K3 работает)
4. Прокликать по чек-листу из 19.6 (Шаг 9.3)
5. Запустить 19.2 → 19.4 → 19.7 → 19.9 → 19.10 (19.8 поглощён 19.14.4)
6. После каждого: скрин мобильной версии (375px) + скрин консоли
7. Только потом: 20-FACTORY-REAL
