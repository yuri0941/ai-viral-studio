...
print("=" * 60)
print("БЕЗОПАСНОЕ ОБНОВЛЕНИЕ v3")
print("=" * 60)
...
backup = OWNER_FILE + ".safe_backup_v3"
...
if "setImpersonatedCabinet(cabinet)" not in c:
    ...
    if old in c:
        ...
        print("   ❌ handleImpersonate не найден для замены")
else:
    print("⚠️ handleImpersonate уже исправлен")

# 3. Баннер перед toast
if "impersonatedCabinet &&" not in c:
