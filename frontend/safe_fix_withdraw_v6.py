...
# -*- coding: utf-8 -*-
"""
safe_fix_withdraw_v6.py — Безопасное добавление 6 типов вывода средств в OwnerDashboardPage.jsx
Включает: Юр. лицо, ИП, Карта, SWIFT/IBAN, Крипта (USDT/BTC/ETH), PayPal
"""
...
def find_line_number(content, substring):
    """Находит номер строки для подстроки."""
...
print("=" * 70)
print(" SAFE FIX v6 — Вывод средств: 6 типов реквизитов + Крипта")
print("=" * 70)
...
if "Bitcoin" not in c:
    # Найдём строку с импортами и добавим Bitcoin
    import_pattern = r"(from 'lucide-react')"
    if re.search(import_pattern, c):
        # Найдём строку с Zap и добавим Bitcoin рядом
        ...
        if old_import in c:
            ...
            if old_import2 in c:
                ...
                errors.append("   ⚠️ Не удалось добавить Bitcoin в импорты — добавьте вручную")
                print("   ⚠️ Не удалось добавить Bitcoin в импорты — добавьте вручную")
    else:
        errors.append("   ⚠️ Блок импортов lucide-react не найден")
        print("   ⚠️ Блок импортов lucide-react не найден")
else:
...
if "const [withdrawType, setWithdrawType]" not in c:
    ...
    print("   ✅ Состояния withdrawType и withdrawRequisites уже есть")


# ═══════════════════════════════════════════════════════════════════
# ШАГ 3: Добавить состояние payments (если нет)
# ═══════════════════════════════════════════════════════════════════
...
if "const [payments, setPayments]" not in c:
    ...
    if marker in c:
        # Проверим, не добавлено ли уже payments после showPassword
        idx = c.find(marker)
        after = c[idx:idx+200]
        ...
        errors.append("   ❌ Маркер showPassword не найден")
        print("   ❌ Маркер showPassword не найден")
else:
...
if old_handle in c:
    ...
    # Проверим, может уже новая версия
...
# ═══════════════════════════════════════════════════════════════════
# ШАГ 5: Заменить блок Finance Tab (история из payments)
# ═══════════════════════════════════════════════════════════════════
...
if changed and all_ok:
    ...
    print("\n" + "=" * 70)
    print("✅ ГОТОВО! Перезагрузи страницу в браузере (F5)")
    print("=" * 70)
    print("\n📋 ЧТО ДОБАВЛЕНО:")
    ...
    print("  5. 🔒 Проверка заполненности реквизитов перед выводом")
