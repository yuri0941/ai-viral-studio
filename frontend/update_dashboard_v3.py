...
# Скрипт для обновления Sidebar.jsx + OwnerDashboardPage.jsx — ВЕРСИЯ 3
# Добавляет: кабинеты в sidebar, подписки с редактированием цен, localStorage
# Запуск: python update_dashboard_v3.py

import os
import re

SIDEBAR_PATH = r"D:\kilo2\frontend\src\components\Sidebar.jsx"
...
def backup(path):
    backup_path = path + ".backup_v3_" + str(int(__import__('time').time()))
...
def update_owner_page():
    ...
    # Добавляем useEffect для синхронизации с localStorage
    old_useauth = "const { user, logout } = useAuth()"
    new_useauth = """const { user, logout } = useAuth()

    // Синхронизация с localStorage
    useEffect(() => {
        localStorage.setItem('owner_cabinets', JSON.stringify(cabinets));
    }, [cabinets]);"""

    # Проверяем есть ли уже useEffect
    if 'useEffect' not in content[:5000]:
...
def main():
    print("=" * 70)
    print("ОБНОВЛЕНИЕ SIDEBAR + OWNER DASHBOARD — ВЕРСИЯ 3")
    print("=" * 70)
    print()
    ...
    if success1 and success2:
        ...
        print("  • Данные НЕ удалятся при обновлении страницы")
    else:
