...
print("=" * 60)
print("ОБНОВЛЕНИЕ v5 — OwnerDashboardPage.jsx")
print("=" * 60)
...
# 2. Добавляем редактирование API ключей
if "editingApiKey === i" not in c:
    ...
    if old_api in c:
        ...
        print("   ❌ Блок API ключей не найден")
else:
...
# 4. Вкладка Подписки в навигации
if "Подписки" not in c:
    print("🔧 Добавляем вкладку Подписки...")
    ...
    if old_nav in c:
        ...
        changed = True
        print("   ✅ Вкладка Подписки добавлена")
    else:
        print("   ❌ Навигация не найдена")
else:
    print("⚠️ Вкладка Подписки уже есть")

# 5. Контент вкладки Подписки
if "activeTab === 'subscriptions'" not in c:
    print("🔧 Добавляем контент Подписок...")
    old_content = "{activeTab === 'servers' && ("
    new_content = "{activeTab === 'subscriptions' && (\n        <div className=\"space-y-6\">\n          <div className=\"flex items-center justify-between\">\n            <h2 className=\"text-xl font-bold text-white flex items-center gap-2\">\n              <CreditCard className=\"text-[#00ff41]\" size={20} />\n              Тарифные планы\n            </h2>\n            <button className=\"px-4 py-2 bg-[#00ff41] hover:bg-[#00cc33] text-black font-medium rounded-lg transition-all\">\n              + Добавить тариф\n            </button>\n          </div>\n          <div className=\"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4\">\n            {subscriptions.map((sub) => (\n              <div key={sub.id} className=\"bg-[#1a1a24] border border-[#2a2a3a] rounded-xl p-5 hover:border-[#00ff41]/30 transition-all\">\n                <div className=\"flex items-center justify-between mb-4\">\n                  <h3 className=\"font-bold text-white\">{sub.name}</h3>\n                  <span className={`px-2 py-1 rounded text-xs font-medium ${sub.popular ? 'bg-[#00ff41]/20 text-[#00ff41]' : 'bg-gray-800 text-gray-400'}`}>\n                    {sub.popular ? 'Популярный' : ''}\n                  </span>\n                </div>\n                <div className=\"mb-4\">\n                  <span className=\"text-2xl font-bold text-[#00ff41]\">${sub.price}</span>\n                  <span className=\"text-gray-500\">/мес</span>\n                </div>\n                <ul className=\"space-y-2 mb-4\">\n                  {sub.features.map((feature, idx) => (\n                    <li key={idx} className=\"flex items-center gap-2 text-sm text-gray-400\">\n                      <CheckCircle2 size={14} className=\"text-[#00ff41]\" />\n                      {feature}\n                    </li>\n                  ))}\n                </ul>\n                <div className=\"flex items-center justify-between pt-4 border-t border-[#2a2a3a]\">\n                  <span className=\"text-sm text-gray-500\">{sub.users} пользователей</span>\n                  <button className=\"text-sm text-[#00ff41] hover:text-[#00cc33] transition-colors\">\n                    Редактировать\n                  </button>\n                </div>\n              </div>\n            ))}\n          </div>\n        </div>\n      )}\n\n      {activeTab === 'servers' && ("
    if old_content in c:
        c = c.replace(old_content, new_content)
        changed = True
        print("   ✅ Контент Подписок добавлен")
    else:
        print("   ❌ Контент servers не найден")
else:
    print("⚠️ Контент Подписок уже есть")
...
print("  3. 👑 Вкладка Подписки с тарифами")
print("  4. 📊 Карточки тарифов (Free, Creator, Pro, Agency, Enterprise)")
