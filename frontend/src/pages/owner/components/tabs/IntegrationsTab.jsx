...
const INTEGRATIONS = [
    ...
    { id: 'notion', name: 'Notion', icon: FileText, color: 'text-white', setup: ['notion.so/my-integrations', 'Create integration', 'Database ID'], env: ['NOTION_TOKEN'] },
        ...
        <div className="space-y-6">
            ...
            <div className="flex gap-2 border-b border-white/10 pb-2">
                {['social', 'external', 'webhooks'].map(t => (
                    <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 text-sm rounded-t-lg ${tab === t ? 'text-white border-b-2 border-[#8b5cf6] bg-white/5' : 'text-gray-500 hover:text-gray-300'}`}>
                    ...
                    {data.integrations.map(integ => (
                        <div key={integ.id} className="p-5 rounded-2xl bg-[#0f0f1a] border border-white/5 hover:border-white/10 transition-colors">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <button onClick={() => data.toggleIntegration(integ.id)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${integ.connected ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-white/5 text-gray-400 border border-white/10'}`}>{integ.connected ? 'Подключено' : 'Подключить'}</button>
                            </div>
                            {integ.connected && (
                            ...
                            <div key={integ.id} className="p-5 rounded-2xl bg-[#0f0f1a] border border-white/5 hover:border-white/10 transition-colors">
                                ...
                                {activeForm === integ.id && (
                                    <div className="space-y-2 pt-3 border-t border-white/5">
                                        {integ.id === 'whatsapp' && <>
                                            <input value={form.phone || ''} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} placeholder="Телефон (79990001122)" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                            <input value={form.message || ''} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} placeholder="Сообщение" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                        </>}
                                        {(integ.id === 'slack' || integ.id === 'discord') && <>
                                            <input value={form.channel || form.channelId || ''} onChange={e => setForm(p => ({ ...p, channel: e.target.value, channelId: e.target.value }))} placeholder={integ.id === 'slack' ? 'Канал (#general)' : 'Channel ID'} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                            <input value={form.text || ''} onChange={e => setForm(p => ({ ...p, text: e.target.value }))} placeholder="Текст сообщения" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                        </>}
                                        {integ.id === 'notion' && <>
                                            <input value={form.databaseId || ''} onChange={e => setForm(p => ({ ...p, databaseId: e.target.value }))} placeholder="Database ID" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                            <input value={form.title || ''} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="Заголовок страницы" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                            <input value={form.content || ''} onChange={e => setForm(p => ({ ...p, content: e.target.value }))} placeholder="Содержание" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                            <input value={form.tags || ''} onChange={e => setForm(p => ({ ...p, tags: e.target.value }))} placeholder="Теги через запятую" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                        </>}
                                        {(integ.id === 'clickup' || integ.id === 'trello') && <>
                                            <input value={form.listId || ''} onChange={e => setForm(p => ({ ...p, listId: e.target.value }))} placeholder="List ID" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                            <input value={form.name || ''} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Название задачи/карточки" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                            <input value={form.description || ''} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Описание" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                            {integ.id === 'clickup' && <input type="date" value={form.dueDate || ''} onChange={e => setForm(p => ({ ...p, dueDate: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />}
                                        </>}
                                        {integ.id === 'shopify' && <>
                                            <input type="number" value={form.limit || 5} onChange={e => setForm(p => ({ ...p, limit: e.target.value }))} placeholder="Limit" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                                        </>}
                                        <button onClick={() => test(integ.id)} disabled={loading === integ.id} className="w-full py-2 rounded-lg bg-[#8b5cf6] hover:bg-[#7c3aed] text-white text-sm font-medium disabled:opacity-50 flex items-center justify-center gap-2">
            ...
            {tab === 'webhooks' && (
                <div className="space-y-4">
                    <form onSubmit={createWebhook} className="p-5 rounded-2xl bg-[#0f0f1a] border border-white/5 space-y-3">
                        <h3 className="text-sm font-semibold text-white flex items-center gap-2"><Webhook size={16} className="text-[#8b5cf6]" /> Новый webhook (Zapier / Make)</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <input value={webhookForm.name} onChange={e => setWebhookForm(p => ({ ...p, name: e.target.value }))} placeholder="Название" required className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                            <input value={webhookForm.url} onChange={e => setWebhookForm(p => ({ ...p, url: e.target.value }))} placeholder="https://hooks.zapier.com/..." required className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                        </div>
                        <input value={webhookForm.events} onChange={e => setWebhookForm(p => ({ ...p, events: e.target.value }))} placeholder="События через запятую: new_post, payment, omega_alert (или *)" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                        <input value={webhookForm.secret} onChange={e => setWebhookForm(p => ({ ...p, secret: e.target.value }))} placeholder="Секрет для подписи (опционально)" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none" />
                        <button type="submit" className="px-4 py-2 rounded-lg bg-[#8b5cf6] hover:bg-[#7c3aed] text-white text-sm font-medium flex items-center gap-2"><Plus size={16} /> Добавить</button>
                    </form>

                    <div className="space-y-2">
                        {webhooks.map(wh => (
                            <div key={wh._id} className="p-4 rounded-xl bg-[#0f0f1a] border border-white/5 flex items-center justify-between">
                                <div className="min-w-0 flex-1">
                                    <div className="text-sm font-medium text-white truncate">{wh.name}</div>
                    ...
                    <div className="p-4 rounded-xl bg-white/5 border border-white/5 text-xs text-gray-400">
                        <div className="font-medium text-white mb-2">Шаблоны Zapier / Make:</div>
