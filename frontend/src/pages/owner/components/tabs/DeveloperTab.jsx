    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-semibold text-white">OMEGA API (B2B2B)</h2>
                    <p className="text-sm text-gray-500 mt-1">API для разработчиков: $0.01/запрос + $99/мес базовый. Ключи одобряются владельцем.</p>
                </div>
                <a href={`${docs?.servers?.[0]?.url || ''}/api/v1/omega/docs`} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-white text-sm">
            ...
            <div className="rounded-2xl bg-white/[0.03] border border-white/10 p-5">
                <h3 className="text-sm font-medium text-white mb-4 flex items-center gap-2"><Plus className="w-4 h-4" /> Новый API ключ</h3>
                <div className="flex gap-3">
                    <input value={newKeyName} onChange={e => setNewKeyName(e.target.value)} placeholder="Название приложения" className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-[#8b5cf6]" />
                    <button onClick={handleCreate} className="px-4 py-2 rounded-xl bg-[#8b5cf6] hover:bg-[#7c3aed] text-white text-sm flex items-center gap-2"><Plus className="w-4 h-4" /> Создать</button>
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                {keys.map(k => (
                    <div key={k._id} className="rounded-2xl bg-white/[0.03] border border-white/10 p-5">
                        <div className="flex items-start justify-between mb-4">
                            ...
                            <button onClick={() => handleDelete(k._id)} className="p-1.5 rounded-lg bg-white/5 hover:bg-red-500/10 text-gray-400 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                        </div>

                        <div className="flex items-center gap-2 mb-4">
                            <code className="flex-1 bg-black/30 rounded-lg px-3 py-2 text-xs text-[#00ff41] break-all">{k.key}</code>
                            <button onClick={() => copy(k.key, k._id)} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-gray-400">
                        ...
                        <div className="border-t border-white/10 pt-4">
                            ...
                            <div className="flex gap-2 mb-2">
                                <input value={webhookForm[k._id]?.url || ''} onChange={e => setWebhookForm(p => ({ ...p, [k._id]: { ...p[k._id], url: e.target.value } }))} placeholder="https://your.app/webhook" className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-xs outline-none" />
                                <input value={webhookForm[k._id]?.events || ''} onChange={e => setWebhookForm(p => ({ ...p, [k._id]: { ...p[k._id], events: e.target.value } }))} placeholder="chat, generate" className="w-32 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-xs outline-none" />
                                <button onClick={() => addWebhook(k._id)} className="px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-white text-xs">Добавить</button>
                            </div>
                            {k.webhooks?.map(w => (
                                <div key={w._id} className="flex items-center justify-between text-xs text-gray-400 py-1">
                                    <span className="truncate">{w.url}</span>
                                    <button onClick={() => removeWebhook(k._id, w._id)} className="text-red-400 hover:text-red-300">×</button>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            {docs && (
                <div className="rounded-2xl bg-white/[0.03] border border-white/10 p-5">
                    <h3 className="text-sm font-medium text-white mb-3 flex items-center gap-2"><RefreshCw className="w-4 h-4" /> Endpoints</h3>
