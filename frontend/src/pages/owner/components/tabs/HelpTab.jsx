    ...
    return (
        <div className="space-y-6 max-w-4xl">
            <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-semibold text-white">Помощь и документация</h2>
            </div>
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full bg-[#0f0f1a] border border-white/5 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-600 outline-none focus:border-purple-500/30"
                />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {GUIDES.map((guide, i) => (
                    <a
                        key={i}
                        href={guide.url}
                        className="flex items-center gap-3 p-4 rounded-2xl bg-[#0f0f1a] border border-white/5 hover:border-white/15 transition-all group"
                    >
                        <guide.icon size={18} className="text-emerald-400" />
                        <span className="text-sm text-white flex-1">{guide.title}</span>
            ...
            <div className="space-y-2">
                <h3 className="text-sm font-semibold text-white mb-3">Частые вопросы</h3>
                {filtered.length === 0 && (
                    <div className="text-center text-gray-500 text-sm py-8">Ничего не найдено</div>
                )}
                {filtered.map((faq, i) => (
                    <div key={i} className="rounded-2xl bg-[#0f0f1a] border border-white/5 overflow-hidden">
                        <button
                            onClick={() => toggle(i)}
