    ...
    return (
        <div className="space-y-4">
            ...
            <DataTable
                data={data.promos}
                columns={[
                    { key: 'code', label: 'Код', render: r => <span className="text-sm font-mono font-medium text-white">{r.code}</span> },
                    { key: 'discount', label: 'Скидка', render: r => <span className="text-sm text-white">{r.discount}{r.type === 'percent' ? '%' : '$'}</span> },
