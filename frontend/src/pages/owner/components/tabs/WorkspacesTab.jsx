        ...
        <div className="space-y-6">
            ...
            <div className="rounded-2xl bg-white/[0.03] border border-[var(--border)] p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <button type="button" onClick={handleCreate} className="px-4 py-2 rounded-xl bg-[#8b5cf6] hover:bg-[#7c3aed] text-[var(--text)] text-sm flex items-center justify-center gap-2"><Plus className="w-4 h-4" /> Создать</button>
                </div>
                        ...
                        {editing?._id === ws._id ? (
                            <div className="space-y-3">
                                ...
                                <div className="flex gap-2">
                                    <button type="button" onClick={() => handleUpdate(ws._id)} className="px-3 py-1.5 rounded-lg bg-emerald-500/20 text-emerald-400 text-xs flex items-center gap-1"><Check className="w-3 h-3" /> Сохранить</button>
                                    <button type="button" onClick={() => setEditing(null)} className="px-3 py-1.5 rounded-lg bg-white/5 text-gray-300 text-xs flex items-center gap-1"><X className="w-3 h-3" /> Отмена</button>
                                </div>
                            </div>
                        ) : (
                            <>
                                <div className="flex items-start justify-between mb-3">
                                    ...
                                    <div className="flex gap-1">
                                        {!ws.isDefault && (
                                            <button type="button" onClick={() => handleSetDefault(ws._id)} title="Сделать default" className="p-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-gray-400">
                                                <Star className="w-3.5 h-3.5" />
                                            </button>
                                        )}
                                        <button type="button" onClick={() => setEditing(ws)} title="Редактировать" className="p-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-gray-400">✎</button>
                                        <button type="button" onClick={() => handleDelete(ws._id)} title="Удалить" className="p-1.5 rounded-lg bg-white/5 hover:bg-red-500/10 text-gray-400 hover:text-red-400">
