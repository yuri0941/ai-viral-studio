    ...
    return (
        ...
        `}
            >
                ...
                {/* Collapse Toggle - на старом месте, внизу */}
                <div className="p-3 border-t border-white/5">
                    <button
                        onClick={() => setCollapsed(!collapsed)}
                        className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-white/5 text-gray-400 hover:text-white transition-all text-sm
              ${collapsed ? 'justify-center' : ''}
            `}
                    >
                ...
                <div className="p-4 border-t border-white/5">
                    ...
                    {!collapsed && (
                        <button
                            onClick={logout}
                            className="w-full mt-3 flex items-center gap-2 px-3 py-2 rounded-xl text-red-400 hover:bg-red-500/10 transition-all text-sm hidden lg:flex"
                        >
...
print("✅ Sidebar.jsx создан успешно!")
print(f"📊 Размер: {len(code)} байт")
print(f"📁 Путь: {SIDEBAR_FILE}")
print("")
