...
## 2026-09-23 — PERF-AUDIT: скорость + честность статусов (ветка fix/perf-audit)
- [П1 BASELINE] `scripts/perf-baseline.mjs` (Playwright + CDP-троттлинг, не входит в qa-launch): прод aiviral-studio.ru, mobile 390px, медиана 2 прогонов. Wi-Fi: TTFB 147ms, FCP 584ms, LCP 1576ms, CLS 0.016, INP~56ms. Slow 4G (150ms/1.6Mbps/CPU4x): FCP 4536ms, LCP 5812ms. Slow 3G (400ms/400kbps/CPU4x): FCP 10540ms, LCP 10824ms, INP 240ms. Оценка «~3,85с на 3G» — в правильной зоне (между Slow 4G и Slow 3G), точную цифру 3.85 не подтвердил. Backend-роуты (локальный :18080, owner-токен, 8 прогонов): version p95 11ms, users/me/quota 13ms, upload/limits 11ms, omega/action-prices 11ms, owner/action-analytics 26ms, owner/action-prices 15ms — p95 далеко <2с.
- [П2 БАНДЛ] OwnerDashboardPage: 71 eager-импорт вкладок -> React.lazy + локальный Suspense вокруг renderTab (OverviewTab оставлен eager — дефолтный таб). Чанк 1120КБ -> 96КБ (-91%); initial не вырос: index 568КБ, vendor 180КБ (ровно baseline). Чанков 56 -> 132 (вкладки грузятся по требованию; OMEGACoreTab 428КБ — только при открытии). Валидация фактом: real-data-audit (78 owner-табов + creator + advertiser x dark/light x 1280/360) — ВСЁ ЗЕЛЁНОЕ, 0 pageerror.
- [П3 RE-RENDER ЧАТА] Корень: `useTranslation` пересоздавал t() каждый рендер -> memo у тяжёлых потомков никогда не срабатывал, keystroke в инпуте перерендеривал всю ленту с markdown-парсингом. Фикс: t() теперь useCallback [lang] (ссылка стабильна); `AiMessageContent` и `YouTubeAnalysisCard` обёрнуты в React.memo. N+1 в API: в request-path горячих роутов не найден (петли с find только в кронах/сервисах вне запроса); индексы Mongo по реальным запросам на месте (users: role/createdAt/email-unique, payments: userId+status, логи: action+createdAt + TTL).
- [П4 PWA STALE] В сгенерированном sw.js не было skipWaiting/clientsClaim (0 вхождений фактом) -> новый SW ждал закрытия ВСЕХ вкладок (мобильный PWA = дни на старой версии). Фикс: `self.skipWaiting()` + `clientsClaim()` в src/sw.js + в index.html периодический `reg.update()` (30 мин + на visibilitychange). Смешение версий ленивых чанков ловит VersionCheck-модалка.
- [Д1 STRIPE] Решение владельца: Stripe-контур НЕ удалять (backend routes/stripe.js + server.js + stripeService.js — под будущее, рубильник владельца). Честность статуса в owner-боте `sendStatus`: нет ключа -> «💳 Stripe: не подключён», ключ есть -> «ключ есть, не протестирован» (было ✅ по наличию env при никогда не тестированном контуре). Фронтом не дёргается: CheckoutPage удалён в CHECKOUT-UNIFY, `/stripe-checkout` — orphan lazy-роут без ссылок (факт grep'ом). До включения нужен батч STRIPE-ENABLE (~2-3ч) — факт в AGENTS.md.
- [Д2 CLIENTS] sendStats owner-бота: сырой `User.countDocuments({})` (включал owner/staff/тестовые) -> фильтр ownerController (`status != deleted` + `isTestAccount != true`) + подпись «Клиентов». [Д3] AGENTS.md: факт «AI-ключи в статусах = сохранённые валидные ключи из БД (buildKeyDigest); слоты PROVIDER_CHAIN (11) включают бесплатных/безключевых — не путать».
- [ГЕЙТЫ] qa-launch 27/27 ✅; real-data-audit ВСЁ ЗЕЛЁНОЕ ✅; build 0 ошибок (initial не вырос); i18n-parity 0-diff (новых строк нет); прод curl / 200 x3 + /login 200, 0 редиректов; /api/version прод TTFB ~0.58с.
- [НЕ ТРОНУТО] платежи/ЮKassa, PlanConfig/цены, реестр цен, боты (только текст статуса), house-ads, CHAT-PRO компоновка, COVERS-SUPREME/FRAMES, YT-FRAMES, SMART-TTL, KNOWLEDGE-PACK, entitlement-снапшоты, _redirects.
- [GIT] fix/perf-audit от origin/main (b7661423). Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/perf-audit. Merge — только по ✅ владельца в TG.

## 2026-09-23 — DESIGN-PRO FULL: база design.json + аудит экранов + фиксы по законам UX (ветка fix/design-pro)
- [З0] `backend/knowledge/design.json` — 8 законов UX с порогами (Hick/Fitts/Jakob/Peak-End/Miller/Aesthetic/Goal-Gradient/Doherty), Refactoring UI (иерархия/отступы/типографика/цвет), SaaS-конверсия conv-1..6, WCAG AA (контраст/фокус/aria/motion), мобильные пороги (44px, safe-area, инпуты >=16px), спека состояний, микро-финиш (<=200ms, noBlackout).
- [З1 АУДИТ] `scripts/design-pro-audit.mjs` (не входит в qa-launch — при поднятых :18080/:4173): 7 ключевых экранов x dark/light x 1440/390, скриптуемые пороги из design.json (hScroll, touch-targets с учётом iconButtonMinPx=40 и inline-ссылок, инпуты >=16px, transition <=200ms, сырые i18n, console errors). Тема app-экранов ставится через PUT /users/me preferences.theme (профиль перекрывает localStorage). Первый прогон: 50 находок -> финальный: 0.
- [З1->ФИКСЫ] переходы 0.3s->0.2s глобально (globals/index/luxury/animations.css, DashboardShell, AppSidebar, MobileNotificationDrawer, SidebarDock, TabNavigation, landing header/карточки) — microFinish.transitionMsMax; сайдбар-навигация 42px -> min-h-44px, waitlist лендинга 40px -> 44px + text-base (fitts, iOS-zoom); чат: пилюля баланса 36->44, чипы Поддержка/Telegram 40->44; тур driver.js — склейка «Шаг 1 из 6»+«Пропустить» (margin-left skip-кнопке).
- [З2] Канон состояний: `components/common/EmptyState.jsx` (уже стандарт, 10+ табов) + новый `components/common/ErrorState.jsx` (design.json states.error: что случилось + что делать + retry, role=alert, иконка+текст — rui-c2); мёртвые дубли `shared/EmptyState.jsx`/`shared/LoadingSkeleton.jsx` удалены (0 импортов); ErrorState в wired PredictionCard (сырой текст ошибки убран) и ReferralsTab.
- [З3] Регистрация conv-5: поле «Подтвердите пароль» убрано (<=3 полей, самопроверка глазком), резерв ошибки min-h-[3rem] убран (CTA «Создать аккаунт» был под фолдом 900px — теперь виден), консенты компактнее. qa-journey-1-register обновлён (поле больше не заполняется). Лендинг conv-1..3: hero уже соответствовал (H1 ценность+для кого, 1 primary CTA + ghost «Войти», пруф «Бета-слотов 50/50»).
- [З4] WCAG AA light-тема: DashboardHeader хардкод text-white -> var(--text) (логотип/заголовок белым по белому); luxury.css: light-оверрайды `.chat-pro-page` для элементов вне embed-скопа (тулбар креатива text-gray-200 -> #1f2937, акцент violet-300 -> #7c3aed, glass-card белый) — до embed-правил, source order решает.
- [З5] Таблица «минус конкурента -> наш плюс» (Opus/Klap/Canva) — в TG-отчёте.
- [З6] Микро-финиш: все interaction-переходы <=200ms (аудитом фактом); .reveal 0.7s осознанно оставлен (entrance-анимация скролла, не interaction-фидбек — doherty про отклик); скроллбары/иконки lucide — уже в норме.
- [ОСОЗНАННО НЕ ПРИМЕНЕНО] inline-ссылки в тексте согласий 15px (WCAG target-size inline exception — 44px ломает поток текста); .reveal 0.7s (entrance); driver.js внутренние анимации поповера (библиотека, scope-исключение).
- [ГЕЙТЫ] qa-launch 27/27 (вкл. обновлённый qa-journey-1-register); keys-universal-shots ALL GREEN; omega-video-upload-shots ALL GREEN; covers-frames-shots ALL GREEN; design-pro-audit 0 находок; build 0 ошибок (CSS 160КБ -> 160КБ, дельта ~0); i18n-parity 0-diff; прод / /register /creative-hub/chat -> 200 x3, 0 редиректов.
- [НЕ ТРОНУТО] платежи/ЮKassa, PlanConfig/цены, боты, рубильники, view-as, гард-матрица, CHAT-PRO компоновка PR#60 (только +4px высоты кнопок), COVERS-SUPREME, KNOWLEDGE-PACK, _redirects.
- [GIT] fix/design-pro от origin/main. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/design-pro. Merge — только по approve владельца в TG.

## 2026-09-23 — KNOWLEDGE-PACK FULL: база вирусных знаний Омеги (ветка feat/knowledge-pack)
- [З0 КАРТА] Полная карта «функция → методика → статус»: разбор/обложки/хуки/сценарии/тайтлы/описания/мета-пакет/скоринг/тайминг/ниша — внедрено; планировщик частично (методология каденса в knowledge, логика постинга инженерная); аналитика канала/платежи/боты/кабинет — «не применимо» с причиной. Артефакт: reports/knowledge-pack/coverage-map.md (не коммитится).
- [З1 ЯДРО] `backend/knowledge/*.json` ×7 (hooks — 21 формула MIT Jakeschincariol + структуры Y7–Y10, titles — формулы Y1–Y6/Y11 2026 + линт-листы MIT sergebulaev, packaging — пара-правила/5 non-negotiables, seo — скелет описания/главы 0:00+≥3+≥10с/теги-honesty/query-test, platforms — спеки 7 площадок, algorithm — эвристики/тайминг/кратность-к-медиане, cis — СНГ-паттерны). Загрузчик `services/knowledgeService.js` — hot-reload ≤60с по mtime, без деплоя логики.
- [З4 СКОРИНГ] `services/viralScorer.js` — JS-порт hookscore.py (5 свойств, вердикт 60% среднего + 40% слабейшего звена) и title.py (линт длины/КАПС/vague/AI-маркеров/цифры/front-load), RU-aware (кириллические словари/regex), lintPair дубля «тайтл↔обложка», rankTitles/rankHooks (лучший первым). Без LLM, мгновенно.
- [ВАЙРИНГ] aiService.generateContent (hook/description/tags/script/thumbnail — knowledge-блоки в промптах), youtubeAI generateTitles (варианты со скором и формулой, обратно-совместимо) + generateShortsScript (хук 3с + петля), omega.js generate-hook (формулы + rankHooks), script-from-idea (хук с id формулы, ре-хук 30–60с, тайтл 40–60), analyze-video-upload (формула хука + правила удержания), bestTimeService (окна 2026 из algorithm.json вместо «на вкус модели»), niche-competitors (метод кратности к медиане канала в ответе).
- [З2 ПАРА] cover-variants принимает videoTitle: дубль значимых слов тайтла в тексте обложки вырезается ДО генерации (lintPair), pairCheck в ответе; если остаток — только стоп-слова/пунктуация, фикс НЕ применяется (duplicate-флаг честно остаётся). Фронт: OmegaChat прокидывает videoTitle из videoAnalysis.
- [З3 МУЛЬТИЯЗЫКА] POST /api/omega/meta-package: одна генерация → пакеты × языки (OwnerSettings.packagingLanguages, EN ВСЕГДА первым — приоритет зарубеж; owner GET/POST /api/owner/knowledge-settings выше generic /:entity; клиент читает /api/upload/limits, hot-reload ≤60с) × площадки (youtube SEO+главы, shorts/tiktok/reels ≤40 декларативный, x панч, twitch клип-контекст, telegram). Формулы пересобираются под язык (RU — cis.json), не дословный перевод. AI-фейл/не-JSON → честный success:false. Тайтлы пакетов скорятся, лучший первым. VK/Дзен исключены (platforms.json excluded, getPlatformSpec → null).
- [З4 SELF-OPTIMIZE] POST /api/omega/meta-choice → models/MetaChoiceLog.js (паттерн CoverChoiceLog: байас формул, agreeRate, ≥8 выборок, write-through в procedural-память).
- [ГЕЙТ] backend/scripts/qaKnowledge.js (30 проверок, входит в qa-launch): целостность 7 JSON + валидность regex, скорер EN/RU (формула «The Statistic» 55 vs хайп 21; RU «Цифра-шок»), линт пары EN/RU, ранжирование (★ 94 vs кликбейт 48), guard anon 401/client 403, hot-reload языков (EN принудительно первым, мусор → 400), meta-choice запись + bias, meta-package честный отказ без AI, линт пары в cover-variants фактом (fixed «мой рецепт», 100% дубль → флаг остался).
- [РЕГРЕССИЯ] qa-launch 27/27 ✅ (вкл. qaSecurityFlow, qaOmegaVideo2, qaCustomProviders, qaKnowledge); covers-frames-shots ALL GREEN; omega-video-upload-shots ALL GREEN; build 0 ошибок (index 566КБ — knowledge на бэке, бандл не вырос); i18n-parity 0-diff (новых строк нет); прод curl / + /creative-hub/chat + /owner → 200, 0 редиректов. Локаль/CI расхождений не было.
- [НЕ ТРОНУТО] COVERS-SUPREME целиком (стикер/cutout, Russo One fontkit, AI-скор, niche-пресет, CoverChoiceLog), YT-FRAMES, SMART-TTL, OMEGA-VIDEO, KEYS-UNIVERSAL, реестр цен и цены ✦ (meta-package без списания — ждёт решения владельца, approve-зона), платежи/ЮKassa, refunds, боты, house-ads, CHAT-PRO, _redirects.
- [GIT] feat/knowledge-pack от origin/main. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feat/knowledge-pack. Merge — только по ✅ владельца в TG.


## 2026-09-22 — KEYS-UNIVERSAL: реестр ключей + универсальный слот «Свой провайдер» (ветка fix/keys-universal)
- [З1 АУДИТ] Фактом по коду: карточки уже есть у всех AI-текст (groq/deepseek/openai/openrouter/cerebras/together/fireworks/mistral/cohere/cloudflare/huggingface/pollinations), elevenlabs (TTS, voiceService), replicate (vision/rembg/SDXL), removebg (cutout), youtube (Data API + тест), youtube OAuth, serpapi, chroma, vk×2, telegram×4, yookassa/stripe/paypal, vapid×2, resend/smtp×3. Пробелы: у removebg НЕ было живой валидации (default «assuming valid»); произвольный новый сервис (Kling/Veo) подключался только кодом. Оба закрыты.
- [З2 КАРТОЧКИ] Replicate — карточка и live-валидация на месте (после COVERS-SUPREME не пострадали, keyGuard работает — в логах регрессии виден штатный auto-disable мёртвого локального ключа). ElevenLabs — карточка + live-валидация /v1/voices. YouTube — карточка + живой тест (videos.list 1 ед. квоты). remove.bg — добавлена ЖИВАЯ валидация GET /v1.0/account (бесплатный эндпоинт). Резерв под Kling/Veo — слотом З3.
- [З3 УНИВЕРСАЛЬНЫЙ СЛОТ] Модель `CustomProvider` (отдельная — enum ApiKey не тронут), сервис `customProviderService.js` (валидация живым GET {baseUrl}/models ДО сохранения — невалидный → 400 с честной причиной, молча не сохраняем; вызов OpenAI-совместимый chat/completions; TTL-кэш ротации 30с), роуты `/api/custom-providers` (owner-only, ключ только masked в ответах). Слоты входят в `tryProviders` после встроенной цепочки. Привязка к функциям (chat) из кабинета, вкл/выкл/удаление без деплоя. baseUrl только https (http localhost — тестовый шов). UI: секция «Свои провайдеры» в ApiKeysTab (модалка имя/baseURL/ключ/модель/привязка, статусы, toggle, re-test, удаление с confirm).
- [З4 АВТО-ПРИМЕНЕНИЕ] Пруф фактом (shots-гейт): вставил ключ → «Сохранить и применить» → карточка «✅ Работает» и слот в ротации чата за 3.1с (лимит 60с), функция отдала реальный результат через вставленный ключ. Кнопка «🔄 Проверить» на КАЖДОЙ карточке (старые+новые): POST `/api/api-keys/test-saved` — живой re-test сохранённого ключа, статус обновляется в БД.
- [З5 МЁРТВЫЙ КЛЮЧ] Слот: 401/402/403 → isActive=false + status=invalid + lastError в БД (переживает рестарт), стоп кэша, TG-алерт владельцу (escapeHtml), ротация продолжается на живых провайдерах; провайдер-фейл в чате → фолбэк-цепочка, при полном отказе — smart-fallback с возвратом ✦ (прежний паттерн). Негатив-прогон фактом: stub 401 → auto-disabled в БД + слот выпал из ротации + карточка «Выключен/Ошибка» в UI. Реальный TG-алерт отправлен (owner-бот, HTTP 401 кейс) — владелец видит в @omega_aiviral_bot.
- [З6 PLAYLIST] `extractPlaylistId` + `fetchPlaylistInfo` в youtubeDataService (playlists.list + playlistItems.list ≤50 + videos.list statistics, топ-5 по просмотрам, кэш 1ч). Чат: playlist-URL без v= → отдельная ветка с честным «Это плейлист из N видео» + разбор топ-5 реальными цифрами; недоступно → честный отказ с причиной БЕЗ вызова AI и БЕЗ списания ✦ (refund фактической цены, паттерн YT-DATA-FIX).
- [ГЕЙТЫ] `backend/scripts/qaCustomProviders.js` (25 проверок, входит в qa-launch): guard anon 401 (битый токен — dev-bypass protect!) / client 403 ×3, мёртвый endpoint → 400 и НЕ сохраняется, живой stub → слот в ротации сразу, callCustomProvider реальный текст, 401 → isActive=false фактом в БД + lastError + выпадение из ротации, re-test/включение обратно, PATCH с мёртвым baseUrl → 400 без изменений, apiKey не светится нигде, test-saved 404/403, playlist парсер + честный unavailable/реальные данные. `scripts/keys-universal-shots.mjs` — UI-гейт со скринами (dark/light/390, модалка, карточка, мёртвый ключ, удаление) — ALL GREEN.
- [РЕГРЕССИЯ] qa-launch 26/26 ✅ (вкл. qaSecurityFlow, qaOmegaVideo2, новый qaCustomProviders); covers-frames-shots ALL GREEN; omega-video-upload-shots ALL GREEN; build 0 ошибок (index 568КБ — initial не вырос, UI в lazy-чанке OwnerDashboardPage); i18n-parity 0-diff (15 ключей ×4 файла); прод curl / + /creative-hub/chat + /owner → 200, 0 редиректов. Расхождение локаль/CI: прогон 2 qa-launch дал флаки ❌ journey-1-register (обе retry-попытки по таймауту сразу после рестарта backend); root-check: 3/3 standalone зелёных при тёплом backend, в прогоне 3 — ✅ 105.8s. Не из диффа (diff не трогает register/onboarding).
- [НЕ ТРОНУТО] ApiKeysTab/getProviderKey/hot-reload ядро (расширено, не переписано), keyGuard replicate/youtube, карточки и их валидаторы, платежи/ЮKassa, refunds, боты, house-ads, CHAT-PRO, OMEGA-VIDEO, COVERS-FRAMES/YT-FRAMES/SMART-TTL/COVERS-SUPREME, реестр цен, _redirects.
- [GIT] fix/keys-universal от origin/main. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/keys-universal. Merge — только по ✅ владельца в TG.
## 2026-09-22 — COVERS-SUPREME: обложки уровня топов YouTube (ветка fix/covers-supreme)
- [З1 NICHE-ADAPTIVE] `backend/services/nicheStyleService.js`: пресет стиля из РЕАЛЬНЫХ топ-обложек ниши (youtubeDataService → до 4 thumbnail → паттерны по пикселям: доминантный насыщенный цвет→accentColor, яркость/насыщенность→сила цветокора, skin-доля→faceBias, текст-энергия→textHeavy; эмоция — маркеры в реальных заголовках). Кэш TtlLruCache 30 мин. Нет данных/ключа → честный UNIVERSAL_PRESET (не пусто, не ошибка).
- [З2 МОМЕНТ ПО ЭМОЦИИ] `frameIntelligence.js`: skin-кластеры (YCbCr) = лицо/крупность, saliency-bbox героя (контраст к размытой копии + центр-буст), энергия краёв = действие; vision (Replicate LLaVA, ключ из кабинета) поверх топ-4 кандидатов: эмоция ×1.4 ПОБЕЖДАЕТ контраст. Нет ключа/сбой → эвристика (честный фолбэк). 401/403 → disableProviderKey.
- [З3 СТИКЕР] `coverFx.js#buildSticker`: объект 30–50% высоты обложки + толстая светлая обводка (альфа→blur→threshold, ~4.5% высоты) + внешнее свечение (screen). PRO: ключ remove.bg (envMap `removebg` + карточка в ApiKeysTab, consoleLink) → полноценный cutout; фолбэк Replicate rembg; без ключей — органичная перо-эллипс маска (эллипс 84% bbox — иначе скруглённый прямоугольник с тёмным кольцом, поймано прогоном). ✦ за PRO НЕ списываются.
- [З4 ФОН] Всегда: цветокор (modulate saturation/brightness + linear contrast), виньетка, радиальное свечение акцентом ниши за объектом, blur-fill (blur 40 + brightness 0.5) при чужом aspect (log-ratio ≥0.16) — вертикаль из горизонтали без кропа объекта. PRO (Replicate): img2img-стилизация лучшего кадра по vision-сцене (`stylizeBackgroundPro`, текста в промпте НЕТ). Anti-bot/thumbnail-фолбэк — тот же пайплайн.
- [З5 ТИПОГРАФИКА] `coverTypography.js`: шрифт Russo One (OFL, кириллица; Anton/Bebas кириллицы НЕ имеют — факт по charset файлов Google Fonts) из репо (`backend/assets/fonts/RussoOne-Regular.ttf`, 39КБ), рендер в SVG-пути через fontkit (новая прямая зависимость; fontconfig не нужен — Render-независимо). UPPERCASE ≤3 строк, авто-фит максимизации fontSize (блок 25–40% кадра, ширина ≤90%), градиент букв + paint-order stroke + feGaussianBlur-свечение + 3D-экструзия, акцентное слово хука (число/hook-маркер) цветом пресета. Вертикаль: текст СВЕРХУ крупно (layoutOverride top-center/top-left), объект снизу.
- [З6 AI-СКОР] `coverScore.js`: CTR-чек-лист фактом по собранному jpeg (subject/emotion/textContrast по полосе текста/colorPop/readability 320px+160px) → 0–100, сортировка (лучший первый, `best:true`), фронт: бейдж скора на тайле + ★ лучшему. Выбор клиента → `POST /api/omega/cover-choice` → `CoverChoiceLog` → `getChoiceBias` (≥8 выборок) подстраивает порядок схем композиции + write-through в procedural-память Омеги.
- [З7 ФОРМАТЫ] `GET /api/omega/cover-export?url&platform`: blur-fill переформат своего cover-*.jpg (чужой → 403), кнопки YouTube/Shorts/VK/TG под сеткой (скачивание blob с auth).
- [ГЕЙТЫ] qaOmegaVideo2 §5f: стикер organic+обводка (белые пиксели фактом), текст-блок ≥25% (0.38), blur-fill маркеры (полоса размыта stdev≈0 + затемнена, fg резкий полной яркости), strip текста из AI-промпта, choice-лог + 401/400 негативы, cover-export ×4 (1280×720/1080×1920) + чужой 403. covers-frames-shots: +AI-скор у всех 3, лучший первый, текст-блок ≥25%, стикер, бейдж/пресет-чип/ряд форматов в DOM, cover-export ×4, stdev blur-полосы — ALL GREEN. omega-video-upload-shots ALL GREEN. Найдено гейтом: sharp `stats()` игнорирует extract() в пайплайне (считает по входу) — регионы материализуются toBuffer() перед stats (фикс в scoreFrameBase centerBoost — раньше всегда давал 0).
- [РЕГРЕССИЯ] qa-launch 25/25 ✅; build 0 ошибок (index 577КБ vs 556КБ — +21КБ UI обложек, initial <1МБ в норме); i18n-parity 0-diff (12 ключей × 4 файла: chat.cover* + apiKeys.where/requiredFor); cover-*.jpg вне TTL/свипов не тронуто. Локаль=CI-расхождений нет (qa-bots зелёный локально — webhook на проде жив).
- [НЕ ТРОНУТО] COVERS-FRAMES/YT-FRAMES пайплайны (кадры, noUpscale, reason-логи, kill-switch), SMART-TTL, YT-DATA честный отказ без списания ✦, OMEGA-VIDEO списание/refund, анализ конкурентов на реальных данных, реестр цен (цена обложек та же `coverGenerationCostCredits`), платежи/ЮKassa, refunds, боты, house-ads, qa-харнес, CHAT-PRO. Порядок вариантов в гейте §5b переведён с «лучший момент первый» на «AI-скор desc» (новая спека З6) — момент-порядок проверяется отдельно в pickBestFrames.
- [GIT] fix/covers-supreme от origin/main. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/covers-supreme. Merge — только по ✅ владельца в TG.
...
- [З2 ЗАЩИТЫ/ФОЛБЭКИ] Любой сбой (anti-bot, приватное, лимиты, нет бинаря, `YT_FRAMES_DISABLE=1`) → null + `getLastYtFramesSkip()` reason → молча прежний thumbnail-фолбэк, клиенту НЕ ошибка (фолбэк-цепочка thumbnail → AI живая). videoId валидирован регексом 11 символов — мусор в fetch не уходит, spawn без shell.
- [З3 РАЗЛИЧИМОСТЬ] Основной режим — 3 РАЗНЫХ кадра (pickBestFrames × композиции): живой прогон dQw4w9WgXcQ → frameIndex 1,0,8, diff 67.5/51.9/54.8. Поймано гейтом: noUpscale на ВСЕХ вариантах давал обложки 896×504 (кроп 0.7 от 720p) — noUpscale теперь только v0, зум-кропы v1/v2 увеличивают область до размера платформы (все 3 → 1280×720). Фолбэк (1 thumbnail) — ДОР-2 кропы/схемы на месте.
- [ГЕЙТЫ] qaOmegaVideo2 §5e: ffmpeg-извлечение 9 кадров 640×360 нативных, файл удалён фактом (`exists=false`), 3 разных frameIndex + diff>8 через тестовый шов `ytFramesProvider`, фолбэк без ошибки, disabled/bad_id → null+reason. covers-frames-shots: youtube-флоу живьём → source=youtube-frames, 3 разных момента, 1280×720; недоступное видео → фолбэк-цепочка без ошибки клиенту; таймауты proxy/grid 150–180с под скачивание ~60–90с. ALL GREEN.
...
- [ГЕЙТЫ] qaOmegaVideo2 §5d: один кадр → 3 варианта, попарный пиксельный diff 64×36 raw > 8 (факт 16.6/45.6/39.0). covers-frames-shots: diff сетки > 6 по фактически загруженным URL (видео 33.2/26.0/41.9; YouTube 25.6/46.5/49.7 — раньше был бы diff только от мелкого текста). Пруф-скрины: yt-1280-dark-grid.png (3 различимых превью), compare-old-vs-new.png (640px-апскейл vs full-res фон), after-reload (F5, 200).
- [РЕГРЕССИЯ] qaOmegaVideo2 зелёный (вкл. §5b/§5c/§5d, крон cover-*.jpg не трогает); covers-frames-shots ALL GREEN (вкл. F5-персистентность); omega-video-upload-shots ALL GREEN; build 0 ошибок (index 574.59КБ — фронт не тронут, бандл не вырос); i18n-parity 0-diff; прод curl / + /creative-hub/chat + /owner → 200, 0 редиректов. qa-launch 23/24 ✅ (все кодовые гейты зелёные, вкл. qaSecurityFlow/qaOmegaVideo2/qaCreditsFlow). Расхождение локаль/CI: qa-bots ❌ локально — getWebhookInfo owner-бота отдаёт пустой url (прод-состояние, НЕ из диффа; гард tgWebhookGuard — локально webhook не трогаем, в CI TG-ключей нет → скип). CI run 34889060083 — SUCCESS (скрин в TG-отчёте).
- [НЕ ТРОНУТО] сданная часть COVERS-FRAMES (3 композиции, скрим+обводка, размеры платформ, превью 1:1 + «Скачать», «🎨 Перегенерировать стиль — N✦», живой чип цены, strip кадров, вечный инвентарь обложек), full-res механика ДОР (noUpscale, fullFrames), реестр цен, платежи/ЮKassa, refunds-контур, боты, house-ads, qa-харнес, CHAT-PRO, OMEGA-VIDEO (vision/списание/refund/TTL исходника).
- [GIT] fix/covers-frames (та же ветка, мерж — только по ✅ владельца). Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/covers-frames

## 2026-09-14 — COVERS-FRAMES ДОРАБОТКА: full-res фон обложек + битая картинка (ветка fix/covers-frames)
- [З1 РАЗРЕШЕНИЕ] Фон обложки больше не собирается из 640px vision-кадров (2x апскейл = мыло). Клиент (`OmegaChat.extractVideoFrames`) извлекает ДВА набора кадров одних таймкодов: 640px (vision+скор, `action.frames`) и полноразмерные до 1920 по ширине без апскейла выше исходника (jpeg 0.8, `action.fullFrames`; strip в saveHistory вместе с frames — localStorage не раздувается). Сервер: `/api/omega/cover-variants` принимает `fullFrames` (≤6 × ≤4.5МБ base64); `generateCoverVariants` скорит по 640px (`pickBestFrames`), рендерит из полноразмерного кадра того же индекса (`composeCover noUpscale` — оверлей по фактическим размерам после resize, иначе composite падает «same dimensions or smaller»); вариант отдаёт реальные width/height + `bgFullRes`. Нет fullFrames → легаси-рендер из 640px (старые сообщения, qaOmegaVideo2 §5b зелёный без изменений).
- [З2 БИТАЯ КАРТИНКА — 3 корня, все закрыты фактом] (в) `helmet()` ставил `Cross-Origin-Resource-Policy: same-origin` — на проде фронт (aiviral-studio.ru) встраивает `<img>` с aiviral-backend.onrender.com → браузер резал ответ: для `/uploads` CORP=cross-origin (server.js). (а) mediaCleanup удалял обложки: сирота-свип (MediaFile analyzedAt null + deleteAt null, старше 24ч) + 7-дневный/лимитный fs-свип (isReferenced смотрел только ScheduledPost) — теперь запись обложки kind `cover` + `analyzedAt` при создании, `cover-*.jpg` пропускается во ВСЕХ свипах (`isCoverFile`), TTL исходного видео на обложку НЕ распространяется (deleteAt не ставится — вечный инвентарь). (б) сетка была in-memory (`coversByMsg`) → после F5 пропадала: результат — персистентное сообщение `action.type:'covers'` (variants = только URL) в истории чата, «Перегенерировать стиль» обновляет его на месте (`updateMessage` в useOmegaChat → спред `{...chat}`), выбор варианта — локальный state; кадры для регенерации ищутся по `sourceMsgId` в живой ленте (после F5 stripped → честный AI-фолбэк, задокументировано).
- [ГЕЙТЫ] qaOmegaVideo2 §5c: рендер из полноразмерного кадра (зелёный маркер 1920×1080 → 1280×720, g-канал доминирует), noUpscale (960×540 не растягивается до 1280×720), крон НЕ удаляет cover-*.jpg (legacy-запись 25ч + mtime 8д). covers-frames-shots дополнен: ширина фона ≥1280 при исходнике 1920 (факт 1280×720), зелёный маркер фона (rgb=33,74,37), naturalWidth>0 + URL 200 сразу и после F5, сетка в ленте из истории после reload — ALL GREEN (27 проверок). Фикс гейта: init-script сидил историю поверх навигаций — теперь не перезатирает существующую (иначе reload-проверка не имела смысла).
- [РЕГРЕССИЯ] qa-launch 23/24 ✅ (все кодовые гейты зелёные, вкл. qaSecurityFlow/qaOmegaVideo2/qaCreditsFlow); omega-video-upload-shots ALL GREEN ✅; build 0 ошибок (index 564КБ, initial <1МБ); i18n-parity 0-diff (1 ключ × 4 файла); прод curl / + /creative-hub/chat + /owner → 200, 0 редиректов. Расхождение локаль/CI: qa-bots ❌ локально — getWebhookInfo owner-бота отдаёт пустой url (прод-состояние, НЕ из диффа: локальный backend webhook не трогает — гард tgWebhookGuard в логе «не прод-хост… не трогаю»; в CI TG-ключей нет → скип). setWebhook локально запрещён — ждёт решения владельца/проверки на прод-хосте.
- [НЕ ТРОНУТО] сданная часть COVERS-FRAMES (3 варианта × композиции, скрим+обводка, размеры платформ, превью 1:1 + «Скачать», «🎨 Перегенерировать стиль — N✦», живой чип цены, strip кадров из истории — кадры чистим, обложка живёт), реестр цен, платежи/ЮKassa, refunds-контур, боты, house-ads, qa-харнес, CHAT-PRO, OMEGA-VIDEO механика (vision-разбор/списание/refund/TTL исходника), entitlement-снапшоты, _redirects.
- [GIT] fix/covers-frames (та же ветка, мерж — только по ✅ владельца). Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/covers-frames

## 2026-09-14 — COVERS-FRAMES: обложки из реальных кадров видео / YouTube-thumbnail, Pollinations — по кнопке (ветка fix/covers-frames)
- [З1 Основа = реальный кадр] `coverGenerator.js` переписан: mode 'frame' (дефолт) — 3 лучших кадра по sharp-скору (контраст σ каналов + энтропия + центр-буст, `pickBestFrames`) из dataURL-кадров, которые клиент УЖЕ извлекает для vision-разбора (OmegaChat: кадры сохраняются в `action.frames` сообщения разбора; в localStorage-историю не пишутся — strip в `saveHistory` useOmegaChat, иначе 5МБ-квота). YouTube-ссылка: официальный thumbnail максимального качества по факту (`fetchYouTubeThumbnail`: maxresdefault→sddefault→hqdefault, 404/<6КБ/<320px = следующее качество; `extractYouTubeId` — только youtube/youtu.be домены, чужой URL не уходит в fetch). Кадр = фон, не текстовый промпт.
- [З2 Оверлей] sharp-оверлей: скрим-градиент под текстом + обводка (paint-order stroke) + тёмная плашка (композиция 0); размеры платформ фактом (YouTube/VK/TG 1280×720, Shorts/Reels/TikTok 1080×1920). Превью 1:1 + «Скачать» прямо из модалки (`cover-preview-download`), скачать из сетки — как было.
- [З3 Три варианта] 3 разных кадра × 3 композиции текста (bottom-center плашка / bottom-left скрим / top-left скрим); гейт читаемости textHeightRatio ≥ COVER_TEXT_MIN_RATIO (≥10px в сетке 320px) — прежний, зелёный.
- [З4 Text-to-image не удалён] Pollinations = отдельная кнопка «🎨 Перегенерировать стиль — N✦» (mode:'ai', source:'ai'), НЕ дефолт; появляется только когда варианты из кадра/thumbnail. Роут `/api/omega/cover-variants`: frames (≤6 dataURL ≤350КБ, валидация), sourceUrl (только YouTube), mode; ответ несёт `source`/`frameIndex`.
- [З5 Цена] Без изменений: `coverGenerationCostCredits` из кабинета (hot-reload), реестр не дублирован; живой чип цены на кнопках — фактом в shots-гейте («Обложки — 1✦»/«Перегенерировать стиль — 1✦»).
- [ГЕЙТ] Новый `scripts/covers-frames-shots.mjs` (не входит в qa-launch — отдельно при :18080/:4173): seeded видео-разбор с кадрами → API отдаёт provider video-frame, сетка 3 превью с кадрами ЭТОГО видео; YouTube-карточка → provider youtube-thumbnail (maxresdefault по факту); превью 1:1 + скачать; перегенерация стиля; 390px; 0 console errors — ALL GREEN. qaOmegaVideo2 §5b: frame-режим 3×1280×720 source=frame, шумный кадр топ-скор фактом, extractYouTubeId (watch/youtu.be/shorts/чужой домен), mode ai игнорирует кадры.
- [РЕГРЕССИЯ] qa-launch 24/24 ✅; build 0 ошибок; бандл НЕ вырос (index 574.44 vs 574.29КБ на origin/main); i18n-parity 0-diff (1 ключ × 4 файла); прод / /creative-hub /owner → 200, 0 редиректов. CI: run 34818175143.
- [НЕ ТРОНУТО] реестр цен действий, аналитика расхода, платежи/ЮKassa, refunds-контур, PlanConfig/CreditPack, house-ads, _redirects, боты, ask-owner, CHAT-PRO, OMEGA-VIDEO механика (drag&drop/vision/списание/refund/TTL), entitlement-снапшоты.
- [GIT] fix/covers-frames от origin/main, 3 коммита. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/covers-frames. Merge — только по ✅ владельца в TG.

- [З1 ADMIN-REFUNDS — вердикт фактом] Контур ЖИВОЙ: роуты смонтированы (`server.js:660` → `/api/admin`), дёргаются из кабинета (MonetizationTab → GET/POST `/admin/refunds`). По решению владельца — миграция, не удаление: `refundService` переписан на Mongo `Refund` (in-memory REFUNDS умирал на рестарте Render) + реальный `createRefund` ЮKassa по паттерну adminRefundHandler (чек 54-ФЗ, Payment→refunded, сбой → 502 + TG-алерт с HTML-экранированием). Выключенный ключ = полный запрет (400, mock-completed выпилен), без paymentId — честный 400. env-фолбэк ЮKassa в `utils/integrationStatus.js` выпилен (HOTFIX-FINAL-2), мёртвый `backend/config/payments.js` удалён. Негативы: qaSecurityFlow B(client)/C(staff) → 403 на GET/POST/process — зелёные.
- [З2 ОБХОД] `scripts/real-data-audit.mjs` расширен до полной матрицы: 78 owner-табов + 9 creator-экранов + 2 advertiser × dark/light × 1280/360 = 356 проходов × (мок-паттерны, сырые ключи, pageerror, горизонтальный скролл) + часть B (реестр цен B1–B4). ИТОГ: ВСЁ ЗЕЛЁНОЕ, 0 провалов. Поймано обходом: мок-уведомления TechBrand/$29/«8.5/10» в `useNotifications.js` (глобальный дефолт в localStorage каждому юзеру) — выпилены, ключ v2; «Mock-оплата» на /signup (disabled-кнопка + «активируется для теста») — честный текст про оплату в ЛК через ЮKassa.
- [З3 МЁРТВЫЕ ФУНКЦИИ] Чек-лист «элемент → клик → факт»: agents/campaigns/promos/news CRUD были local-only при живых кнопках → переведены на реальный generic CRUD `/owner/<entity>` (AIAgent в ENTITY_MAP). Модалки addTask/addApiKey/createAgent/sendEmail/omegaApproval — без единого открывателя (факт grep) → удалены с файлами и хендлерами (state tasks/apiKeys/approvalRequests тоже мёртвый). toggleIntegration/updateStaff/removeStaff/updateCabinetStatus/impersonateCabinet — серверных эндпоинтов нет → честные error-тосты (паттерн toggle2FA). Хардкод-логи «Pricing Agent ::» в AgentsTab → empty-state. TasksTab — локальный трекер владельца (localStorage, честно подписан) — оставлен.
- [З4 СВЕРКА С ТЗ 1/2] Все пункты подтверждены статикой/прогонами; найдено и закрыто: спавн агента из OMEGA Core фактически падал (AIAgent.id required без default → validation error при POST /owner/agents) → добавлен default ObjectId, спавн подтверждён API-фактом (201). *.bak-архивов в репо нет (пункт 1/2 закрыт фактом). З5.3 B1–B4 зелёные на этой ветке.
- [ИНФРА/ХАРНЕС] qaSupportFlow/qaReferralFlow/qaAddonsFlow/qaWaitlistFlow имели хардкод `http://localhost:18080` → переведены на `QA_API_URL || 18080` (паттерн остальных). Локальный e2e в условиях параллельного контура — через git worktree (`.tmp-rd2`) + свои порты (backend :18081, preview :4180) + JWT_SECRET dev-fallback в env прогона.
- [РЕГРЕССИЯ] qa-launch 24/24 ✅ (прогон на чистой БД, изолированный контур); real-data-audit 356/356 ✅; build 0 ошибок, index 574.29КБ (main = 574.98КБ — бандл не вырос); i18n-parity 0-diff; прод curl: / + /owner + /creative-hub/chat → 200, 0 редиректов.
- [ЖДЁТ РЕШЕНИЯ ВЛАДЕЛЬЦА] (a) staff: реальное редактирование/удаление аккаунтов (сейчас честный отказ — удаление данных = approve-зона); (b) App.jsx.bak/App.jsx.bak2/OwnerDashboardPage.jsx.* архивы в frontend/src — не бандлятся, удаление только с ✅ (повторно, с 1/2).
- [НЕ ТРОНУТО] платежи/ЮKassa (создание/webhook/adminRefundHandler), PlanConfig/CreditPack, house-ads, _redirects, боты, ask-owner, CHAT-PRO, OMEGA-VIDEO механика, entitlement-снапшоты, qa-харнес (только параметризация API URL).
- [GIT] ветка fix/real-data-2. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/real-data-2

## 2026-09-13 — MEMORY-FIX: диагностика и снижение RAM на проде (ветка fix/memory-footprint)
- [КОНТЕКСТ] Anti-Fail алертил RAM FAIL 516MB/498MB за 15 мин при потолке Render Free 512MB — сервис жил на грани OOM-рестарта. Старый порог FAIL 480MB алертил при штатной работе у границы.
- [З1 ДИАГНОСТИКА] Новый `backend/utils/memoryLog.js`: снапшот `process.memoryUsage()` (rss/heapUsed/heapTotal/external) раз в 5 мин из healthMonitor + точечно на vision-разборе (`routes/omega.js` analyze-video-upload), бэкапе (disasterRecovery runBackup), загрузке видео (routes/upload.js). Инвентарь безлимитных структур (факт по коду): in-memory fallback redisClient (TTL только при чтении, без лимита), aiService responseCache (TTL 1ч, без лимита), demo.js cache, vectorStore memoryFallback (лимит на записи юзера, но не на число юзеров), omegaBot supportState/videoState/freeTextRate/takeoverCache (мёртвые chatId навсегда), aiGateway userRequests, financeService FINANCE_ENTRIES, pushController subscriptions, memoryLayer TTL применялся только при restore. Уже ограниченные (не тронуты): monitoringService (prune по окну + MAX_HISTORY), rateLimiter ipMinCounter (sweep), vk stateStore, telegramConnectStore, boardroom/performanceMonitor/autoRecovery/omegaResourceManager (жёсткие лимиты).
- [З2 ФИКСЫ] Новый хелпер `backend/utils/ttlLruCache.js` (TTL + LRU + лимит записей + sweep). redisClient fallback → TtlLruCache(1000, sweep 60с); aiService responseCache → TtlLruCache(1000); demo.js → TtlLruCache(500); vectorStore fallback → LRU по юзерам (200); omegaBot sweep 10мин (freeTextRate 1ч, takeoverCache 5мин, videoState 30мин, supportState 24ч) с unref; aiGateway sweep 5мин неактивных userRequests; financeService лимит 5000; pushController лимит 5000 подписок; memoryLayer TTL-прочистка при каждой записи (раньше — только при restore). Юнит TtlLruCache: LRU-вытеснение/touch/TTL-экспирация — OK фактом.
- [З2 СТРИМИНГ] Бэкап: `createZipArchive` переведён на `JSZip.generateNodeStream({streamFiles:true})` + createReadStream — gz-файлы и архив больше не собираются в RAM целиком (пруф: zip создаётся, контент совпадает). Upload `/api/upload/media` переведён с multer memoryStorage на diskStorage — видео до 250МБ стримится сразу в `uploads/<userId>/`, не поднимаясь в heap; буфер читается только для HEIC/sharp (картинки), при отказе/ошибке temp-файл удаляется. `/image` остался memoryStorage (лимит 10МБ, нужен sharp).
- [З3 ПОРОГИ] healthMonitor: WARN 420MB (console.warn), FAIL 490MB; RAM-алерт с cooldown 30 мин, повтор раньше — только при росте RSS сверх предыдущего алерта; после восстановления цикл сбрасывается. Не-RAM сбои (MongoDB и т.п.) алертят сразу, как раньше. getHealthStatus теперь отдаёт memory-снапшот.
- [РЕГРЕССИЯ] qa-launch 24/24 зелёных (включая qaSecurityFlow, qaOmegaVideo2, qaCreditsFlow); `npm run build` 0 ошибок; i18n-parity OK; curl прода / и /creative-hub → 200. node --check всех 16 изменённых файлов чист.
- [НЕ ТРОНУТО] платежи/ЮKassa, боты (логика), house-ads, реестр цен, OMEGA-VIDEO механика, CHAT-PRO, _redirects, гард-матрица. Функциональность не урезана — только лимиты/стриминг/очистка.
- [ПРУФЫ ПАМЯТИ] Локальный прогон с [MEM]-логом: см. отчёт в TG (boot rss=293MB, динамика 30 мин). До/после на проде будет видно по [MEM]-строкам в Render-логах после деплоя — для этого и добавлен логгер.
- [GIT] ветка fix/memory-footprint. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/memory-footprint
 

## 2026-09-13 — REAL-DATA ЧАСТЬ 1/2: 0 моков, реестр цен действий ✦, ключи через ApiKeys (ветка fix/real-data)
- [З2 ЯДРО] `backend/controllers/ownerController.js` переписан на реальные агрегаты: totalUsers (User), MRR/платящие (calcMRR из Subscription), доход 30д (Payment succeeded), расходы (expenseTracker/InfraExpense), тарифы (PlanConfig + фактические счётчики юзеров), серверы/интеграции/промо/новости/аудит — только БД, пусто → []. `backend/data/ownerFallbackData.js` УДАЛЁН. Фронт `useOwnerData.js`: старт пустой, API авторитетен (пустой массив не подменяется моком), localStorage-кэш метрик запрещён (NO_CACHE_KEYS), фейковый джиттер серверов убран.
- [З2 ЭКРАНЫ] Owner Обзор: KPI MRR/юзеры/AI-вызовы/платящие из /owner/overview (Uptime 99% удалён — источника нет). AIAnalyticsTab: реальный churn (selfImprovement) + MRR; «прогноз $»/«+$5» → честные empty-state. OmegaFinanceTab: реальный MRR из API, ₽ вместо $, крипто/портфели Quick/Growth/Wealth → empty-state «нет реального источника». OMEGACoreTab: статусы провайдеров реальные (/owner/ai-providers/status), выдуманные % кругов/телеметрия агентов/логи/статистика → empty-state, фейковые спарклайны и «+12%» убраны. AnalyticsPage (creator): хардкод-серии 12K→35K/89K→134K, топ-видео, «аудитория по времени», +23%/+324 — удалены; графики рисуются только из stats.series API, иначе empty-state. CreatorDashboardPage: график виральности/платформы из API или empty-state, ачивки из реальной статистики, стрик без хардкода 7/7, доход в ₽, выдуманная раскладка дохода 45/35/15/5 удалена. NotificationsTab: фейковые «$15 000»/«+$5»/«чужой IP» удалены (ключ хранилища v2). TasksTab: мок-задачи (AI Worker #2) удалены. FinanceTab: DEMO_TRANSACTIONS и «+15.2%/−4.3%/+22.1%» удалены, транзакции = реальные Payment.
- [З3 МЁРТВЫЕ ФУНКЦИИ] toggle2FA → честное «не подключено на сервере» (2FA в backend нет). rotateApiKey → отказ с указанием ApiKeysTab (фейковая генерация ключа убрана). Spawn-агенты в OMEGA Core → реальный POST /owner/agents + рефетч (был тост-пустышка). Owner-чат с AI-агентом → реальный /omega/chat (был setTimeout «Анализирую…»). Creator: «Опубликовать»/«Редактировать»/«Применить»/«Все посты»/«Все работы» → реальные переходы (были toast-заглушки). Экспорт в AnalyticsPage без данных → честная ошибка вместо фейк-успеха.
- [З4 КЛЮЧИ] ЮKassa: paymentService.createPayment и admin refunds переведены с env на ApiKeys (`getProviderKey`, hot-reload); основные пути (yookassaController/yookassaService/payments/addons) уже были на ApiKeys. Без ключей — честный `unavailable`, не мок-успех.
- [З5.3 РЕЕСТР ЦЕН] OwnerSettings: плоские поля chatMessageCostCredits(1)/linkAnalysis(0)/vision(0)/tts(0)/nicheCompetitors(0) + существующие video/cover/script — дефолты = поведение до батча. `getActionPrices/setActionPrices` кэш ≤60с. Owner GET/POST `/api/owner/action-prices` (выше generic /:entity), GET `/api/owner/action-analytics?days=7|30`. Клиент: GET `/api/omega/action-prices` + `actionPrices` в /api/upload/limits. Списание с `action` во всех платных точках (chat/link_analysis/vision + video/script/cover), refund фактической цены. `consumeGeneration` принимает cost 0 = бесплатно.
- [З5.3.1/2/3] Себестоимость: `AiUsageLog.action` (threading через chatWithAI options.action → tryProviders → logAiUsage), ₽ = USD×(1/0.011 — тот же курс, что /subscriptions/exchange-rate). Маржинальный пол 70% на POST цены: below_cost → 400 «Минимум N✦ при текущей себестоимости» (пруф фактом: синтетический лог 500$ → min 169112✦, 400; откат чистый). Аналитика: ActionSpendLog (TTL 180д) из consumeGeneration → таблица «функция → штук → ✦ → ₽ себестоимость → ₽ маржа», убыточные подсвечены. Кабинет: `ActionPricesCard` в Подписках (цены ✦ + себестоимость + маржа + аналитика 7/30д), i18n ×4.
- [З5.5 РЕФЕРАЛКА] Одна цифра OwnerSettings.referralPercent везде: ReferralsTab без DEMO_DATA и «до 40%» (тиры зеркалят backend TIER_REWARDS, суммы в ₽, порог вывода 500₽), house-ads креатив с живым % из /public/referral (i18n {{pct}}), backend текст «$10 кредитов» → «10 ✦ кредитов».
- [З6] PWA: /api/* = NetworkOnly (кроме чата SWR), registerType autoUpdate + cleanupOutdatedCaches — обновления без ручного сброса (факт по sw.js/vite.config).
- [РЕГРЕССЫ ПОЙМАНЫ ГЕЙТАМИ] 1) TDZ в useOwnerData (addAuditLog в deps до объявления → «Cannot access before initialization» на /owner — ломало qa-view-as/qa-role-switch-flash) → addAuditLog поднят выше. 2) Композер чата 360px: ряд кнопок сжимал omega-input до w=0 (chat-pro-luxe З5) → flex-wrap + max-sm:min-w-[140px]. 3) qaSecurityFlow 404 на новых роутах — локальный backend был старый (не код); 4) 429 в view-as — локальный backend без OWNER_IP=127.0.0.1 (в CI есть; AGENTS.md дописан).
- [РЕГРЕССИЯ] qa-launch 24/24 ✅; real-data-audit (scripts/real-data-audit.mjs, 48 проверок): 10 экранов × 2 темы — 0 мок-паттернов, 0 pageerror + B1 hot-reload цены (1→2✦ без деплоя) + B2 below_cost 400 + B3 откат + B4 аналитика ✅; chat-pro-luxe-shots ✅; omega-video-upload-shots ✅ ALL GREEN; build 0 ошибок; i18n-parity OK.
- [НЕ ТРОНУТО] платежи/ЮKassa (создание/webhook/реальный refund через payments.js), PlanConfig/CreditPack цены, house-ads логика (добавлен только fetch % в существующий слот), _redirects, боты, гард-матрица (qaSecurityFlow зелёный + 2 новых негативных), CHAT-PRO, entitlement-снапшоты, OMEGA-VIDEO механика.
- [ЖДЁТ РЕШЕНИЯ ВЛАДЕЛЬЦА] legacy admin-возвраты (in-memory REFUNDS в refundService) — пометка «отмечен локально», деньги двигает owner.refund; полный перенос — approve-зона. Файлы-архивы *.bak/.backup (DashboardPage.jsx не роутится, OwnerDashboardPage.jsx.*) — удаление только с ✅.
- [GIT] ветка fix/real-data. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/real-data
...
- [П3 ЛИМИТ ИЗ КАБИНЕТА] `OwnerSettings.mediaUploadLimitMb` (1–250, дефолт 250 = жёсткий потолок multer), кэш ≤60с (паттерн owner-флагов). Owner GET/POST `/api/owner/media-limit` (в routes/owner.js ВЫШЕ generic `/:entity` — иначе Unknown entity, словил фактом). Клиент: GET `/api/upload/limits`; backend 413 `{error:'file_too_large', limitMb, fileMb}`. Превышение → тост «Лимит N МБ, файл M МБ» (RU/EN), не молчание/не краш. Пруф фактом: owner 42 → клиент видит 42 сразу; клиент POST → 403; кабинет — карточка «Лимит загрузки медиа» в Подписках (SubscriptionsTab).
- [П4 ПРОГРЕСС+ОТМЕНА] XHR (не fetch — нужен upload.onprogress): прогресс % на чипе, ✕ → xhr.abort() (чип исчезает, файл не зависает в ленте), обрыв сети → status error + кнопка «Повторить».
- [П5 РАЗБОР СРАЗУ] После загрузки — автостарт разбора: кадры на клиенте (video+canvas, ≤6 кадров, 640px jpeg, таймауты от зависшего seek) → `POST /api/omega/analyze-video-upload`: только свой `/uploads/<userId>/` (чужой URL → 403, traversal заблокирован — проверено), списание 1✦ через `consumeGeneration` (та же квота, что чат; owner=∞), vision по кадрам (visionCore/Replicate) + синтез «таймкоды/хук/удержание» через chatWithAI. smart-fallback = мок → 200 success:false ai_unavailable + `refundGeneration` (✦ возвращён, файл НЕ удалён — можно повторить). После успешного разбора файл авто-удаляется (unlink с нормализацией пути `replace(/\\/g,'/')` — Windows-баг пойман фактом). Цена «Анализ видео = 1✦ · осталось N✦» на чипе ДО анализа (фактическая стоимость по квоте; выдуманных цен нет). CreativeHub = внешний режим: добавлен `injectMessages` в useOmegaChat — иначе разбор не попадал в ленту (словлено гейтом).
- [ЧЕСТНОСТЬ] Локально все AI-провайдеры мёртвые (env-ключи невалидны, гард выводит их из ротации) → гейт проверил честный отказ: сообщение «AI-провайдеры недоступны — попробуйте позже. Генерация не списана.» в ленте вместо мока, квота не изменилась (generationsUsed до=после). Живой разбор (vision+синтез) на проде — ключи там живые (Mongo/кабинет).
- [ГЕЙТ] `scripts/omega-video-upload-shots.mjs` 27/27 зелёных: drop на 390(iPhone)/1280 × dark/light → чип+цена, оверлей «Отпустите файл», прогресс %, отмена (нет 📹 в ленте), файл >лимита → тост + чип не завис, hot-reload лимита 1→250 МБ без деплоя, карточка в кабинете, 0 console errors. Скрины reports/omega-video-upload/ (9 шт). Не входит в qa-launch — запускать отдельно при поднятых :18080/:4173.
- [РЕГРЕССИЯ] qa-launch 23/23 (qaCreditsFlow упал на 429 rate-limit в общем прогоне — после 10-мин отката перегнан отдельно: зелёный; причина — накопленные запросы прогонов, не код батча). chat-pro-luxe-shots: все проверки зелёные. `npm run build` 0 ошибок (бандл не растёт: route-lazy, +~6КБ в чанке чата). i18n-parity OK (интерполяция кастомного t() — только `{{var}}`, факт в AGENTS.md). curl прода / /chat /login /credits → 200, 0 редиректов. AGENTS.md дописан [OMEGA-VIDEO ДОП-З1].
...
- [З1+З3 ЯДРО] `frontend/src/components/chat-pro/LuxeHubChat.jsx` — оболочка над `OmegaChat` (embedded, variant=compact): люкс-шапка (✦ OMEGA Studio, «люкс-режим · онлайн/думает», пилюля баланса → QuotaDetailsModal с Esc/focus-trap), поиск по ленте (новый проп `searchQuery` в OmegaChat: фильтр + empty-state «Ничего не найдено»), черновик инпута (localStorage `omega_chat_draft`, переживает reload), сайдбар: `BalanceRingCard` (кольцо баланса на реальном /users/me/quota, count-up с reduced-motion, CTA «Пополнить» → /credits) + «В этом месяце» (генераций/тариф/цикл до — только реальные поля, стабов нет). Скелетон набора ответа в OmegaChat (role=status, animate-pulse, reduced-motion=статика). Стриминга в API нет (POST /omega/chat отдаёт целиком) — решил сам: скелетон вместо SSE, backend AI-провайдеров не трогаем (красная зона).
- [З2 ИНВЕНТАРЬ] Вся логика чата осталась в OmegaChat без изменений: голосовой ввод (RU/EN/ES/ZH), вложения (скрепка+drag&drop), TTS + настройки голоса, быстрые действия (ролевой фильтр), приветственные чипы, YouTube-карточка анализа, действия (обложка/драфт/best time), превью HTML по платформам, саппорт-тикет + Telegram, reasoning steps, фидбек 👍/👎, UpsellModal при 402, счётчик вариантов, цена «1 сообщение = 1✦» под композером, история в localStorage. Перенесены презентационно: пилюля квоты и статус «онлайн/думает» (из шапки OmegaChat → люкс-шапка), чипы-подсказки хаба (из шапки хаба → люкс-шапка, видны при свёрнутых инсайтах/≥2xl). Тулбар инструментов с ценой 1✦ и house-ads баннер ≤34px — на месте. Старый UI (OmegaChat fullscreen) не удалён: работает в режимах analyzer/viral.
- [З4 ДИЗАЙН-ГЕЙТ] `scripts/chat-pro-luxe-shots.mjs` 26/26 зелёных: 390(iPhone)/768/1280/1920 × dark/light × RU/EN, без горизонтального скролла, пилюля → модалка → Esc, поиск фильтрует + empty-state, черновик переживает reload, house-ads баннер h=32px (≤34 по спеке Р3). Фиксы по скринам: чипы-подсказки налезали на пилюлю при открытых инсайтах (теперь только свёрнутые/2xl), сайдбар кольца сужал ленту при открытых инсайтах (теперь xl при свёрнутых, иначе ≥1920), подзаголовок шапки — truncate. Скрины reports/chat-pro-luxe/.
- [З5 РЕГРЕССИЯ] `npm run build` 0 ошибок (3492 модуля); `i18n-parity` OK (ключи chatPro.* во всех 4 locale-файлах); `qa-launch` 23/23 зелёных (включая qaSecurityFlow, qa-journey-1..4, qa-bots); curl прода: / /chat /login /credits → 200, 0 редиректов. AGENTS.md дописан (факты Люкс-хаба).
- [НЕ ТРОНУТО] платежи/ЮKassa, PlanConfig/тарифы, витрина пакетов, `_redirects`, оба бота, ask-owner, гард-матрица, house-ads логика (спека Р3), entitlement-снапшоты аддонов, витрина цен.
- [GIT] ветка feat/chat-pro. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feat/chat-pro

## 2026-09-06 — URGENT-HOTFIX: прод лежал (308-петля /200), SPA-fallback возвращён на rewrite (ветка fix/urgent-spa-redirect, merge 2f2bc791, approve ✅ владельца TG qid 359cbf27cd65)
- [ПРОБЛЕМА] Прод aiviral-studio.ru в петле: `/` → 308 → `/200` → 308 → `/200` (ERR_TOO_MANY_REDIRECTS), pages.dev тоже. Корень: `/* /200.html 200` (2597e6b5, HOTFIX-FINAL 0.1) + каноникализация `.html` в Pages (rewrite-цель `/200.html` → 308 на `/200` → снова `/*` → петля). Факт «`/* /index.html 200` режется валидатором code 10021» из AGENTS.md ОПРОВЕРГНУТ: правило работало на проде до 06.09 (прямые заходы /privacy → 200) и сейчас работает на preview и проде.
- [З1 ОТКАТ] Два rollback в дашборде Pages владельцем (ask-owner qid 6caab5215479, 6485b5d77a8c) НЕ помогли — петля сидела в самих деплоях, собранных с 06.09 19:14. Выход — новый деплой с фиксом.
- [З2 ФИКС] `frontend/public/_redirects` → `/* /index.html 200` (rewrite, статус 200, без «!»); `frontend/scripts/spa-200.mjs` удалён, `package.json` build без postbuild; 200.html больше не генерируется.
- [ПРУФЫ PREVIEW до мержа] `https://fix-urgent-spa-redirect.ai-viral-studio.pages.dev`: `/` → 200 (0 редиректов), `/chat` → 200 (0), `/profile` → 200 (0).
- [ПРУФЫ ПРОД после деплоя] `https://aiviral-studio.ru/` → 200 (0 редиректов); `/login` `/chat` `/profile` `/credits` → 200 (0); `http://` → 301 → https → 200 одной ступенью (поведение Always Use HTTPS корректно, действий в дашборде не нужно). Playwright iPhone/Safari UA: главная 200, 0 page errors, скрин `reports/urgent-spa-redirect/prod-home-iphone-safari.png` (отправлен владельцу в TG).
- [AGENTS.md] Вписано навсегда: SPA-fallback — только rewrite на `/index.html`; rewrite на `*.html`-цель запрещён (петля через каноникализацию .html); любое изменение `_redirects`/redirect-логики — только с curl-пруфами на preview ДО мержа. Устаревший факт про code 10021 удалён.
- [НЕ ТРОНУТО] всё, кроме 3 файлов фикса: платежи/ЮKassa, PlanConfig, боты, гард-матрица, лендинг-код (0 строк фронта/бэка).
- [GIT] ветка fix/urgent-spa-redirect, merge-коммит 2f2bc791 в main (3 файла, +2/−9). Compare: https://github.com/yuri0941/ai-viral-studio/compare/a6ada4aa...2f2bc791


- [З1/З2 ВИТРИНА ПАКЕТОВ, backend] Новая модель `backend/models/CreditPack.js` (дефолты 100кр=99₽ / 500кр=449₽ / 1000кр=849₽ / 5000кр=3990₽, сид при первом GET) + роуты `backend/routes/credits.js`: GET /api/credits/packs (публично), POST /api/credits/purchase (protect; цена/размер ТОЛЬКО с сервера из CreditPack — клиент не влияет; Payment + ЮKassa с metadata packId/credits), GET /api/credits/packs-admin + PUT /api/credits/packs/:packId (owner-only, маржинальный пол: below_cost → 400, <70% → 409 без allowLowMargin, всё в PriceChangeLog + ownerActionsService).
- [З1 WEBHOOK] `paymentController.yookassaWebhookHandler`: payment.succeeded по пакету (metadata.packId/credits) → начисление фактом через `creditGenerations(userId, credits)` (usageQuotaService; лимит цикла += credits), идемпотентно (повторный webhook не начисляет: Payment.status уже succeeded). Тарифная ветка не тронута. Добавлено событие refund.succeeded: верификация через API ЮKassa (mark_refunded, object.payment_id), Payment → refunded, алерт владельцу; кредиты автоматически НЕ списываются (могли быть потрачены) — ручная проверка. Подделка по-прежнему отклоняется (verifyWebhookNotification, qa: ignored).
- [ЭКОНОМИКА] `backend/services/marginService.js`: себестоимость 0.24₽/кр, комиссия ЮKassa 3.5%, налог 6% (верх 4–6, консервативно), пол 70%. Дефолты: pack_100 73.2% ✅, pack_500 70.5% ✅, pack_1000 68.8% ⚠️, pack_5000 67.5% ⚠️ — два пакета ниже пола при текущих ценах; минимум для 70%: 1000кр ≥ 884₽, 5000кр ≥ 4420₽. Цены НЕ менял (approve-зона) — ждёт решения владельца.
- [ПРОВЕРКА] node --check ✅; qaCreditsFlow 11/11 ✅ (витрина, guard 401/403/404, пол 400/409/allowLowMargin+откат, поддельный webhook → ignored без начисления, creditGenerations +100 фактом); qaSecurityFlow вся матрица ✅; qa-launch 22/22 ✅.
- [E2E С ДЕНЬГАМИ] Ключи прода LIVE (`live_...O730`) — тест-карта 5555…4477 на live-магазине не работает. Решение владельца (TG, qid 61a6321c7c49): «Сам оплачу 99₽ картой после деплоя» — после мержа+деплоя: владелец покупает pack_100 на проде, я подтверждаю webhook → Payment.succeeded → +100✦ в БД/UI и делаю refund (проверка refund.succeeded).
- [НЕ ТРОНУТО] создание платежа подписок/аддонов, верификация webhook (только расширена ветками), PlanConfig/тарифы, refund-роут подписок, боты, гард-матрица (добавлены только проверки).
- [GIT] ветка fix/hotfix-final-payments от origin/main. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/hotfix-final-payments

## 2026-09-05 — BOTS-FIX + TG-OWNER-CONTEXT: фиксы ботов, изоляция owner-контура (ветка fix/bots-design-lab, стекнута на feat/tg-owner-context)
- [З1 КЛИЕНТ — ВЫХОД ИЗ «ТИКЕТНОГО ПЛЕНА»] Было: любой текст при открытом тикете молча уходил в обращение («Сообщение добавлено…»), AI молчал, выхода не было. Стало (backend/services/omegaBot.js): сообщения дописываются в тикет ТОЛЬКО в ветке поддержки (кнопка «Поддержка», «👤 Человек», deep-link support_); снаружи — живой AI-ответ. Выход: кнопка «❌ Закрыть обращение» (support:close + CSAT), /start, авто-закрытие 24ч без активности (lazy-проверка) и авто-закрытие после ответа оператора (supportService.replyToTicket, takeover не тронут). Закрытие/возврат тикета владельцем из TG снимает клиента с ветки.
- [З2 ПУШ ВЛАДЕЛЬЦУ О НОВОМ ТИКЕТЕ] Цепочка createTicket → alertOwner(type:'ticket', без cooldown) с кнопками [💬 Взять диалог][✅ Закрыть][⬆️ Эскалация] была на месте; обрыв глушился молча: добавлены логи «ticket alert NOT sent» (bot/ownerChatId) и «alert send failed». Коренная причина «нет пушей» на проде = «тикетный плен» (все обращения схлопывались в один открытый тикет → findOpenTicket early-return без пуша) — снята З1; 11 тестовых тикетов закрываются скриптом (З4).
- [З3 + ДОП.1 КРИТИЧНО — ИЗОЛЯЦИЯ OWNER-КОНТУРА] Инцидент 2026-09-05: «Не работают кнопки» от владельца ушло в очередь задач Омеги → автопост в канал @aiviralstudio (пост 147) без команды. Стало: НИЧТО из чата владельца не уходит в очередь напрямую — fallback свободного текста в ownerBot заменён на превью-гейт (backend/services/ownerTaskPreview.js): «📋 Поставить задачу Омеге? ✅/❌», submitOwnerCommand вызывается только по кнопке ✅ (otask:run, TTL 15 мин). Матрица: «привет/статус» → ownerFreeText-сводка; вопросы и ЖАЛОБЫ (не работает/баг/кнопки/ошибка — QUESTION_RE расширен) → ownerFreeText по фактам; «сделай/найди» → превью. Тест backend/tests/owner-routing-unit.mjs 20/20: жалоба → НЕ в очередь; превью не вызывает очередь без кнопки; статически запрещён прямой вызов submitOwnerCommand в ownerBot.
- [ДОП.2 ASK-OWNER КНОПКИ НА ПРОДЕ] Два обрыва: (а) часы машины кодера отставали ~28ч → expiresAt рождался просроченным (фикс: время из Date api.telegram.org + в прод-режиме сроки считает СЕРВЕР); (б) скрипт писал вопрос в локальную Mongo (db ai_viral_studio: 2 тикета, нет owner chat id), а прод-обработчик читал ДРУГУЮ БД → «Вопрос не найден». Стало: новый роут backend/routes/askOwnerApi.js (POST/GET /api/ask-owner/questions, auth x-ask-owner-key = sha256 от owner-токена, timingSafeEqual) — вопрос живёт только в прод-БД; scripts/ask-owner.mjs ходит в прод-API, legacy-Mongo — fallback. Тексты «не найден/просрочен» → понятные + «кодер задаст заново». Живой тест на проде — после мержа+деплоя (эндпоинт появится там).
- [ДОП.3 CHANNEL-AUTO НА ПАУЗЕ] startChannelAutonomy выходит с логом, пока CHANNEL_AUTOPOST_ENABLED !== '1' (код не удалён; включит владелец после контент-плана). Явные /autoposttest, /post не тронуты.
- [З4 ХВОСТЫ] Not-owner меню ownerBot: голый sendMessage → sendNotOwnerMenu с catch (тест: сетевой отказ не роняет процесс). Закрытие 11 тестовых тикетов: scripts/close-test-tickets.mjs через /api/ask-owner/tickets/open+close (маркеры QA/тест/test, реальные не трогает, --dry) — прогон после мержа+деплоя. Selftest-отчёты: нажатие ✅/❌ на сообщении qa-bots без «Ветка:» → «🧪 Это selftest — мерж не нужен» вместо «Не удалось распарсить ветку».
- [ПРОВЕРКА] node --check всех изменённых ✅; owner-routing-unit 20/20 ✅; tg-freetext-unit PASS ✅; qa-launch 21/21 ✅ (qaSecurityFlow, qa-bots, qaSupportFlow — зелёные); i18n-parity чист ✅. Живые диалоговые пруфы (клиент/owner боты на проде) — после мержа+деплоя: локально боты webhook'и не трогают (гард).
- [НЕ ТРОНУТО] платежи/ЮKassa, PlanConfig/тарифы, webhook-гард, гард-матрица (только добавлены ветки), рефералка, аддоны/entitlement-снапшоты, ownerFreeText снапшот данных (расширен только QUESTION_RE), владельческие рубильники.
- [GIT] ветка fix/bots-design-lab (стекнута на feat/tg-owner-context — ownerFreeText нужен З3, в main его ещё нет). Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/bots-design-lab
...
- [З2 — РЕДАКТОР] AddonMarketplace (owner-only): состав = чекбоксы из GET /subscriptions/addons/entitlements-catalog (не свободный текст); «скоро»-функции disabled; отображаемый текст правится отдельным полем includes; подписи витрины генерируются из выбранных ключей, если includes пуст
- [З3 — РАЗБЛОКИРОВКА] middleware/entitlement.js: hasEntitlement(userId, key) по активной записи UserAddon (живой ∪ снапшот, owner bypass); GET /subscriptions/my-entitlements; GET /subscriptions/entitlements/:key/check → 402 без покупки. e2e: клиент без покупки → 402 ×2; owner собрал 2 функции → купивший клиент 200 ×2; owner снял функцию → у купившего жива (снапшот), витрина новым — только актуальная; «скоро» → 402 даже с аддоном
- [ПРОВЕРКИ] qaAddonsFlow 29/29; qaSecurityFlow зелёный; node --check OK; build 0 ошибок; i18n-parity OK (+addons.featuresLabel/soonBadge)
- [БАГ ПО ХОДУ] i18n не деструктурирован из useTranslation в AddonMarketplace → краш редактора (пойман скрин-прогоном, исправлен)
- [СНИМКИ] reports/addons-composition-link/: 01 редактор с чекбоксами функций (1280 RU), 02 пункт «Управление аддонами» у owner, 03 сайдбар admin без пункта, 04 витрина клиента
- [GIT] Ветка fix/addons-composition-link; compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/addons-composition-link
## 2026-08-30 — ADDONS-MARKETPLACE-RESTORE: доработка управления аддонами (ветка fix/onboarding-empty-state)
- [ДИАГНОСТИКА] Маркетплейс НЕ был выпилен: витрина (AddonMarketplace) жива в SettingsPage→tab=addons, API /api/subscriptions/addons работает, покупка ЮKassa — после fad2f77a/e39627a0 (Б3). Не хватало управления у owner: таб 'addons' в сайдбаре owner не имел case в OwnerDashboardPage → показывался Overview; редактирование было моком цены для owner+admin без AuditLog
- [П1 — РЕДАКТОР OWNER] PATCH /api/subscriptions/addons/:id/price расширен: price/currency/discountPercent/paymentMethods/name/description/includes(состав)/isActive — сохранение сразу в БД → сразу на витрине. Каждое изменение пишется в AuditLog (action owner.addon_update, diff до/после). Модель Addon +поле includes[]. AddonMarketplace: в edit-режиме поля название/описание/состав/вкл-выкл/цена; owner видит и выключенные аддоны (pricing-config без фильтра isActive)
- [П2 — ПУНКТ ПАНЕЛИ] «🛠 Управление аддонами» (sidebar.addonsManage) в OWNER_GROUPS сайдбара → /owner?tab=addonsManage → тот же компонент AddonMarketplace (без legacy-дублей). Пункт виден только owner (группы сайдбара рендерятся по isOwner)
- [П3 — ГАРД] Все owner-эндпоинты аддонов переведены на строгий authorize('owner'): PATCH price, GET pricing-config, POST reset-price, POST analyze-price, GET pricing-report — admin и прочие роли → 403. Кнопка «Редактировать цены» рендерится только при user.role==='owner'
- [П4 — НЕГАТИВНЫЕ ТЕСТЫ] qaAddonsFlow расширен до 14/14: клиент PATCH → 403; admin PATCH → 403; admin GET pricing-config → 403; owner PATCH → 200 (поля сохранены); AuditLog записан; выкл аддона → пропал с витрины, купленный у пользователя аддон НЕ сломался (status active); вкл → снова на витрине, цена откачена. qaSecurityFlow — вся матрица зелёная
- [ПРОВЕРКА] node --check OK; npm run build 0 ошибок (3487 модулей); i18n-parity OK (добавлены addons.name/description/includesPlaceholder/enabledLabel/disabledBadge, sidebar.addonsManage, common.saved — RU/EN src+public)
- [НЕ ТРОНУТО] платежи/ЮKassa (флоу покупки как в Б3), PlanConfig и тарифы, боты, лендинг, модалка обновлений, рубильники, гард-матрица, view-as
- [СНИМКИ] reports/addons-marketplace-restore/: 01 редактор у owner (1280 RU), 02 пункт «Управление аддонами» в сайдбаре owner, 03 сайдбар admin без пункта, 04 витрина у клиента без кнопки редактирования
- [GIT] Ветка fix/onboarding-empty-state; compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/onboarding-empty-state
## 2026-08-30 — ONBOARDING-EMPTY-STATE (ветка fix/onboarding-empty-state)
- [ЗАДАЧА 1 — EMPTY-STATE НЕ БЛОКИРУЕТ] CreatorDashboardPage: убран гейт `(statsLoading || posts > 0)`, скрывавший ВЕСЬ дашборд при 0 постах — из-за него заглушка «Постов пока нет» была единственным контентом и выглядела обязательным шагом. Теперь empty-state — inline-карточка сверху (без backdrop/fixed), под ней весь дашборд (hero, статистика, графики, quick actions), меню и нижняя мобильная навигация свободны. Заодно: guard деления на 0 в шкале монетизации (`stats.income = 0` → NaN% width) и сырой плейсхолдер `{{time}}` в «Совет OMEGA» (t с params)
- [ЗАДАЧА 2 — ТУР 1 РАЗ И СО СКИПОМ] OnboardingTour: кнопка «Пропустить» (tour.skip RU/EN) на КАЖДОМ шаге через onPopoverRender driver.js; destroy (скип/готово/Esc) пишет флаг и в localStorage (`omega_onboarding_tour_done`), и на сервер — PATCH /users/me/onboarding { tourDone: true }. При старте проверяется `user.preferences.onboarding.tourDone` с /auth/me — тур не всплывёт на другом устройстве/после смены браузера. КОРНЕВЫЕ БАГИ: ① preferences в models/User.js — strict-схема без блока onboarding → mongoose молча отбрасывал поле при save() (прогресс визарда тоже никогда не сохранялся на сервере) — блок добавлен; ② таймер автостарта гасился ререндерами (t — новая ссылка каждый рендер → cleanup снимал setTimeout) — флаг запуска ставится в момент срабатывания таймера
- [ЗАДАЧА 3 — ПРОВЕРКА] e2e scripts/qa-onboarding-tour.mjs (preview :4173 + локальный backend :18080, регистрация через API): новый клиент → тур показан → «Пропустить» видна с 1-го шага → после скипа кабинет доступен, empty-state inline, переходы по меню работают → reload без тура → «другое устройство» (чистый localStorage, флаг с сервера) без тура — 10/10 PASS. ui-audit пустого дашборда × 360/428/768/1280/1920 × RU/EN (UI_AUDIT_MOCK_AUTH=1): 10 снимков, 0 скроллов, 0 налезаний, навигация не перекрыта. node --check OK; npm run build 0 ошибок (3487 модулей); i18n-parity OK
- [НЕ ТРОНУТО] платежи/ЮKassa, PlanConfig, боты, лендинг, модалка обновлений, рубильники, гард-матрица, view-as
- [СНИМКИ] reports/onboarding-empty-state/ (tour-step1-with-skip, dashboard-empty-1280-ru, tour-second-device-none, ui-audit 360–1920 × RU/EN)
- [GIT] Ветка fix/onboarding-empty-state; compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/onboarding-empty-state

- [ЗАДАЧА 5г — АДДОНЫ] КОРНЕВОЙ БАГ (находка Б3): middleware/auth.js protect клал в req.user только `id`, без `_id` — все запросы вида `{ userId: req.user._id }` превращались в `{ userId: undefined }` (mongoose отбрасывает ключ) и матчили ЧУЖИЕ записи: чужой активный аддон-«сирота» блокировал покупку аддона всем клиентам (400 «Аддон уже подключен»), а upsert покупки создавал записи-сироты без userId. Фикс: `_id: user._id` в req.user. qaAddonsFlow 7/7: клиент manual → 403; yookassa без ключей → честные 400; owner manual → 200; webhook payment.succeeded → active ~30 дней, повтор идемпотентен. Оплата аддонов — реальная ЮKassa (pending → active по webhook), paymentId 'manual' у клиентов нет
- [ЗАДАЧА 6 — АУДИТ] ui-audit по клиентскому пути (лендинг/логин/регистрация/юрстраницы + весь creator-кабинет) × 360/428/768/1280/1920 × RU/EN (UI_AUDIT_MOCK_AUTH=1): 205 снимков, найдены 2 краша → исправлены; ресъёмка 20/20 чисто. Итог: 0 скроллов, 0 сырых ключей, 0 console errors. ui-buttons (public+creator, 150 страниц): 0 EMPTY, 0 RAWKEY (мёртвых/сырых кнопок нет); COVERED/OFFSCREEN = 183/14 — все совпадают с baseline (sticky-хедер/нижняя навигация), 0 новых флагов
- [БАГИ АУДИТА] /challenge и /leaderboard падали в ErrorBoundary («n.map is not a function») при не-массиве data — защита Array.isArray; SchedulerPage грузил /youtube/videos+playlists без подключённого канала → шумные 400 youtube_not_connected на каждом визите — теперь списки только при connected (journey-3 после фикса 8/8)
- [ПОЛНОТА Б3] ЗАДАЧА 1 (белый список ролей + AuditLog security.role_change) — закрыта ранее (17be3489); ЗАДАЧА 2 — путь клиента e2e journey-1/2/4 зелёные, journey-3 8/8; ЗАДАЧА 4 — панель готовности интеграций (IntegrationsTab: статусы configured/not_configured + тест-кнопки, скрин 08-owner-integrations-readiness.png); ЗАДАЧА 5 — реф-цепочка e2e 11/11 (78424c63), вывод реферальных реальной заявкой в поддержку с порогом $5≈500₽ (130f6afc, был setTimeout-фейк с $50), политика возврата 14 дней в карточке тарифа RU/EN (39201897)
- [РАСХОЖДЕНИЕ — НЕ ЧИНИТЬ, решение владельца] Реф-экономика: referralService markReferralPaid начисляет фиксированные +$4 за оплату реферала, а тир partner рекламирует «12% комиссии» — рассинхрон значения и механики
- [CHECKS] node --check/esbuild затронутых OK; npm run build 0 ошибок; i18n-parity OK; mojibake чисто; BOM нет
- [GIT] Ветка fix/client-journey-qa; compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/client-journey-qa; diff --stat: 37 файлов, +1002/−88 (против 52544a37)
...
- [ЗАДАЧА 3 — LUXE-CARD] YouTubeAnalysisCard: бейдж «✅ Реальные данные» (statsAvailable), градиентный SVG-донат (violet→fuchsia→cyan) с прозрачной формулой под ним, 5 баров с ЧЕСТНЫМИ цветами (<40 красный, <70 янтарный, иначе изумруд), «💡 Советы» — с бэкенда или из 2 самых слабых метрик (ytCard.tip.*), кнопка «🔖 SEO-теги» — реальные snippet.tags с бэкенда (omegaController теперь шлёт tags в videoAnalysis). Кнопки: обложка (реальный generateCover → картинка), пост (драфт в Планировщик), когда постить (bestTimeService), PDF, копировать ссылку. КОРНЕВОЙ БАГ «карточки нет»: useOmegaChat терял videoAnalysis/action при маппинге ответа — поля добавлены (обе ветки, incl. retry)
- [ЗАДАЧА 4 — MARKDOWN] Новый components/omega/MarkdownText.jsx (без зависимостей): заголовки, **bold**, *italic*, `code`, списки, таблицы |---|, ссылки, <br>; встроен в AiMessageContent и тела LuxuryMessageCard — сырые **/|---|/<br> не видны
- [ЗАДАЧА 5 — ПАРИТЕТ] Owner-вкладки aiChat/contentAnalyzer/contentAnalysis/viralChat в OwnerDashboardPage → Navigate на единый Creative Hub (старые ?tab=-ссылки не ломаются); legacy-страницы AIChatPage.jsx (372), ViralChatPage.jsx (451), ContentAnalyzerPage.jsx (929) УДАЛЕНЫ (−1752 строки); owner/staff видят те же компоненты, что и клиент. Таблица паритета — в отчёте батча
- [ЗАДАЧА 6 — АДАПТИВ] ui-audit creator+owner × 360/428/768/1280/1920 × RU/EN по hub-роутам и дашбордам: 110 снимков, 0 проблем, 0 неожиданных редиректов; после ДОП — финальный прогон 100 снимков, 0 проблем. ui-buttons: флаги COVERED/OFFSCREEN совпадают с baseline main (reports/ui-audit-reshoot/buttons) — sticky-форма/скролл-стрип/collapsed-сайдбар, регрессий нет
- [ДОП-1] «Creative Hub» → «Ω OMEGA»: значения во всех 4 локалях, заголовок Hub через hub.title, CommandPalette/OwnerDashboard QuickAction — «OMEGA»
- [ДОП-2а] OnboardingTour переписан: i18n (tour.* RU/EN), шаги — welcome про единый чат + «Режимы внутри OMEGA» (data-tour="hub-modes" на десктопной и мобильной карточке режимов, выбирается видимая), мёртвый шаг support-widget удалён; визард мёртвых ссылок не содержал (/dashboard, /scheduler валидны)
- [ДОП-2б] Пустой чат не пустой: welcome-экран «Что умею» + 4 кликабельных чипа (анализ видео/хук/обложка/когда постить), для owner/admin +код/сайт; клик → handleSendMessage(forcedText) → запрос уходит сразу; i18n chat.welcome.*
- [ДОП-2в] Путь новичка проверен e2e (.tmp-ui-polish/newbie-flow.mjs, mock-auth): /onboarding 200 → тур стартует (welcome=true, tour=true) → чип «Придумай вирусный хук» → ответ OMEGA markdown-ом, 0 page errors × 360/1280 × RU/EN. Оговорка: регистрация на проде закрыта флагом владельца (registration_closed), путь проверен с шага /onboarding
- [ИНСТРУМЕНТЫ] ui-audit.mjs/ui-buttons.mjs: UI_AUDIT_MOCK_AUTH=1 — стаб auth/me+users/me+пустой success для /api/* (прод-токены протухли, регистрация закрыта; без флага поведение прежнее). Причина «лендинга вместо кабинета»: api.js при любом 401 делает жёсткий location.href='/login'
- [i18n] hub.* (32), tour.* (15), chat.welcome.* (6), ytCard.* (+9) во все 4 файла; i18n-parity OK, mojibake чисто, UTF-8 без BOM
- [CHECKS] esbuild/node --check всех затронутых файлов OK; npm run build — 0 ошибок
- [СНИМКИ] reports/chat-unify/ (аудит 110+100), card/ (ссылка→карточка creator+owner × 360/1280 × RU/EN), dop/ (меню Ω OMEGA RU/EN, welcome+тур, чип→ответ)
- [GIT] Ветка fix/chat-unify-luxe; compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/chat-unify-luxe
- [NEXT] Владельцу: проверить на проде — меню с одним входом «Ω OMEGA», ссылка на YouTube в чате → люкс-карточка, под клиентом нет кнопок кода/сайта; для полноценного e2e с реальным бэкендом — перевыпустить тестовые токены (.tmp-ui-polish/token-*.json протухли)

## 2026-08-24 — YT-DATA-REAL-STATS + ACTIONS + LUXE-CARD
- [ШАГ 0 — ТОЧКА ОТКАЗА ВАЛИДАЦИИ] backend/routes/apiKeys.js case 'youtube': ключ YouTube Data API (AIza...) проверялся запросом `channels?part=snippet&mine=true&access_token=${key}` — т.е. API-ключ подставлялся как OAuth access_token с параметром mine=true. Google на такой запрос отвечает 401 → валидный ключ всегда отклонялся. Фикс: дешёвый вызов `videos.list?part=id&id=dQw4w9WgXcQ&maxResults=1&key=...` (1 ед. квоты) + корректный разбор ответа
- [МОДАЛКА] Человеческие причины от Google в тост: accessNotConfigured → «включите YouTube Data API v3» (+ссылка на console.cloud.google.com), keyInvalid → «ключ недействителен», quotaExceeded/dailyLimitExceeded → «квота исчерпана, сброс после 10:00 МСК», ipRefererBlocked/forbidden/403 → «снимите ограничения ключа (Application restrictions → None)»
- [BACKEND] Новый services/youtubeDataService.js — единая точка YouTube Data API v3: ключ из кабинета через getProviderKey('youtube') (ApiKey в MongoDB), кэш квоты в Redis/in-memory (видео 1 ч, канал 6 ч, поиск 6 ч — search.list = 100 ед!), graceful fallback при quotaExceeded + лог, человеческие причины ошибок, прозрачная формула AI-рейтинга (engagement*0.3 + virality*0.25 + retention*0.15 + seo*0.15 + growth*0.15, все метрики реальные)
- [BACKEND] omegaController.analyzeVideo: YouTube-ссылка → реальный fetch videos.list(snippet+statistics) + channels.list; AI получает реальные цифры в контексте с запретом выдумывать; stats:null + statsError при недоступности — честный качественный разбор без метрик
- [BACKEND] omegaController.chat: детект YouTube-ссылки в сообщении → реальный fetch (кэш), данные в extraSystemContext + структурированный videoAnalysis в ответе для люкс-карточки; текстовые команды «сделай обложку» (generateCover → Pollinations), «создай пост» (драфт в ScheduledPost), «когда постить» (bestTimeService) — реальные вызовы сервисов, при невозможности — честное сообщение в контексте AI
- [BACKEND] youtubeService.js: убран захардкоженный API-ключ, search/stats/trending идут через youtubeDataService (ключ из БД + кэш); youtubeAI.analyzeChannel: удалены демо-цифры (12500 подписчиков и т.п.) — честный отказ с needsKey
- [FRONTEND] Новый components/omega/YouTubeAnalysisCard.jsx — LUXE-карточка: превью из snippet.thumbnails (лучшее качество), карточки Просмотры/Лайки/Комментарии/Подписчики с иконками и count-up анимацией, формат 36 857 (toLocaleString), донат «AI-рейтинг» + бары (Виральность/Вовлечённость/Удержание/SEO/Потенциал роста), «Советы для улучшения», действия (обложка → картинка + «Использовать в посте», драфт в Планировщик, best time), «Экспорт PDF» (печать браузера), «Копировать ссылку»; компактный вариант в OMEGA Chat (OmegaChat.jsx), полный — на экране «Анализ контента» (ContentAnalyzerPage.jsx, реальные цифры вместо нулей)
- [i18n] Ключи ytCard.* во все 4 файла локалей (ru/en × src/public); i18n-parity OK, mojibake чисто
- [CHECKS] node --check затронутых backend-файлов OK; npm run build — 0 ошибок
- [E2E] ui-audit (creator:/creative-hub/chat, creator:/creative-hub/analyzer × 5 ширин × RU/EN): 0 налезаний, 0 сырых ключей, 0 console errors; часть снимков на прод-API (Render cold start) — спиннер/редирект, пересъём после прогрева
- [GIT] Ветка fix/yt-data-real-stats; compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/yt-data-real-stats
- [NEXT] Владельцу после мержа: Google Cloud → Credentials → ключ → API restrictions → только YouTube Data API v3 → Regenerate (старый засвечен) → вставить свежий в Кабинет → API Ключи → YouTube Data API. Чек-лист: ключ принят → «Подключено»; ссылка на shorts в OMEGA → карточка с превью + реальные цифры; «сделай обложку» → реальная картинка; «создай пост» → драфт в Планировщике; удалить ключ → честное «статистика недоступна». Глубокая аналитика своего канала (YouTube Analytics API) — отдельным батчем после Google-аудита
...
- [ДИАГНОЗ 3] SignupPage «← Назад» был disabled на шаге 1 — уйти на лендинг было нельзя. Фикс: на шаге 1 ведёт на `/`
- [ДИАГНОЗ 4] «Войти»: переход / → /login обновляет пропс authMode БЕЗ ремонта (одинаковый тип компонента в обоих роутах) — useState не перечитывался, модалка не открывалась. Фикс: useEffect по authMode; плюс контраст «Войти» (glass-кнопка вместо бледного текста). Дополнительно добавлен frontend/public/_redirects (SPA-fallback `/* /index.html 200`) — прямые заходы на /login /register /signup на Cloudflare Pages больше не 404
- [ВОССТАНОВЛЕНО из legacy] metrics/visit beacon (аналитика владельца, 1/сутки); правовая строка владельца в футере (ownerLegalInfoApi.public() — в legacy запрос был, но данные не рендерились; теперь рендерится: ФИО · тип · ИНН, auto-update из кабинета); PWAInstallButton в шапке; ссылки Roadmap и Скачать (футер); Consent в футере; соцсети VK/YouTube/X в футере
- [ОСТАВЛЕНО УДАЛЁННЫМ] протухшие цены config/plans.js, тумблер «Год −20%», прямая оплата с лендинга (CHECKOUT-UNIFY), confetti без keyframes, Discord-заглушка your_invite, рекламная CTA с нерабочим ClientChatWidget, OmegaCompetitorRadar (статика RU-хардкод + тяжёлый recharts в главном чанке), мёртвый fetch legalInfo без рендера (теперь рендерится)
- [CHECKS] i18n-parity OK (1875 ключей), mojibake чисто, npm run build 0 ошибок, dist/_redirects на месте
- [E2E] scripts/landing-restore-e2e.mjs на собранном бандле: 10/10 PASS (тарифы при мёртвом API, валидные t.me href без VITE_*, «Войти»→модалка, «Назад»→/, beacon/roadmap/download/legal на месте). Скрины: reports/landing-restore/
- [GIT] Commit+push: fix/landing-restore-hotfix, compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/landing-restore-hotfix
- [NEXT] Владельцу: после выкатки проверить — тарифы видны всегда (даже при спящем API), футер ведёт в @aiviral_alerts_bot и @aiviralstudio, «Войти» открывает модалку, «Назад» на регистрации возвращает на главную; в Cloudflare Pages задать VITE_CLIENT_BOT_USERNAME / VITE_OWNER_BOT_USERNAME / VITE_TELEGRAM_CHANNEL_USERNAME
...
## 2026-08-24 — PLANCONFIG-ADMIN — админ-редактор тарифов, founding, AI-советчик, цены из PlanConfig везде (ветка feature/planconfig-admin)
- [БЛОК A] `8e3ab489` Legacy `backend/config/plans.js` удалён вместе с роутами `/api/plans` и `/api/checkout` (legacy in-memory цены Creator 2900/Pro 7900). Новый `backend/services/planConfigCache.js` (кэш PlanConfig, invalidate); `utils/canUse.js`, `usageQuotaService`, `subscriptionController`, `rateLimiter` переведены на PlanConfig cache. `server.js` больше не монтирует legacy-роуты.
- [ФИКС email] `dfccbcb2` `emailService.js`: число бесплатных генераций берётся из PlanConfig (не хардкод), исправлен захардкоженный URL.
- [БЛОК B] `6b694b54` API редактора тарифов в `routes/planConfig.js` (+178 строк: price/featureList, история, откат). Новая модель `FoundingConfig` (+35); `PlanConfig` расширен (featureList и пр.); `Subscription` +2 поля (planSnapshot). Динамические цены в `emailService`, `omegaBot`, `foundingService`, `salesAutopilot`; `yookassaController` считает founder-цену от PlanConfig.
- [БЛОК C] `cbdff571` AI-советник тарифов: новый `services/planAdvisorService.js` (+110), API в кабинете владельца (`routes/owner.js` +12) и команда в owner-боте (`ownerBot.js` +30). Фикс `pricingAnalysis.js` (planId), инвалидация кэша при price:apply.
- [БЛОК D] `53d7d8ac` UI редактора тарифов в `PricingTab.jsx` (+245: цена, featureList, founding, история, откат, предпросмотр, AI-советчик). Лендинг и Settings показывают founding динамически; `omegaSkills.js` — живые цены; frontend `config/plans.js` удалён (−55); ключи во все 4 локали (+24); `SubscriptionsTab` причёсан.
- [БЛОК E] `69ce11c1` Stripe-дыра закрыта: `stripeService` берёт цены из PlanConfig (было in-memory creator 2900/pro 7900). ЮKassa добавлена в `/subscriptions/config` paymentMethods. Возврат квоты при сбое AI-генерации (`usageQuotaService` +29, `omegaController`). Фикс ключа в `enforceQuota`. Карточки «хватит на» в Settings; пустые отзывы на лендинге скрываются; FinanceTab доработан. E2E-скрипт `backend/scripts/planconfig-e2e.mjs` (+131): 16/16 ✅ (по сообщению коммита).
- [ИТОГО по ветке] 6 коммитов, 41 файл: +1232/−391. Закрыты наблюдения из CHECKOUT-UNIFY: Stripe in-memory цены ✅, /api/plans + config/plans.js удалены ✅, ЮKassa в /subscriptions/config ✅.
- [TEST] E2E planconfig 16/16 ✅ (из сообщения коммита); npm run build — см. запись о финализации ниже.
- [ВНЕ КОММИТА] В рабочем дереве остались untracked артефакты, НЕ включённые в ветку: `agent-reports/`, `tools/omega-agent/` (содержит `.env`!), `MASTER_PLAN_ULTIMATE_COMPLETE (2).md` (в тексте — рабочие API-ключи YouTube/Groq/OpenRouter в открытом виде), мусорный файл `} бесплатных генераций")`. Внимание владельцу: ключи скомпрометированы в файле на диске — рекомендуется ротация и удаление.
- [NOTE] Ручная проверка владельцем после деплоя: 1) Owner → тарифы: смена цены Pro → лендинг/кабинет/боты показывают новую цену без деплоя, история + откат работают; 2) founding: смена слотов/скидки отражается на лендинге и в ЮKassa-сумме; 3) Stripe-оплата заряжает цену из PlanConfig; 4) сбой AI-генерации возвращает квоту; 5) «хватит на N» в Settings совпадает с квотами; 6) при пустых отзывах блок отзывов на лендинге скрыт.

## 2026-08-27 — LEGAL-PAGES — /privacy и /terms на i18n-ключах legal.*, актуальные провайдеры, дата 27.08.2026 (ветка fix/legal-pages)
- [ВОЗОБНОВЛЕНИЕ] Продолжение прошлой сессии: /consent (домен aiviral-studio.ru, провайдеры MongoDB Atlas/Cloudflare/Render/YooKassa/Groq/OpenRouter, ссылки на /privacy и /terms) и RU-лейблы футера были сделаны ранее (e8a8132e, b9243559) — не переделывались. Ссылки /privacy /terms /consent в футере лендинга уже были (LandingPage L541-543).
- [ТЕКСТЫ] Источник текстов — файл владельца «Юридические_документы_для_Google_верификации.md» (прислан полностью). Провайдеры ТОЛЬКО актуальные: MongoDB Atlas (США, БД), Groq (США, AI), OpenRouter (США, AI-резерв), Cloudflare (США/ЕС, CDN/хостинг), Render (США, backend), ЮKassa (РФ, платежи). GitHub Models / HuggingFace / Nodemailer удалены из таблицы и текста трансграничной передачи (b799fa8c). Дата вступления в силу — 27 августа 2026 г. / August 27, 2026 (было «30 июля 2026»). В privacy добавлен раздел 6 «Использование YouTube API Services» (OAuth, отзыв доступа, ссылка на Google Privacy Policy) — требование Google-верификации.
- [ЛОКАЛИ] Новый объект legal.* (~114 ключей: privacy 11 разделов, terms 9 разделов, общие back/effectiveDate/operatorLabel/operatorLine/operatorPending/siteLabel) во все 4 файла локалей (src+public × ru+en), вставка после якоря pdfFooter (1a56a720, 41df1270). {{EMAIL}} зашит: tihonovu560@gmail.com. {{OPERATOR}}/{{INN}} НЕ зашиты — рантайм-интерполяция.
- [СТРАНИЦЫ] PrivacyPage/TermsPage переписаны на useTranslation + legal.* (fbfcacf1): оператор/ИНН через ownerLegalInfoApi.public() (как в футере лендинга), честный fallback «Оператор: данные уточняются»; RU/EN-переключатель в шапке (setLanguage из src/i18n.js, как на лендинге — app_language в localStorage); email/URL в текстах кликабельны (linkify); футер страниц на ключах landing.footer.*. Мобильный фикс: h1 text-3xl на <sm — «конфиденциальности» не рвалось на 360px (32dba6f2).
...
## 2026-08-27 — LEGAL-EMAIL-RUNTIME — контактный email на /privacy и /terms из ownerLegalInfoApi (ветка fix/legal-email-runtime)
- [ПРОБЛЕМА] Контактный email tihonovu560@gmail.com был зашит в 5 строках legal.* (privacy s8i3/s8i4/s11p2, terms s7p2/s9p2) во всех 4 файлах локалей — смена контакта требовала правки кода, хотя оператор/ИНН уже подтягивались рантайм.
- [ЛОКАЛИ] Зашитый email → плейсхолдер {{email}} в src/locales и public/locales × ru/en (10 замен). public/locales — зеркало src (подтверждено diff до копирования).
- [СТРАНИЦЫ] PrivacyPage/TermsPage: `const contactEmail = legalInfo?.contactEmail || 'tihonovu560@gmail.com'` + передача `{ email: contactEmail }` в t() на всех 5 ключах; mailto-ссылка — существующий linkify (email кликабелен). Поле API — contactEmail из ownerLegalInfoApi.public() (то же, что «будет указан в договоре оферте и политике»).
- [TEST] i18n-parity OK (2040 ключей, diff пустой) ✅; JSON валидны ✅; npm run build ✅ (0 ошибок); ui-audit /privacy + /terms × 5 ширин (360/428/768/1280/1920) × RU/EN = 20 снимков, 0 проблем ✅ (reports/legal-email-runtime/, не коммитятся). Рантайм-проверка reports/legal-email-runtime/verify-email.mjs (CORS-прокси на прод, как в ui-audit): обе страницы показывают tvinki05@yandex.ru из прод-API с mailto-ссылками ✅; без доступа к API — fallback tihonovu560@gmail.com ✅. Скрины с подтянутым email: runtime-email-privacy.png, runtime-email-terms.png.
- [НАБЛЮДЕНИЕ вне скоупа] linkify жадно захватывает завершающую точку предложения в mailto (s8i3/s8i4: «mailto:…ru.») — поведение было и до фикса, не тронуто по правилу «ничего кроме email-подстановки».
- [NOTE] Ручная проверка владельцем после деплоя: 1) сменить контактный email в кабинете владельца (редактор реквизитов) → /privacy и /terms показывают новый email без правок кода; 2) при недоступном API страницы показывают fallback tihonovu560@gmail.com.

## 2026-08-28 — Б4 OWNER-OMEGA — аудит 76 вкладок owner-кабинета + 8 задач (ветка feature/owner-omega)
- [З0 АУДИТ] 76 owner-страниц × 5 ширин (360/428/768/1280/1920) × RU/EN = 760 снимков (reports/owner-omega-audit/, не коммитятся). Найдено: 15 вкладок падали в ErrorBoundary на не-массиве из API (map/filter/slice) — нормализация Array.isArray/дефолты (`4a8c3072`, `3f7472f3` — salesMetrics summary.total). Ложные RAW-KEYS на autoImprove (package.json/main.jsx) и apiKeys (huggingface.co) — детектор ui-audit дополнен whitelist имён файлов, document.*, TLD .co (`c81a8790`). Точечная пересъёмка 20 вкладок = 200 снимков: 0 скроллов, 0 сырых ключей, 0 console errors, 0 редиректов (reports/owner-omega-reshoot/).
- [З1] owner/admin/staff без CTA продаж: UpgradeNudge, UpsellModal (OmegaChat), ResponsiveAdBanner глушатся по роли (`20839093`); owner-роль добавлена в ui-audit/ui-buttons.
- [З2] Триаж свободного текста в owner-боте покрыт commandExecutor; проактивный алерт об оплате с кнопками «Возврат…» (refund:ask → карточка ✅/❌) и «Клиенты» (`4b900e97`).
- [З3] Продление подписки «продли email на N дней» из TG с подтверждением ✅/❌ + паритет в кабинете (OverviewTab); общая обёртка ownerActionsService; уведомление клиенту TG+email (`0b094605`). QA: backend/scripts/qaExtendFlow.js 14/14 ✅.
- [З4] Hot-reload токенов ботов из ApiKeysTab: botReloader (подмена token + deleteWebhook→setWebhook), env остаётся fallback, текущие боты не ломаются (guard на мусорный/невалидный токен — bad_format/invalid_token) (`9d9cc265`, QA в qaExtendFlow ✅).
- [З5] Changelog-редактор из кабинета (UpdatesTab): модель ChangelogVersion + CRUD, audience all/client/owner, RU/EN; UpdateModal берёт динамический changelog с fallback на bundled — e2e-update-modal-smart зелёный (guest без owner-tech, owner видит оба блока, toast после reload) (`5ed15133`).
- [З6] Сбор расходов лайт: AiUsageLog (трекер AI-провайдеров по факту usage) + InfraExpense (ручной ввод инфраструктуры), карточка сводки day/week/month в FinanceTab (`5ed15133`, QA ✅).
- [З7] Группировка ключей в ApiKeysTab: AI-текст / AI-картинки / соцсети / платежи / боты (+Push/Email), статусы, подписи «куда вставить» (i18n ru/en); панель готовности Б3 не дублируется (`9d9cc265`).
- [З8] Резерв генерации обложек: Pollinations 5xx/таймаут → Cloudflare Workers AI (бинарный PNG, multipart sendPhoto); при полном провале — text-only пост + алерт владельцу в TG (кулдаун 1ч). Новых платных провайдеров не добавлено (`541023f7`).
- [TEST] qaExtendFlow 14/14 ✅ (продление, botReloader, expenseTracker); e2e-update-modal-smart ✅; npm run build 0 ошибок ✅; i18n-parity OK ✅; mojibake чист ✅; BOM нет ✅; node --check всех затронутых backend-файлов ✅.
- [НЕ ТРОНУТО] Платежи/ЮKassa, PlanConfig и значения тарифов, боты (только hot-reload токенов), лендинг, рубильники, реф-экономика 12%, юрстраницы, белый список ролей.
- [GIT] 10 коммитов, 46 файлов, +1577/−67. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feature/owner-omega
- [NOTE] Ручная проверка владельцем (10 минут): 1) /owner?tab=apiKeys — группы ключей, вставка токена бота → «Сохранить и применить», бот отвечает без рестарта; 2) /owner?tab=updates — добавить версию changelog (audience all, RU) → модалка обновлений у клиента показывает её; 3) /owner?tab=finance — карточка расходов AI + ручной ввод инфры; 4) TG owner-бот: «продли <email> на 3 дня» → ✅ → клиенту пришло уведомление; 5) пост с обложкой при недоступном Pollinations → пришёл text-only + алерт.

## 2026-08-28 — Б5 SECURITY — харденинг: audit-fix, гард-матрица, webhook-верификация, секреты, rate-limit, Sentry (ветка feature/security-hardening)
- [З0.1] DreamMode night shift — гард trends (`getTrends` → `{trends:[]}`), try/catch на шаг обучения, юнит-тест 5/5 (`fb71882b`).
- [З0.2] Key Health — DeepSeek 402 / OpenAI quota → `invalid` в ApiKeysTab; `isInvalidKeyError`: 402, insufficient balance, quota; юнит-тест 5/5 (`4fd82c81`).
- [З1] npm audit — backend 0 vuln (js-yaml, nanoid high устранены); frontend react-router-dom 6.30.6→7.18.2 + dompurify/nanoid/js-yaml — 0 vuln, build OK (`66f8c57d`; sync tracked node_modules `4ff35086`).
- [З2] TG webhook `secret_token` (приём 403 без токена + setWebhook в 4 точках); ЮKassa webhook — верификация платежа через GET API перед начислением, fail-closed + алерт владельцу; тест 6/6 (`1f309151`). Legacy webhook paymentController — та же верификация, qaAddonsFlow обновлён (`30fe7186`).
- [З3] Гард-матрица: `/api/owner/*` (overview/finance/audit/subscriptions/entity CRUD) — protect+owner/admin; IDOR `/api/analytics/churn-risk/:userId` — 403 на чужой userId (`fa57019c`). qaSecurityFlow — 39 проверок: anon→401, client→403 на owner/admin, staff→403 на owner-only (+whitelist support), IDOR, protect._id регрессия — зелёная (`2b78c70f`).
- [З4] Hardcoded секреты удалены из кода: `backend/config/env.js` (fallback JWT_SECRET + AI-ключи), `backend/scripts/seed.js` → env, `frontend/.env.example` → плейсхолдеры; в prod без JWT_SECRET — fail-fast. `.gitignore`: `.env*`, планы, agent-reports (`86bcdf8b`). Значения жили в git-истории — требуется ротация (см. SECURITY.md).
- [З5] Rate-limit: forgot-password 5/ч, support-тикеты 10/15мин (RU/EN 429); express-mongo-sanitize; CORS уже whitelist-only (проверено, не тронут) (`5b470d67`).
- [З6] Sentry back+front: DSN из env, без DSN молча off, beforeSend фильтрует PII/секреты; крит-события → TG-алерт с кулдауном 10 мин; 5xx-волна трекается; qaSecurityFlow --alert (`1fef7b98`).
- [З7] SECURITY.md — контакт для репортов, список защит (З2–З6), политика ротации ключей (`fb1995aa`).
- [TEST] qaSecurityFlow 39/39 ✅; ui-audit security 10/10 ✅; jest backend зелёный ✅; npm run build frontend 0 ошибок ✅; ЮKassa verify тест 6/6 ✅; юнит-тесты З0.1/З0.2 по 5/5 ✅.
- [НЕ ТРОНУТО] Логика платежей/начислений (только верификация webhook), PlanConfig и значения тарифов, боты (только приём webhook), CORS-политика, рубильники, лендинг, реф-экономика.
- [GIT] 12 коммитов Б5 (З0.1–З7 + node_modules sync), 32 файла, +1337/−127 (без node_modules). Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feature/security-hardening
- [NOTE] Ручная проверка владельцем (10 минут): 1) ротация засвеченных ключей (JWT_SECRET, GROQ, OPENROUTER, DEEPSEEK, YOUTUBE, BUFFER) — значения были в git-истории; 2) TG-бот отвечает (webhook с secret_token принимается); 3) тестовый платёж → начисление проходит (webhook верифицируется через API ЮKassa); 4) кабинет клиента: навигация по разделам после react-router-dom 7 — без сбоев; 5) forgot-password 6-й раз за час → 429 с понятным сообщением RU/EN.

## 2026-08-28 — МИКРОФИКС RATE-LIMIT-VPN — лимиты без привязки к голому IP (ветка fix/ratelimit-vpn)
- [БЫЛО] Все лимитеры Б5-З5 (`backend/middleware/rateLimiter.js`) ключевались по голому IP (`keyGenerator: getClientIp`): login 10/15мин, register 5/ч, forgot-password 5/ч, support-тикеты 10/15мин, omega/chat 50/15мин + бан IP на 1ч через BlockedIP. За общим IP VPN страдали бы легальные пользователи.
- [СТАЛО] Auth-эндпоинты (login/register/forgot-password): ключ = IP + нормализованный email (trim+lowercase). Авторизованные (apiLimiter, omega/chat, support-тикеты): ключ = userId (из req.user или JWT-payload без верификации — верифицирует protect), IP только fallback, публичная форма тикетов — IP+email. Бан IP на 1ч из omegaChatLimiter убран (остался 429). 429-ответы: RU/EN + «Повторите через ~N мин / Try again in ~N min» (из RateLimit resetTime).
- [ПРОВЕРЕНО-ОТСУТСТВУЕТ] Привязки сессий/JWT к IP нет, геоблокировок и банов по стране нет (geo — только валюта на лендинге). Autoban anti-DDoS (1000 req/мин с IP) оставлен — это не сессионная привязка.
...
## 2026-08-28 — Б4-ДОП АУДИТ КАБИНЕТОВ admin/staff + готовность поддержки (ветка fix/staff-audit-dop)
- [З1 АУДИТ] ui-audit admin+staff × 360/428/768/1280/1920 × RU/EN: 0 скроллов, 0 сырых ключей, 0 console errors (отчёт reports/ui-audit-b3final, shots reports/staff-dop-audit). ui-buttons: 0 мёртвых кнопок (reports/staff-dop-buttons). Починено в этой ветке: admin users падал на не-массиве ответа; staff-кабинет читал legacy /api/tickets → переведён на /api/support (живой список, ответ клиенту в TG, takeover, эскалация); FAB staff перекрывался MobileBottomNav; админ-таблица на мобильной ширине уползала под навбар; моковая «Модерация» в staff (ban/dismiss/warn не ходили в API) — выпилена; KB-статьи хардкодом RU — переведены на i18n RU/EN; i18n-parity: 56 пробелов (admin.*/staff.* в public-locales) — досинхронизировано, parity OK; в staff-модалку тикета добавлен блок «контекст клиента» (GET /api/support/:id/context — всего обращений, последнее, AI-подсказка) (`31c8df9a`,`a43966f6`,`7fbf8f47`,`5ee05884`,`47d2ca90`,`9c489725`).
- [З2 ФУНКЦИИ] Таблица «функция → есть/нет»: видеть все тикеты ✅; ответить клиенту (TG) ✅; takeover ✅; эскалация владельцу ✅ + TG-уведомление назначенному; контекст клиента ✅ (добавлено этим батчем); бан/разбан и продление тарифа ✅ в admin-кабинете с подтверждением ✅/❌ (по гард-матрице Б3 staff не банит и не продлевает — только эскалация); создание staff-аккаунта ✅ из owner-кабинета (POST /api/owner/staff, временный пароль показывается один раз). Некритичная дыра в бэклог владельца: живая модерация пользовательского контента (мок удалён, реальной фичи нет).
- [З3 ГОТОВНОСТЬ] Создание staff из owner-кабинета; уведомления о новых тикетах владельцу и назначенному сотруднику; takeover владельцем в любой момент; бан/разбан (isActive реально режет логин и старые токены), продление/сокращение ±дни с подтверждением. Всё проверено e2e (`scripts/qa-staff-dop-e2e.mjs`, 21/21 ✅).
- [З3.БАГ-ФИКС] Найдено и починено e2e: 1) POST /api/owner/staff перехватывался generic router.post('/:entity') → 400 (роут поднят выше); 2) blockClient ставил status='blocked', но login/protect смотрят на isActive — забаненный продолжал работать. Теперь бан выставляет isActive=false, разбан восстанавливает (`ca7a23b1`). Микрофикс owner-access (матрица Б3: staff/admin НЕ может создать privileged-аккаунт, owner-only гард, 403 проверены в qaSecurityFlow C2) — зафиксирован здесь, т.к. отдельной записи не было.
- [З4 LINKIFY-ТОЧка] linkify жадно забирал завершающую точку в href — исправлено (точка/запятая вне ссылки, в тексте остаётся), покрыты юрстраницы и все вызовы linkify (`37bd926c`).
...
## 2026-09-04 — REFERRAL-HOTFIX + ДОП: реферальная комиссия из кабинета владельца (ветка fix/referral-hotfix)
- [ДИАГНОСТИКА] На origin/main уже 12% (fix приехал не через несмёрженную fix/ref-12pct, а другим путём), но: Math.round вместо ceil, нет отзыва при возврате, процент захардкожен, тексты расходились (backend «12%», owner-ReferralsTab «40%»). Сценарий B: доработка на свежей ветке от origin/main.
- [МЕХАНИКА] `backend/services/referralService.js`: комиссия = Math.ceil(amount × pct / 100) в ₽; pct — из OwnerSettings (getOwnerFlags, кэш ≤60с как у рубильников), дефолт 12. Новое markReferralRefund: refund.succeeded → −ceil(amount × pct/100), идемпотентно (новый флаг refundMarked в Referral + контроллер дёргает только при modifiedCount>0). Фикс $4 в логике отсутствует (остались только исторические комментарии).
- [НАСТРОЙКА] OwnerSettings.referralPercent (Number, 0–50, default 12) + setReferralPercent с валидацией. PUT /api/owner/control/flags принимает referralPercent — authorize('owner'): client/admin → 403 (проверено тестом). GET flags отдаёт процент (owner+admin read).
- [UI] Кабинет владельца (OverviewTab): поле «Реферальная комиссия, %» (0–50, save → toast). ReferralsTab: текст partner-тира динамический из /analytics/referrals (referralPercent) — обещание = механика всегда; захардкоженные «40% комиссия»/«12%» убраны. getReferralData отдаёт referralPercent и динамический nextReward для partner.
- [TEST] qaReferralFlow 28/28 ✅: 693₽→+84 (ceil), 3493₽→+420 (итого 504), дубль webhook без повторного начисления, 990₽→+119, 4990₽→+599, возврат 990₽→−119, дубль refund без повторного списания, owner 12→15 → новая оплата 990₽→+149 (старые не тронуты), текст тира «12%»→«15%», client/admin PUT → 403. qaSecurityFlow вся матрица ✅. npm run build 0 ошибок ✅. i18n-parity OK ✅ (4 locale-файла).
- [ПОСТОРОННЕЕ, НЕ ИСПРАВЛЕНО] регистрация в тесте флаковала на 6-м сабмите (rate-limit auth/register) — в тест добавлена диагностика; Referral требует и code, и referralCode (дублирующие поля схемы).
- [НЕ ТРОНУТО] платежи/ЮKassa (refund-блок только дополнен отзывом), PlanConfig и тарифы, боты, лендинг, рубильники (поведение), гард Б5, rate-limit, view-as, аддоны.
- [GIT] ветка fix/referral-hotfix от origin/main: 5 коммитов, 12 файлов, +278/−28. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/referral-hotfix

## 2026-09-04 — ADDONS-EDITOR-REDESIGN: компактный редактор аддонов + AI-панель цены (ветка fix/addons-editor-redesign)
- [РЕДАКТОР] `frontend/src/components/subscriptions/AddonMarketplace.jsx`: состав аддона — компактные строки с iOS-тумблерами (role=switch) вместо чекбоксов-простыни; функции implemented:false свёрнуты под нативным <details> с бейджем «скоро», тумблеры disabled (не продаются — подтверждено QA: autopilot.full → 402). Цена+валюта+скидка — одна строка grid-cols-3 с ярлыками; «Что входит» — rows=2.
- [STICKY-СЕЙВ] Кнопка «Сохранить» sticky внизу карточки: disabled до первого изменения (снапшот savedEdits, dirty = JSON-diff), янтарная точка «несохранено» + sr-only текст, после сохранения — toast. Тумблер «Аддон включён» (isActive) — тот же iOS-switch; сохранение проверено фактом: PATCH /addons/:id/price принимает isActive (backend/routes/addons.js:215), QA «выкл аддона → на витрине нет» ✅.
- [AI-ПАНЕЛЬ] Inline-панель цены в карточке (вместо модалки с recharts): себестоимость (basePrice) / рынок (средняя competitorPrices) / рекомендация (recommendedPrice + уверенность + обоснование) + «Применить» — подставляет цену в поле и включает dirty-точку, сама НЕ сохраняет. analyzePrice больше не дёргает load() — несохранённые правки не затираются.
- [ЗАГОЛОВОК] У owner — «🛠 Управление аддонами» (t('addons.manageAddons')), у клиентов витрина как была. Битого alt «aiviral-studio» на странице аддонов нет (посторонняя находка: строка 'aiviral-studio.ru' — это siteUrl в LegalPage, не alt).
- [ГЕЙТ UI] overflow-гейт 360/428/768/1280/1920 × RU/EN — 10/10 без горизонтального скролла (scripts/addons-editor-redesign-shots.mjs); тултипы (title) у обрезанного (truncate/line-clamp); стили только дизайн-системы (glass-card, var(--primary), btn); motion-reduce:transition-none на всех transition; empty-state inline («Аддоны не найдены»); focus-trap/Esc не нужен — модалка удалена, панель inline.
- [TEST] npm run build 0 ошибок ✅; esbuild-синтаксис ✅; qaAddonsFlow 0 ❌ (регрессия entitlement/снапшотов: снапшот состава у подписчика жив, «скоро» не продаётся, client/admin PATCH → 403) ✅; qaSecurityFlow вся матрица ✅; i18n-parity OK (4 locale-файла, +8 ключей addons.*) ✅. Скрины: reports/addons-editor-redesign/ (1280 RU редактор, dirty-состояние, AI-панель, 360 RU full-page); «до» — reports/addons-marketplace-restore/01-owner-addons-editor-1280-ru.png.
- [НЕ ТРОНУТО] логика entitlement/снапшотов UserAddon, платежи/ЮKassa, PlanConfig, рефералка, боты, лендинг, рубильники, гард Б5, rate-limit, view-as. Изменения только в представлении + i18n + скрипт скринов.
- [GIT] ветка fix/addons-editor-redesign от origin/main: 2 коммита, 6 файлов, +364/−177. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/addons-editor-redesign

## 2026-09-05 — CI-FOUNDATION: автогейты + qa-launch + AGENTS.md (ветка ci/foundation)
- [WORKFLOW] `.github/workflows/ci.yml`: на PR и пуш в fix/*/ci/*/main — mongo:6 service, backend deps + `node --check` всех backend/js, backend up :18080, frontend `npm ci && npm run build` (0 ошибок), vite preview :4173, playwright chromium, `node scripts/qa-launch.mjs`. Падение любого шага = PR красный. Зелёный прогон: run 33948868441 (Success, 8m06s).
- [QA-LAUNCH] `scripts/qa-launch.mjs`: одна кнопка — 19 qa-скриптов последовательно (явный ORDER по зависимостям: journey-1 → сидеры → journey 2–4 → support/referral → addons/security → e2e), автоподхват новых qa-*.mjs/qa*.js, единая сводка скрипт→✅/❌→время, tail-дамп при падении, exit 1 если есть ❌. Итог в CI: 19/19 зелёных.
- [AGENTS.md] Прошивка кодера в корне: правила (без сварма, 1 ответ=1–3 файла+node --check, wip-коммиты, отчёт без пуша=не сдан, негативные тесты), красные зоны (платежи/ЮKassa, PlanConfig, рефералка OwnerSettings.referralPercent, боты, лендинг, рубильники, гард Б5, rate-limit, view-as, entitlement/снапшоты аддонов), стандарты (UI_QUALITY_GATE, i18n-parity, секреты только env), ПРАВИЛО ЭКОНОМИКИ (цены только через кабинет/PlanConfig, кредит ≈0.24₽, маржа платных тарифов ≥70%, изменение цен — через владельца). Ссылки на PROJECT_CONTEXT.md/PROGRESS_REPORT.md без дублирования.
- [ФИКСЫ ПО ХОДУ] ① `qa-role-switch-flash.mjs`: шаг «назад на owner» под VIEW-AS-PERSIST — выход через кнопку «Выйти» плашки (прямой переход /owner в режиме staff теперь корректно → /unauthorized, гард не ослаблен). ② `qa-journey-3-plans.mjs`: YT-карточка пропускается при QA_SKIP_YT=1 (в CI нет YOUTUBE_API_KEY). ③ `qa-journey-1-register.mjs`: ожидание ответа OMEGA 60→120с (CI холоднее). ④ `rateLimiter.isWhitelisted`: нормализация ::1/::ffff: в IPv4-форму — точное равенство сохранено, whitelist не расширен; qaRateLimitVpn зелёный локально без OWNER_IP.
- [CI-ГРАБЛИ] npm init -y падает в dot-папке (.tmp-ui-polish → package.json пишется вручную); sudo теряет PATH (sudo env PATH=$PATH); NODE_ENV=test глобально на job собирал dev-React → validateDOMNesting warnings валили console-error проверки (убрано); OWNER_IP=127.0.0.1 в CI — общий лимитёр не душит e2e-матрицы (auth-лимитеры whitelist не учитывают, qaRateLimitVpn в силе).
- [TEST] CI run 33948868441: 19/19 ✅ (journey 1–4, support, referral, addons, security, extend, ratelimit, token, view-as, role-switch, staff-dop, onboarding, honesty). Скрин: reports/ci-foundation/ci-green-run.png.
- [ПОСТОРОННЕЕ, НЕ ИСПРАВЛЕНО] витрина /api/version/structure → 404 локально (роут есть только на проде?); GitHub Actions предупреждает о deprecate Node 20 раннеров (миграция на node 24 — отдельной задачей).
- [GIT] ветка ci/foundation от origin/main: 7 коммитов. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...ci/foundation

## 2026-09-05 — TG-REPORT-HOOK: сдача батчей владельцу в TG с кнопками approve/reject (ветка feat/tg-report-hook)
- [СДЕЛАНО] `scripts/report-to-owner.mjs` — отчёт владельцу в owner-бот (батч/ветка/compare/тесты/файлы/не-тронуто) с inline-кнопками [✅ Принять и смёржить] [❌ Отклонить] (`callback_data: breport:approve/reject`). chat_id: env `TELEGRAM_OWNER_CHAT_ID` → Mongo `OwnerSettings.ownerTelegramChatId`; токен: env `TELEGRAM_OWNER_BOT_TOKEN`/`TELEGRAM_BOT_TOKEN`. TG недоступен → «TG недоступен» + текст в stdout, exit 0.
- [СДЕЛАНО] `backend/services/batchReport.js` — обработчик кнопок: ✅ → CI по ветке через GitHub API (`GITHUB_TOKEN`/`GITHUB_API_KEY`, последний workflow run = success) → зелёный: мерж в main (`POST /repos/:repo/merges`), ответ «✅ Смёржено: <ветка> → main»; красный/идёт → «⏳ CI не зелёный, мерж заблокирован». ❌ → «Опиши проблему одним сообщением», следующий текст владельца = причина → TG-подтверждение + запись в PROGRESS_REPORT.md. Ветка/selftest парсятся из текста отчёта (callback_data ≤64 байт не лимитирует).
- [ИНТЕГРАЦИЯ] `backend/services/ownerBot.js`: ветка `breport:*` в `callback_query` (после owner-гарда), перехват причины в `bot.on('message')` (после takeover, до FAQ). Логика ботов не менялась — только добавлены обработчики.
- [ТЕСТЫ] `node --check` ×3 ✅; import ownerBot ✅; мок-тесты: parse ✅, selftest reject → подтверждение без мержа ✅, selftest approve → мерж пропущен ✅, real reject → причина записана ✅, постороннее сообщение не перехватывается ✅.
- [БЛОКЕР] локальный самотест с живой доставкой: `TELEGRAM_OWNER_BOT_TOKEN` есть только в env Render, `ownerTelegramChatId` в Mongo пуст — сообщение владельцу с кнопками физически уходит только при запуске там, где env задан (Render shell или локально с экспортом). Проверено: graceful fallback + stdout-дубль ✅.
- [ПОСТОРОННЕЕ, НЕ ИСПРАВЛЕНО] в локальном `backend/.env` нет TG-секретов вообще (ни owner-, ни client-бота); `OwnerSettings.ownerTelegramChatId` пуст — первая привязка владельца через «мой id» ещё не делалась.
- [GIT] ветка feat/tg-report-hook от origin/main: 5 коммитов, 3 файла. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feat/tg-report-hook

## 2026-09-05 — TG-REPORT-HOOK-ФИКС: источник TG-ключей = apikeys кабинета + живой самотест (ветка feat/tg-report-hook)
- [ФИКС] `scripts/report-to-owner.mjs`: TG-ключи теперь env → коллекция `apikeys` (provider `telegram_owner_bot` / `telegram_chat_id` — туда пишет ApiKeysTab через `PATCH /api-keys/:provider`, hot-reload через `invalidateApiKeysCache`/`botReloader`) + `OwnerSettings.ownerTelegramChatId`. MONGO_URI/MONGODB_URI из backend/.env. Секреты в логи/отчёты не выводятся.
- [ПРОВЕРКА] DB-fallback end-to-end пробой с фейковыми ключами в локальной dev-БД: скрипт нашёл их в apikeys, сходил в TG API (ожидаемый отказ на фейке), корректно «TG недоступен» + stdout, exit 0; фейки удалены.
- [ЖИВОЙ САМОТЕСТ ✅] `node scripts/report-to-owner.mjs --batch "TG-REPORT-HOOK SELFTEST" --selftest` — сообщение с обеими кнопками доставлено владельцу в TG (факт подтверждён ответом Bot API, без токенов/chat_id в логах). Нажатия ❌ и ✅ на самотесте обработаны реальным `handleBatchCallback` с живой доставкой подтверждений: «❌ Отклонено (selftest)… Мерж не выполнялся» и «✅ Selftest approve: мерж пропущен». Мерж не выполнялся. ВАЖНО: обработка нажатий на проде активируется после мержа в main + автодеплоя Render (webhook owner-бота живёт на проде, до деплоя прод-бот на старом коде кнопки молча игнорирует).
- [БЛОКЕР ДЛЯ ✅] нужен GITHUB_TOKEN (PAT с repo scope) в env Render — локальный GITHUB_API_KEY проверен: это GitHub Models ключ (/user → 401), для мержа не годится. Без него approve честно отвечает «⏳ CI не зелёный, мерж заблокирован».
- [ПОСТОРОННЕЕ, НЕ ИСПРАВЛЕНО] локальный MONGO_URI → localhost dev-БД (прод-ключи там отсутствуют штатно); в dev-БД 10 документов ownersettings с пустым ownerTelegramChatId.
- [GIT] ветка feat/tg-report-hook: +1 коммит (фикс скрипта). Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feat/tg-report-hook

## 2026-09-05 — ADDONS-EDITOR-REDESIGN (доп): Ω-кнопка, рыночный диапазон, empty-state анализа (ветка feat/addons-editor-redesign)
- [ДОРАБОТКА ПО ВЕРДИКТУ] База батча уже была в main (395526c2: iOS-тумблеры, «скоро» свёрнуто с бейджем количества, sticky-сейв с dirty-точкой, тумблер «Аддон включён» сохранён, цена+валюта+скидка одной строкой). Этот прогон закрыл остаток чек-листа: кнопка «Ω Анализ цены» (было «🤖 AI Анализ цены»); AI-панель: средняя рынка → честный диапазон min–max по competitorPrices (ключ addons.marketRange); пусто/ошибка analyze-price → inline empty-state «Нет данных для анализа» (ключ addons.noAnalysisData), без выдуманных цифр, вместо молчаливого toast; tabular-nums в полях цена/скидка и цифрах AI-панели. «Применить» подставляет только в поле — сохранение исключительно через «Сохранить».
- [TEST] build 0 ошибок ✅; i18n-parity OK (4 locale-файла, +2 ключа) ✅; qa-launch 18/19 (qaAddonsFlow ✅ — регрессии entitlement/снапшотов нет; qaSecurityFlow ✅ — вся матрица); упавший qa-journey-1-register — флаки, повтор с QA_BASE=4173 ✅ (фактически 19/19). Overflow-гейт: 360/428/768/1280/1920 × RU/EN — h-scroll=false во всех 10 ✅. Скрины: reports/addons-editor-redesign/ (01 — 1280 RU редактор, 02 — dirty+sticky-сейв, 03 — AI-панель с диапазоном, 04 — 360 RU full-page).
- [НЕ ТРОНУТО] логика entitlement/снапшотов UserAddon, платежи/ЮKassa, PlanConfig и тарифы, боты, лендинг, рубильники, гард Б5, rate-limit, view-as, модалка обновлений, маркетплейс-логика. Изменения только представление + i18n + селектор в shots-скрипте.
- [ПОСТОРОННЕЕ, НЕ ИСПРАВЛЕНО] qa-journey-1-register флаки (в прогоне qa-launch — таймаут на 74с, повтор зелёный); на скрине 360 артефакт — открытая шторка уведомлений поверх страницы (состояние сессии, не дефект вёрстки; overflow-гейт чист).
- [GIT] ветка feat/addons-editor-redesign от origin/main: 3 коммита, 7 файлов. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feat/addons-editor-redesign

## 2026-09-05 — TG-ASK-OWNER: вопросы кодера владельцу в TG + терминал, webhook-гард, правило молчания (ветка feat/tg-ask-owner)
- [ЗАДАЧА 0 — WEBHOOK-ГАРД] `backend/utils/tgWebhookGuard.js`: `isProdWebhookHost()` — setWebhook/deleteWebhook ТОЛЬКО когда `RENDER_EXTERNAL_URL` совпадает с прод-URL (`PROD_BACKEND_URL`, дефолт aiviral-backend.onrender.com; оверрайд стейджинга `TG_WEBHOOK_FORCE=1`). Гард вшит в ownerBot.js (init-webhook), omegaBot.js (оба места: стартовый deleteWebhook и trySetWebhook; локально — без polling), botReloader.js (hot-reload токена на не-проде: токен применяется, webhook не переустанавливается, `webhookSkipped: true`). Причина: локальный запуск перезаписывал webhook без secret_token → прод отклонял команды владельца (403).
- [ЗАДАЧА 1 — ASK-OWNER] `scripts/ask-owner.mjs`: вопрос владельцу в ДВА канала — TG owner-бота (❓ контекст + вопрос + inline-кнопки `bask:<qid>:<idx>` или «ответь текстом») и терминал (stdin, цифра/текст). Первый ответ побеждает; проигравший канал закрывается: TG-сообщение редактируется («отвечено в другом канале»), readline гасится. TG-ответ: webhook прода → `backend/services/askOwner.js` (паттерн batchReport.js: `handleAskCallback`/`handleAskFreeText` за owner-гардом в ownerBot.js) → атомарный updateOne в Mongo `askowner` → скрипт поллит запись. Свободный текст перехватывается только при mode:'free' (вопросы с кнопками текст не едят — уходят в обычный контур команд).
- [ЗАДАЧА 2 — ПРАВИЛО МОЛЧАНИЯ] вшито в шапку ask-owner.mjs и AGENTS.md («Вопросы владельцу»): вопросы в батче запрещены кроме реального блокера; нет ответа 15 мин (--timeout от отправки в оба канала) → безопасный дефолт + строка в отчёт «решил сам: X, причина: Y»; APPROVE-ЗОНА (--approve-zone: деньги/тарифы/безопасность/удаление/секреты) — сам не решать никогда (exit 3 «ждёт решения владельца»); пуш — сам, мерж — только ✅ владельца в TG. AskUserQuestion не используется.
- [ЗАДАЧА 3 — ЖИВОЙ ТЕСТ ✅] ① `--selftest`: вопрос доставлен в TG (message_id подтверждён Bot API) + напечатан в терминале; ответ «Вижу, но есть замечание» прошёл по полной цепочке: POST /webhook/owner (callback_query `bask:<qid>:1` с secret_token) → processUpdate → owner-гард → handleAskCallback → Mongo → поллинг скрипта → «Ответ получен из TG», терминал закрыт, TG-сообщение отредактировано «✅ Ответ принят». Примечание как в TG-REPORT-HOOK: нажатие реальной кнопки на проде активируется после мержа+автодеплоя (прод-бот на старом коде bask:* игнорирует); транспорт прод→handler идентичен уже работающему breport:*. ② Таймаут 20 сек (--timeout 20): сработал безопасный дефолт «Пропустить (безопасно)», вывод «решил сам: …, причина: нет ответа владельца», exit 0. ③ Webhook-гард: локальный backend (порт 5000) — в логе «не прод-хост — webhook не трогаю, polling не включаю»; getWebhookInfo до и после локального запуска идентичны (url=прод /webhook/owner, pending=0, last_error=null) — webhook НЕ перезаписался, прод-бот продолжает отвечать на /start.
- [QA-BOTS] `scripts/qa-bots.mjs` — регрессия обоих ботов через Bot API: owner-бот getMe ✅, webhook жив (url=прод, pending=0) ✅, тестовые sendMessage владельцу ✅ (статус + отчёт с кнопками breport selftest), «/start отвечает» — косвенно (Bot API не шлёт /start от имени пользователя: webhook без ошибок, pending=0). Клиентский бот: ⏭ токен есть только в env прода (локально ни env, ни apikeys dev-БД) — пропуск без падения; в CI без ключей — скип exit 0. Итог 6/6 ✅.
- [ПРОВЕРКИ] `node --check` ×6 файлов ✅; frontend `npm run build` 0 ошибок ✅; qa-launch 17/19, оба падения (qa-journey-1-register, qa-view-as) — флаки, повторы зелёные (фактически 19/19); qaSecurityFlow ✅ (гард-матрица Б5 не тронута). Порядок qa-launch: backend :18080 + preview :4173 поднимаются заранее (скрипт сам среду не поднимает).
- [ПОСТОРОННЕЕ, НЕ ИСПРАВЛЕНО] ① на 4173 висел зомби-процесс vite preview из прошлых прогонов (прибит вручную); vite preview по умолчанию слушает только IPv6 [::1] — для qa-скриптов нужен `--host 127.0.0.1`. ② В локальной dev-БД нет токена клиентского бота (только env прода). ③ Первая TG-отправка из скрипта однажды уперлась в 10с таймаут при живом API — таймаут поднят до 20с.
- [НЕ ТРОНУТО] платежи/ЮKassa, PlanConfig и тарифы, рефералка, логика ботов (только добавлены обработчик bask:* и webhook-гард), лендинг, модалка обновлений, рубильники, гард Б5, rate-limit, view-as, аддоны. Секреты — только env, в логи не выводились.
- [GIT] ветка feat/tg-ask-owner от origin/main. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feat/tg-ask-owner

## 2026-09-05 — TG-OWNER-CONTEXT: владельческий контур свободного текста в owner-боте (ветка feat/tg-owner-context, база feat/tg-ask-owner)
- [СДЕЛАНО] `backend/services/ownerFreeText.js`: свободный текст владельца в owner-боте больше не уходит в клиентскую витрину. Детектор: приветствие/статус («привет», «как дела», «статус», «что нового»…) → сводка из реальных данных: в работе (ветка + последний коммит из git, fallback «нет данных»), прод (uptime, Mongo, число AI-провайдеров с ключами, Stripe — без значений секретов), числа (клиенты/открытые тикеты/заказы рекламы из БД), следующее (последний заголовок PROGRESS_REPORT.md). Вопросы о проекте («что с рекламой», «когда запуск»…) → AI строго по снапшоту фактов (системный промпт «ассистент владельца, без маркетинга»). Команды («сделай/опубликуй/изучи…») НЕ перехватываются — уходят в submitOwnerCommand как раньше.
- [АНТИГАЛЛЮЦИНАЦИИ] chatWithAI при мёртвых провайдерах возвращает `{provider:'fallback'}` с шаблонной клиентской витриной («Вот идея для поста…») — такой ответ владельцу не показывается: вместо него честное «AI-провайдеры недоступны, не выдумываю» + снапшот фактов. Ловятся provider 'fallback'/'smart-fallback', source 'template', success===false.
- [ИНТЕГРАЦИЯ] `backend/services/ownerBot.js`: хук `handleOwnerFreeText` в message-handler после bask free-text, до submitOwnerCommand. Owner-гард не дублируется — не-владельцы отсекаются выше (строка «Not owner → simple menu»), контур видят только владельцы. Логика ботов не менялась — только добавлен контур.
- [ЖИВОЙ ТЕСТ ✅] локальный backend + POST /webhook/owner (message update от chat_id владельца): «привет» → 200, владельцу в TG ушла сводка «Статус проекта» (ветка/прод/числа/последний отчёт — подтверждено юнит-прогоном содержимого); «что с рекламой?» → честный ответ с фактами, БЕЗ клиентской витрины (юнит: «AI-провайдеры недоступны… Факты: …»); «опубликуй пост про кофе» → командный контур не перехвачен (OmegaCommand intent=POST создан, выполнялся; публикация не ушла — локально нет telegram_chat_id, ожидаемо). Негативный: чужой chat_id → существующий гард «Not owner».
- [QA-BOTS ФИНАЛ ✅] `node scripts/qa-bots.mjs` — 6/6 ✅: owner-бот getMe ✅, webhook жив (url=прод, pending=0) ✅, тестовые sendMessage владельцу ✅ (статус + отчёт с кнопками), клиентский бот ⏭ (токен только в env прода — пропуск без падения). Добавлена устойчивость: сетевой сбой TG API → ⏭ пропуск, не краснит CI (пойман реальный ConnectTimeout).
- [ПРОВЕРКИ] node --check ✅ (ownerFreeText.js, ownerBot.js, qa-bots.mjs); qaSecurityFlow — ВСЯ МАТРИЦА ЗЕЛЁНАЯ ✅ (гард Б5 не тронут).
- [ПОСТОРОННЕЕ, НЕ ИСПРАВЛЕНО] ① ownerBot message-handler строка «Not owner → simple menu» шлёт bot.sendMessage БЕЗ catch — на несуществующем chat_id (фейковый апдейт) TelegramError «400 chat not found» крашит процесс (unhandled rejection); в реальности TG не присылает апдейты от несуществующих чатов, но гасить такие sendMessage стоит отдельным батчем. ② Локальные AI-ключи невалидны (401/402 у всех провайдеров в dev-БД/env) — локальные AI-ответы всегда fallback.
- [НЕ ТРОНУТО] платежи/ЮKassa, PlanConfig, логика ботов (только +владельческий контур), лендинг, рубильники, гард Б5, rate-limit, view-as, аддоны. Секреты — env.
- [GIT] ветка feat/tg-owner-context от feat/tg-ask-owner (те же файлы ownerBot.js; после мержа первой — без конфликтов). Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feat/tg-owner-context

## 2026-09-06 — DESIGN-LAB PR-2: дизайн-лаборатория /preview/* — 7 экранов, глобальное меню, светлая тема, студия (ветка feat/design-lab-preview)
- [СДЕЛАНО] Публичная дизайн-лаборатория `/preview/*` (демо-данные из `frontend/src/preview/demoData.js`, API не дёргается, прод-роуты не подменяются): Фокус-чат (пилюля квоты «118✦» → модалка детализации + «Пополнить», цена действия до отправки), Люкс-хаб (кольцо баланса 350/500 с count-up + reduced-motion, платежи «Пополнения/Списания» со статусами), кабинет рекламодателя в 3 вариантах контентной зоны — 🅰 Дашборд (KPI+график+таблица), 🅱 Командный центр (кампания дня+воронка+неделя), 🅲 Терминал (тикер+AI-диагноз+dense-таблица); Слоты (баннер в чате / сайдбар / мобильная плашка, все с пометкой «Реклама»); Студия (8 инструментов с ценой в ✦ до запуска, режимы Ручной/OMEGA, OMEGA Control — 4 контура с живыми статусами и локальным переключением, контур постов: «Вариант N» = выбор к публикации, утверждённый пост 1:1 с чек-листом хэштеги+CTA+ссылка; ручной режим — редактор с живым чек-листом).
- [ГЛОБАЛЬНОЕ МЕНЮ — уточнение владельца] Левое меню ОСТАЁТСЯ на всех экранах включая чат: 100% паритет пунктов с AppSidebar (OWNER_GROUPS — 7 групп, все пункты от Обзора до DevStudio), приведено к дизайн-системе (CSS-vars, active = primary-soft + левая полоса, сворачиваемые группы). Десктоп ≥1024 — сайдбар 280px всегда; мобильный — бургер-дроуер (Esc/focus-trap через useModalA11y) со ВСЕМИ пунктами. Пункты прод-меню в лаборатории демонстрационные (не навигируют, title-подсказка). Выбор А/Б/В касается только контентной зоны кабинета рекламодателя.
- [СВЕТЛАЯ ТЕМА — ПОЧИНКА] `.light …` блоки в globals.css/index.css были мёртвые: useTheme/ThemeToggle ставили только класс `dark`, а index.css `:root` перебивал --bg/--text/--border тёмными — «светлая» тема оставалась тёмной. Теперь оба хука ставят класс `light`/`dark` + `data-theme` всегда.
- [ЮKASSA WEBHOOK — ФАКТ] Webhook (`paymentController.yookassaWebhookHandler` → `yookassaService`) читает ключи через `getProviderKey('yookassa_shop_id'/'yookassa_secret')`: приоритет MongoDB `apikeys` (кабинет владельца, hot-reload, кэш; isActive=false = явный запрет, env игнорируется) → env `YOOKASSA_SHOP_ID`/`YOOKASSA_SECRET_KEY` как запасной. Код не менялся (красная зона), зафиксировано для отчёта.
- [ПРОВЕРКИ] frontend `npm run build` 0 ошибок ✅ (чанки PreviewLab/PreviewStudio собраны); i18n-parity OK ✅ (новых ключей нет — демо-тексты хардкод RU, как утверждённые экраны); qa-launch 20/21 + qa-journey-1-register повтором зелёный (известный флаки-таймаут ~80с, фактически 21/21) ✅; qaSecurityFlow ✅ (матрица Б5); скрины 30 шт reports/design-lab-pr2/: 7 экранов × dark/light × desktop 1280/iPhone 390 + бургер-меню открытым (обе темы) — налезаний/скроллов нет, обе темы читаемы.
...
## 2026-09-06 — DESIGN-LAB-ПРИМЕНЕНИЕ: выбранный дизайн в прод + хвосты (ветка feat/design-lab-preview)
- [РЕШЕНИЕ ВЛАДЕЛЬЦА — ФИНАЛ, перекрывает запись PR-2] Кабинет рекламодателя: **вариант Б «Командный центр»** (по живым preview; запись PR-2 про вариант А отменена). Чат «Фокус-чат», профиль «Люкс-хаб», слоты, студия — как в preview.
- [ШАГ 0.1 ЮKassa webhook] Контур не менялся (красная зона). Unit верификации 6/6 PASS (подделка/подмена userId/fail-closed/refund). Живой платёж 1₽ → webhook → начисление = действие владельца на проде (деньги — approve-зона): Кабинет → API Ключи → 🧪 ЮKassa → оплатить тестовой картой. Ключи на проде резолвятся из MongoDB apikeys с hot-reload (факт PR-2); в локальной dev-БД их штатно нет.
- [ШАГ 0.2 Контур постов] Перенесён из preview-студии в owner-бот TG: «черновик <тема>» → 3 варианта (AI) → «вариант N» = выбор → превью РОВНО того текста + чек-лист (хэштеги/CTA/ссылка) → «Публикую? ✅/❌» → publishExactChannelText (telegramChannelManager): текст уходит 1:1, единственная правка — repair невалидного HTML. Живой пруф на проде — после деплоя (нужен прод-бот на новом коде).
- [ШАГ 0.3 OMEGA Control в TG] `backend/services/omegaControl.js`: /omega → панель 4 контуров с живыми статусами (AutoPilot — флаг+расписание; Dream — lastRun/метрики из ai/omega/dreamMode; Self-Healing — health/провайдеры из getSelfHealingStatus; Memory — узлов CognitiveNode + последняя запись). Рубильники `OwnerSettings.omegaControl` (дефолт true = поведение прода не меняется), гейты: autoPilot.runAutoPilot, dreamMode.runNightShift/sendMorningBriefing, selfHealing.runHealingTick. Вкл/выкл — только через превью-подтверждение ✅/❌ (oc:ask→oc:set). Memory — статус-only осознанно: системный контур, отключение сломало бы Dream/посты. Смоук на живой БД: панель + тумблер OFF→ON ✅. Живые скрины в TG — после деплоя.
- [ШАГ 0.4] «⚠️ Тикет не найден» → «✅ Тикет уже закрыт» (обе точки: takeover и close).
- [ШАГ 0.5] /preview/* закрыт гардом owner-only сразу в начале батча, после переноса роуты и файлы лаборатории удалены совсем (клиент/аноним → общий 404/редирект). Пруф-скрипт scripts/design-lab-apply-shots.mjs: клиент и аноним → /preview/chat,/preview/studio не открываются.
- [ШАГ 0.6 Анти-флаки] qa-journey-1-register: таймауты ↑ (goto 90с, default 60с), весь проход повторяется 1 раз с новым email, вывод упавшей попытки гасится (qa-launch ловит ❌) — смысл теста не ослаблен. Плюс корневая причина ложных 502: транзиентный сбой прокси→backend — один ретрай через 700мс в journey-1/2/3 (проверка console errors осталась строгой).
- [ШАГ 0.7] AGENTS.md + шапка ask-owner.mjs: на вопросах ВЫБОРА владельца (дизайн/цены/публикации) таймаут-дефолта НЕТ — вопрос ждёт ответа без срока; таймаут-дефолт только на технических мелочах.
- [ЗАДАЧА 1+3 Перенос в прод, реальные данные] Кабинет рекламодателя: новая ДЕФОЛТНАЯ вкладка «🏆 Командный центр» (вариант Б) — кампания дня (лучший CPL среди активных), воронка (показы→клики→конверсии из /api/ads/campaigns), неделя кампаний (по startDate/endDate); пусто → честный ноль + «Создайте первую кампанию» (открывает существующую модалку). Все 8 старых вкладок на месте. Чат «Фокус-чат» (OmegaChat): пилюля квоты ⚡ N/10 стала кликабельной → модалка «Баланс генераций» (Esc/focus-trap, «Пополнить» → /settings?tab=subscription); под инпутом цена действия до отправки «1 сообщение = 1✦ · осталось N✦» (реальные trialTokens из /users/me/quota). Профиль «Люкс-хаб» (Settings → Профиль): кольцо баланса генераций (remaining+trial, count-up, reduced-motion), «＋Пополнить» → тарифы; платежи — существующая живая вкладка «Платежи» со статусами. Студия: на инструментах тулбара CreativeHub цена «1✦» до запуска (title = пояснение). Глобальное меню/шапка — штатный DashboardShell на всех боевых экранах (паритет по вкладкам 100%, мобильный — существующий дроуер).
- [ЗАДАЧА 2 Паритет] Удалено осознанно: демо-витрина «Слоты» (реальные рекламные плашки AdBanner/ResponsiveAdBanner с пометкой «Реклама» уже живут в проде — отдельного боевого экрана слотов не существовало) и preview-варианты кабинета 🅰/🅲 (выбран Б). Все функции старых боевых экранов сохранены: кабинет — все вкладки (AdStudio/Создать/Кампании/NeuroSales/Календарь/Чат/Отчёты/Тарифы), чат — все действия, профиль — все вкладки Settings.
- [ЗАДАЧА 4 Уборка] /preview/* роут+гард+frontend/src/preview/* (включая варианты А/В)+shots-скрипты лаборатории удалены. Клиент на черновики попасть не может — роутов нет.
- [ЗАДАЧА 5 Голос владельца] Контур уже был (voice → Groq whisper-large-v3-turbo → OpenAI fallback → re-emit в owner-контур). Добавлено: isOwner-гард на STT (чужие голосовые не жгут квоту) + гард пустого текста (voice/фото больше не доходят до ask/freetext-контура). Тест «голосовое "статус" → сводка» — после деплоя.
- [ЗАДАЧА 6 Удаление постов канала] `backend/services/channelDeleteService.js`: «удали последний пост» (факт из CognitiveNode-журнала publishToChannel: messageId/url/текст) / «удали пост <ссылка t.me/.../N>» (публичная и t.me/c/ приватная) → превью поста → ✅/❌ → Bot API deleteMessage токеном клиентского бота; удалённые помечаются metadata.deleted (не предлагаются повторно). Только owner (через isOwner), только с превью.
- [ЗАДАЧА 7 VAPID] ФАКТ: ключи на месте (fallback-пара + env/apikeys hot-reload), push работал; «Ошибка» рисовал валидатор — у VAPID нет онлайн-пинга, validateApiKey возвращал valid:false → status:'invalid'. Фикс: офлайн-валидация формата (public 65 байт 0x04 / private 32 байта, base64url) в POST /api-keys и 🧪 /test + самолечение старых invalid-записей при GET /api-keys (валидный формат → active). Проверено: реальная fallback-пара проходит валидацию.
- [НЕ ТРОНУТО] платежи/ЮKassa (только чтение факта + unit), PlanConfig и тарифы, рефералка, гард-матрица Б5 (qaSecurityFlow ✅), лендинг, рубильники maintenance/registration, rate-limit, view-as, аддоны/entitlement, Hard Limit, Auto Downgrade, ask-owner контур, ownerFreeText (только +гард пустого текста), изоляция owner-контура, пауза CHANNEL-AUTO (env-флаг не тронут), webhook-гард.
- [GIT] ветка feat/design-lab-preview от origin/main: PR-2 (7 коммитов) + этот батч (wip-коммиты). Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feat/design-lab-preview

## 2026-09-07 — HOTFIX-FINAL-2 (Часть 2/2): owner-бот, auth-таймауты, Replicate-гард, waitlist, house-ads, CORS+бандл-диета, регрессия (ветка fix/hotfix-final-p2-integration)
- [З1 Owner-бот] `/balance email`, `/givecredits email N` (owner-only, начисление через creditGenerations); try/catch с ответом у всех onText-хендлеров (молчаливые падения убраны) (`b2d43e94`).
- [З2 TG «Загрузка»] AuthContext `/auth/me` — AbortController 10с (без него TG WebView/спящий Render = вечный спиннер); мини-апп: watchdog 12с → error-state с retry; основное приложение: authError в AuthContext → error-state с «Повторить» в ProtectedRoute (`b2d43e94`, `be7992c1`). Пруф: shot auth-error-state-retry-mobile ✅.
- [З3 Replicate 401] env-байпас после getProviderKey убран (мёртвый ключ не воскресает); 401/403 провайдера → providerKeyGuard.disableProviderKey (isActive=false + стоп кэша + TG-алерт). Новый ключ — только владелец через кабинет (`b2d43e94`).
- [З4 Бета-форма] waitlist `/api/launch/waitlist` пишет в MongoDB (Waitlist, был in-memory массив — умирал на рестарте Render) + TG-алерт владельцу на каждую заявку. qaWaitlistFlow 5/5 ✅ (`1babbf7e`, `be7992c1`).
- [З5 Лендинг] BetaCounter: живой total из FoundingConfig API; CTA на месте (`1babbf7e`).
- [З6 House-ads Р3] HouseAdSlot: чат=топ-баннер 34px (инпут не сдвигает), сайдбар ≥1440, пилюля 768–1439, нижняя плашка <768 (не на /creative-hub); ротация аддоны→апгрейд→рефералка→YT 10с, crossfade 275ms, pulse 3.5с, reduced-motion=статика, «Скрыть на день» per-slot, owner/admin/staff не видят (`1babbf7e`). Пруф: shot-прогон 12/12 слотов × темы × ширины ✅ (reports/hotfix-final-2/shots).
- [З7.1 CORS] allowlist в backend/config/cors.js подтверждён curl: allowed origin → 204 + ACAO конкретного origin; чужой → 403 без ACAO. Код не менялся (`80847ad6`).
- [З7.2 Бандл] route-level lazy для всех экранов кроме лендинга; manualChunks vendor/icons/charts/motion/ai/omega (recharts/framer-motion вне initial). Цифры: index 2302КБ → 562КБ (gzip 175КБ); initial (JS+CSS+HTML) 939КБ raw / 312КБ gzip (<1МБ ✅). First screen на Regular-3G эмуляции (1.6Mbps/300мс, gzip как на проде): 3.85с, FCP ~2.7с — цель <2с НЕ достигнута: физический пол SPA-boot ≈2.3с (312КБ gzip ≈1.6с передачи + RTT + eval ~0.5с), <2с требует пререндера лендинга → ждёт решения владельца (отдельная задача).
- [З8 AGENTS.md] Правила: ключи провайдеров без env-фолбэков, waitlist в БД, owner-бот команды, house-ads спека, бандл-диета, CORS-проверка, auth-таймаут (`6cba055c`).
- [З9 Регрессия] qa-launch 23/23 ✅ (вкл. qaCreditsFlow, qaSecurityFlow, qaWaitlistFlow); shot-прогон батча ✅; i18n-parity OK; build 0 ошибок; curl прод / /login /chat /credits → 200, 0 редиректов. Найдено и починено в регрессии: qa-view-as/qa-role-switch-flash сидили только legacy-ключ `i18n-lang` — после route-lazy чанк staff ре-инитит второй i18n-контур (`i18n.js`, ключ `app_language`) и en-варианты улетали в ru по navigator; сидим оба ключа (паттерн ui-audit/ui-buttons) (`59d57be2`). qa-view-as: устаревший LOCAL_API-дефолт :5000 → :18080 (единственный скрипт на старом порту; EACCES ::1:5000) + FLAKY-RETRY 700мс в прокси (`a8101490`). Shot-скрипт: регекс «Реклама» → /реклама/i (CSS uppercase ломал innerText-матч), 3G-замер через express+compression :4175 (vite preview отдаёт raw — завышал в ~3 раза) (`3d206c16`).
- [DEBUG view-as] Баннер owner→staff на integration-сборке работал всегда; «баг» был в debug-скрипте (неверный localStorage-ключ языка + prefs.owner=ru перекрывает). Пруф: reports/role-switch-flash/debug-integration-360en.png (360/en, «You're viewing as Staff / Exit»). Debug-скрипт удалён, в репо не коммичен.
- [НЕ ТРОНУТО] ЮKassa/платежи, PlanConfig/тарифы, рефералка, гард-матрица, entitlement/снапшоты аддонов, webhook-гард, боты-рубильники, лендинг-верстка, view-as логика (красные зоны).
- [GIT] fix/hotfix-final-p2-integration = merge fix/hotfix-final-owner2 + fix/hotfix-final-landing + fix/hotfix-final-infra + З9. Merge в main — только по ✅ владельца в TG.

## 2026-09-07 — ASK-OWNER-FIX: двойной канал не доставил отчёт Части 2 в TG (блокер) — найдено фактом, починено, пруф кнопкой владельца (ветка fix/ask-owner-html-esc)
- [ФАКТЫ ДИАГНОСТИКИ] owner-бот жив: getMe → HTTP 200 (@omega_aiviral_bot); chatId валиден: getChat → HTTP 200 (private); webhook активен: url=…/webhook/owner, pending=0, last_error=нет. Креды (env) валидны. Обрыв был НЕ в инфраструктуре.
- [ПРИЧИНА — факт] ask-owner шлёт parse_mode=HTML; отчёт Части 2 содержал «<1МБ», «<2с», «<768» → TG API: HTTP 400 «can't parse entities: Unsupported start tag "1МБ"» (воспроизведено прямым вызовом sendMessage с тем же паттерном). Сообщение молча не ушло, warning утонул в `tail` фонового запуска; владелец смержил PR #62 через GitHub без кнопок.
- [ФИКС scripts/ask-owner.mjs] экранирование & < > в context/question/note перед HTML-отправкой (tgSend и tgClose); гард лимита 4096 (обрезка контекста вместо молчаливого 400); код ответа TG API теперь ВСЕГДА в логе: «TG API sendMessage → HTTP 200, ok=true, message_id=…». report-to-owner.mjs и backend/services/askOwner.js проверены — parse_mode не используют, разлома там нет.
- [ПРУФ] Тестовый ask-owner с тем же ломающим паттерном «<1МБ» в тексте: TG API sendMessage → HTTP 200, ok=true, message_id=2976 → владелец нажал кнопку «✅ Кнопки дошли» (ответ пришёл через webhook прода → БД → поллинг, ASK_RESULT via=telegram). Контур «вопрос → кнопки → ответ» подтверждён фактом end-to-end.

## 2026-09-07 — CHAT-PRO-REWORK: экран chat 1:1 по утверждённому эталону (вариант B «Люкс-хаб»), инвентарь в шторку, регрессия зелёная (ветка feat/chat-pro)
- [З1 Компоновка 1:1] На экране chat только: люкс-шапка (✦ OMEGA Studio · статус, переключатель режимов md+, поиск, пилюля баланса → QuotaDetailsModal, кнопка-меню), чистая лента, справа ≥821px одна колонка (BalanceRingCard + «В этом месяце», реальные /users/me/quota + /api/scheduled-posts). Панелей «Сессии»/«Режимы»/«Инсайты» на экране НЕТ. Светлая тема — та же компоновка в светлых токенах (.chat-pro-scope + html.light-оверрайды в luxury.css, фон #f5f5f7, лента белая) (`4f1ab8d0`).
- [З2 Инвентарь — ничего не удалено] Сессии (список + «Новый чат») → шторка по кнопке-меню (все ширины, Esc/фон закрывает); Режимы → переключатель в люкс-шапке (md+) + в шторке; Инсайты (4 вкладки) → шторка; бейдж роли + AutoPilot (owner) → шторка. В chat-режиме скрыты hub-header, левая колонка, инсайт-тогл, нижняя нав, FAB (условие !isChatMode в CreativeHub) (`4f1ab8d0`).
- [З3 Сохранено из CHAT-PRO] стриминг, композер 1→5 строк, скелетоны, поиск по ленте (searchQuery), черновик localStorage omega_chat_draft (переживает reload — пруф в гейте), клавиатура/Esc, reduced-motion, адаптив 360→4K — всё на месте, проверено гейтом.
- [З4 Регрессия] qa-launch 23/23 ✅ (после фикса шелла); chat-pro-luxe-shots (переписан под эталон) ✅ все проверки зелёные: 360/768/1280 × dark/light × client(business)/admin/creator, «панелей нет на экране», шторка открывается/Esc, пилюля→модалка→Esc, поиск empty-state, черновик reload, house-ads 32px ≤34px, EN. build 0 ошибок; i18n-parity OK; curl прод / /chat /login /credits → 200, 0 редиректов. AGENTS.md: факт [CHAT-PRO-REWORK] (`c83f6c71`).
- [ФИКС высотки] Найдено гейтом: страница /creative-hub/chat была выше вьюпорта на ~172px (h-[calc(100vh-64px)] + шелл pt-20 + py-6) и сама скроллилась вниз → шапка Люкс-хаба уезжала под глобальный хедер, на 360 welcome клиппился. Фикс: DashboardShell для /creative-hub/chat — main h-[100dvh] flex-колонка, обёртка flex-1 min-h-0 без pb под навбар; CreativeHub chat-режим — flex-цепочка root→main→section. Замер probe: docH=vh, scrollY=0 (`c83f6c71`).
- [ФИКС гейта] Светлая тема в тесте не применялась: DashboardShell синкает тему из префов юзера в БД (сильнее localStorage) — гейт теперь ставит theme через PUT /api/users/me с мерджем префов. Проверка «правая колонка скрыта» переписана с aside (ложный матч на шелл-навигатор) на текст кольца (`c83f6c71`).
- [НЕ ТРОНУТО] house-ads (спека Р3: баннер 32px ≤34px, «Скрыть на день», метка «Реклама» — гейтом подтверждено), платежи/ЮKassa, _redirects, боты, ask-owner, qa-харнес, каркас навигации (левая полоска иконок), логика OmegaChat.
- [GIT] feat/chat-pro от origin/main. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...feat/chat-pro. Merge — только по ✅ владельца в TG.

## 2026-09-08 — CHAT-PRO-FIX: все режимы в люкс-компоновке, безлимит владельца, шторка слева solid, скролл-навигация ленты, управление историей (ветка fix/chat-pro-modes)
- [З1 Режимы] AI Chat / Content Analyzer / Viral Studio — одна люкс-компоновка (чистая лента + кольцо справа, без панелей): `isLuxeLayout = true` в CreativeHub, старые grid-экраны недостижимы, но НЕ удалены (ждут ✅ владельца). Меняются шапка (режим в подзаголовке), плейсхолдер композера (`inputPlaceholder` в OmegaChat), чипы-подсказки. Старый заголовок режима за гейтом `!isLuxeLayout` (нашёл гейт: торчал обрезанным на analyzer/viral).
- [З2 Безлимит владельца] Причина «0✦ из 10» у владельца: checkQuota возвращает `Infinity`, JSON сериализует в null. `/users/me/quota` теперь отдаёт `unlimited: true` + null в числах; фронт (пилюля, BalanceRingCard, QuotaDetailsModal, хинт квоты) показывает ∞ без плашки лимита и без CTA пополнения. QA: owner.test@aiviral-studio.ru в сидере. Пруф: скрин luxehub-owner-unlimited (кольцо ∞, «Безлимит ✦», Тариф Owner).
- [З3 Пустой блок] «В этом месяце» растянута flex-1 — мёртвой области под картой баланса нет.
- [З4 Шторка] «Меню чата» СЛЕВА, solid (`.chat-pro-drawer`: #0f0f18 / #fff в light + оверрайды внутренних карточек), ≤360px, скролл внутри, свайп с левого края на мобайле, Esc/✕/клик по фону, анимация 280ms (reduced-motion — без неё).
- [З5 Скролл/навигация] Страница 100dvh (isLuxeChat теперь на весь /creative-hub — analyzer/viral скроллились), скроллится только лента `.omega-chat-scroll`; шапка/композер всегда видны (гейт: лента в середине → шапка+пилюля+композер видны, docH=vh). Авто-скролл вниз только у низа или после своего сообщения; кнопка ↓ «к последнему» (бейдж новых) + ↑ «к началу» (>3 экранов); окно истории 30 + догрузка вверх со scroll-anchor (позиция не прыгает, гейт: scrollTop сохранён); таблицы/код — внутренний скролл/перенос внутри ленты; reduced-motion → мгновенный скролл.
- [З6 История] ⋯ сессии (long-press на мобайле) → «Переименовать» / «Удалить» («Удалить чат навсегда?» ✅/❌); ⋯ в шапке чата → «Очистить историю» («Сообщения удалятся навсегда?») → welcome-экран, сессия остаётся. Только свои чаты (локальные по построению).
- [ФИКС гейта] Дропдаун ⋯ перекрывался правой колонкой (шапка z-20); ожидание smooth-скролла ↓ на 360 увеличено; qa-view-as падал на 429 — in-memory бакет rate-limit юзера owner после серии прогонов, лечится рестартом локального бэкенда (не регрессия кода; отдельно проверено зелёным).
- [РЕГРЕССИЯ] дизайн-гейт chat-pro-luxe-shots 228/228 ✅; билд 0; i18n-parity OK; прод / /chat /login /credits → 200, 0 редиректов; qa-launch — см. итог в TG-отчёте.
- [НЕ ТРОНУТО] house-ads (спека Р3), платежи/ЮKassa, _redirects, боты, ask-owner, qa-харнес, каркас навигации.
- [GIT] fix/chat-pro-modes от origin/main. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/chat-pro-modes. Merge — только по ✅ владельца в TG.

## 2026-09-13 — OMEGA-VIDEO ДОП-2: цены ✦ и TTL из кабинета, сценарий из мысли, обложки, конкуренты ниши, честная публикация (ветка fix/omega-video)
- [З2/З3 Цены + TTL] Плоские поля OwnerSettings (паттерн mediaUploadLimitMb, реестр цен НЕ создан — зона REAL-DATA): videoAnalysisCostCredits (1✦), coverGenerationCostCredits (1✦), scriptGenerationCostCredits (2✦), videoStorageTtlHours (0 = удалить файл сразу после разбора, дефолт; результат анализа в чате навсегда). Кэш ≤60с, owner GET/POST /api/owner/video-settings ВЫШЕ generic /:entity, клиент читает /api/upload/limits. consumeGeneration(userId, role, {cost})/refundGeneration(userId, cost) — фактическая цена. Негативы: anon 401, client 403 (`4a2655d0`, `4e135599`).
- [Хранение] MediaFile учёт (best-effort в routes/upload.js), TTL — services/videoStorage.js#applyVideoStorageTtl; крон cron/mediaCleanup.js: deleteAt истёк → unlink, сироты (analyzedAt null >24ч) → unlink, сводка sendOwnerAlert в TG. Счётчик GET /api/upload/storage-usage фактом с диска. Пруфы гейта: TTL 0 удалил сразу; TTL 24ч — deleteAt ≈ +24ч, крон удалил по истечении; сирота удалена; загружено 100МБ → занято 100МБ → после удаления 0.
- [З4 Сценарий из мысли] POST /api/omega/script-from-idea: неполная цель → needClarification БЕЗ списания; сценарий хук/таймкоды/озвучка/обложка/CTA + реальные цифры топ-видео ниши (youtubeDataService; нет ключа — честно, выдуманных чисел нет); smart-fallback → refund фактической цены; драфт в Планировщик в 1 клик (POST /api/scheduler/posts status:'draft'); цена ✦ показана до запуска (`375291d5`).
- [З5 Обложки] services/coverGenerator.js: размер фактом под платформу (youtube/vk/telegram 1280×720, tiktok/instagram/shorts 1080×1920), текст 2–4 слова sharp-оверлеем с тёмной плашкой (AI текст НЕ рисует), textHeightRatio — гейт читаемости в сетке 320px; POST /api/omega/cover-variants (3 варианта, сбой → refund). Чат: кнопка «Обложки N✦», сетка 3 вариантов → применить к драфту/скачать, превью 1:1 (`a9bde4c0`).
- [З6 Конкуренты ниши] GET /api/omega/niche-competitors: YouTube Data реально (поиск+статистика видео/каналов, рейтинг из реальных метрик); прочие платформы → available:false + requiredKey + consoleLink — клиенту «скоро», владельцу карточка «Требуется ключ» в ApiKeysTab. Выдуманных цифр нет (honesty-гейт зелёный) (`22bf2e91`).
- [З7 Публикация] platformPublisher — честные «подключите X» без моков; YouTube-интеграция не тронута (`5245e6ca`).
- [ГЕЙТ-ФИКСЫ — факты из прогонов] (1) vision кадров последовательно не успевал → Promise.allSettled параллельно; (2) Pollinations отдаёт текст ошибки бюджета/рейт-лимита 200-м ответом → детект providerErrorLeak в analyze-video и script-from-idea → честный отказ + refund; (3) qaOmegaVideo2: cwd=backend (счётчик видел 0 при файле на диске), роль user→creator (consume); (4) shots-гейт: лог analyze-video в прокси (статус+мс+тело), ожидание ответа по data-msg-id до 150с (проверка по словам «хук»/«анализ» ловила статику чипов — ложнозелёное), чип 6с, тост лимита 8с, клиентский таймаут разбора 150с (`4027f925`).
- [РЕГРЕССИЯ] qa-launch 24/24 ✅ (вкл. qaOmegaVideo2 22 проверки, omega-honesty); omega-video-upload-shots ALL GREEN (карточка цен в кабинете, живая цена 3✦ на чипе клиента без деплоя, тост лимита); build 0 ошибок; i18n-parity 0-diff; прод / /creative-hub /owner /deep/route → 200, 0 редиректов; CI GitHub run 34750866842 → Success (qa 8m46s). AGENTS.md дописан фактом [OMEGA-VIDEO ДОП-2].
- [НЕ ТРОНУТО] итог допа З1 (drag&drop, чип Film, прогресс, лимит, vision-разбор, smart-fallback с возвратом ✦), drag&drop картинок, house-ads (спека Р3), платежи/ЮKassa, PlanConfig/тарифы, _redirects, боты, ask-owner, qa-харнес, CHAT-PRO-REWORK + CHAT-PRO-FIX.
- [GIT] fix/omega-video от origin/main, 7 коммитов. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/omega-video. Merge — только по ✅ владельца в TG.

## 2026-09-14 — STAFF-MGMT + CLEANUP: редактирование/удаление staff owner-only, AuditLog с diff, негативы 403, чистка .bak (ветка fix/staff-mgmt)
- [З1 Редактирование] PATCH /api/owner/staff/:id owner-only (ВЫШЕ generic /:entity/:id в routes/owner.js), логика services/userManager.js#updateManagedUser: сохранение в БД фактом, AuditLog `staff_updated` с diff до/после (пароль — факт смены, без значения). UI TeamTab: карандаш → модалка → тост «Сотрудник обновлён», строка в таблице обновляется (`c3c5b524`, `7acf3ef2`).
- [З2 Удаление] DELETE /api/owner/staff/:id owner-only: гарды на бэке (не UI-only) — нельзя удалить/понизить себя и owner-аккаунт (403), soft-delete с анонимизацией email, AuditLog `staff_deleted`. UI: удаление только через confirm-диалог с именем и email аккаунта (data-testid staff-delete-confirm) (`c3c5b524`, `7acf3ef2`).
- [З3 Негативы] qaSecurityFlow C3: admin/client/staff → 403 на PATCH и DELETE — зелёные фактом в qa-launch (`7acf3ef2`).
- [З4 Гейт] scripts/staff-mgmt-shots.mjs ALL GREEN: логин owner/admin, создание staff, admin PATCH/DELETE → 403 (API-факт), UI редактирование (email сохранён и виден), confirm-диалог с именем, строка исчезла, AuditLog staff_updated/staff_deleted в кабинете, 0 критичных console errors (`473b84d6`).
- [ФИКС найден гейтом] AuditLog сортировался по createdAt, которого в схеме нет (timestamps: createdAt→'timestamp') — sort по несуществующему полю = натуральный порядок, свежие записи уходили за limit 50. safeFind принимает sortField, для AuditLog — 'timestamp' (getAudit + дашборд recentActivity) (`473b84d6`).
- [З5 Cleanup] Удалён 31 .bak/.backup архив из frontend/src (App.jsx.bak/.bak2, Sidebar ×4, OwnerDashboardPage ×13, DashboardPage/AdminPage/LandingPage/SchedulerPage/SettingsPage/StaffDashboardPage и др.) — не роутились, −25 995 строк (`7acf3ef2`).
- [РЕГРЕССИЯ] qa-launch 23/24: упал только qa-bots «webhook жив» — url пуст в getWebhookInfo (внешнее состояние TG на проде, НЕ регрессия батча: дифф ботов не трогает, лог локального бэкенда «webhook не трогаю, polling не включаю»; в CI TG-ключей нет → пропуск зелёный). Отдельные гейты: omega-video-upload-shots ALL GREEN, covers-frames-shots ALL GREEN, real-data-audit ВСЁ ЗЕЛЁНОЕ (78 табов + creator/advertiser × dark/light × 1280/360 + реестр цен). build 0 ошибок (index 564КБ — не вырос); i18n-parity 0-diff; прод / /owner /creative-hub → 200, 0 редиректов. AGENTS.md: факт [STAFF-MGMT]. CI GitHub run 34866189629 → Success.
- [НЕ ТРОНУТО] итог REAL-DATA/REAL-DATA-2 (0 моков, реестр цен, generic CRUD owner/<entity>), платежи/ЮKassa, refunds-контур, боты, house-ads, qa-харнес, CHAT-PRO, OMEGA-VIDEO, COVERS-FRAMES.
- [GIT] fix/staff-mgmt от origin/main, 4 коммита. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/staff-mgmt. Merge — только по ✅ владельца в TG.

## 2026-09-15 — SMART-TTL: событийная очистка исходников (used/heartbeat/idle), результаты вечны (ветка fix/smart-ttl)
- [З1 «Результат принят»] Обложка скачана (сетка и превью-модалка) / применена к посту / драфт создан → клиент зовёт POST /api/upload/used {url, reason} → services/videoStorage.js#deleteMediaNow удаляет исходник немедленно (только свой /uploads/<userId>/, cover-*.jpg отказ — инвентарь вечен) + TG-алерт владельцу (HTML-экранирование) + мягкое уведомление в чат «исходник очищен, результаты сохранены». URL исходника живёт в action.videoUrl сообщения разбора (переживает F5 — strip касается только кадров) (`4a2a923a`, `7b4c9e55`).
- [З2 «Клиент покинул задачу»] Heartbeat POST /api/upload/heartbeat раз в 30с из OmegaChat (только при TTL>0, цель = последний action.videoUrl ленты; unmount/закрытие/новая задача → молчание). OwnerSettings.videoIdleMinutes (дефолт 30, 1–1440, hot-reload ≤60с, карточка в Подписках рядом с TTL, клиент читает в /api/upload/limits). Молчание > N мин → idle-свип 0c в cron/mediaCleanup.js (только kind video + lastSeenAt ≠ null — легаси без heartbeat идут по старым свипам) (`4a2a923a`).
- [З3 TTL-потолок] deleteAt (свип 0) срабатывает при ЛЮБОМ heartbeat — гейт: свежий lastSeenAt + deleteAt в прошлом → файл удалён кроном (`c80009c9`).
- [З4 Уведомления] chat.sourceTtlNotice (после разбора при TTL>0, ДО удаления: «хранится до N ч, очищается после M мин бездействия — результаты навсегда») + chat.sourceClearedNotice (после used) — i18n ×4 файла, parity 0-diff (`7b4c9e55`).
- [З5 Логи/счётчик] Все событийные удаления → TG-лог владельцу (deleteMediaNow + сводка крона с разбивкой «в т.ч. по бездействию: X»), storage-usage фактом с диска до/после (гейт: 50МБ → ≤200КБ) (`c80009c9`).
- [ГЕЙТЫ] backend/scripts/qaSmartTtl.js 16/16 (guard anon 401/чужой URL 403, hot-reload N=1 из кабинета без деплоя, used → файл удалён + обложка на месте, idle > N мин → крон удалил, живой heartbeat → жив, TTL-потолок поверх heartbeat, storage-usage до/после). scripts/smart-ttl-shots.mjs ALL GREEN: скрин карточки N в кабинете + тост «применится за минуту», N=1 живой в /upload/limits из UI-клика, TTL-уведомление в ленте (скрин), heartbeat до backend (200 в proxy-логе), used → deleted + usage до/после. Найдено гейтом: mongoose/dotenv не резолвятся из корня (backendRequire), /auth/me отдаёт user.id, оверлей над кнопкой save — DOM-click (`c80009c9`, `6243368d`, `dd46e8f5`-потомок).
- [РЕГРЕССИЯ] qa-launch 24/25: упал qa-view-as «EN: плашка You're viewing as» — флаки (повторный прогон зелёный без правок; к диффу батча отношения нет — i18n только добавка ключей). omega-video-upload-shots ALL GREEN, covers-frames-shots ALL GREEN (вкл. yt-проверки). build 0 ошибок (index 564КБ — в пределах цели); i18n-parity 0-diff; прод / /creative-hub /login → 200, 0 редиректов. AGENTS.md: факт [SMART-TTL].
- [НЕ ТРОНУТО] TTL-контур 0/24ч и крон сирот, COVERS-FRAMES + доработки, YT-FRAMES, OMEGA-VIDEO (списание/refund), платежи/ЮKassa, refunds-контур, боты, house-ads, qa-харнес, CHAT-PRO, _redirects.
- [GIT] fix/smart-ttl от origin/main. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/smart-ttl. Merge — только по ✅ владельца в TG.

## 2026-09-15 — YT-DATA-FIX: YouTube-анализ и yt-frames — диагностика фактом, keyGuard/алерты для youtube, честный отказ без списания ✦, лог reason фолбэков (ветка fix/yt-data)
- [ДИАГНОСТИКА З1 фактом] Живой videos.list по id из прод-лога (887uCBMcnIM, lWZpOOIAz8o) ключом из env: HTTP 200, items:1, реальные цифры (3 800 939 / 4 124 103 просмотра) — API и id здоровы. Parse Shorts-URL НЕ сломан: регексы чата/обложек/скрапера берут id из watch?v=, /shorts/, youtu.be/, с si-параметром (таблица фактов + регресс в qaOmegaVideo2 §5b). Корень молчания на проде: (1) keyGuard (disableProviderKey + TG-алерт + карточка invalid) был подключён только для replicate — youtube-ошибки уходили просто в console.warn; (2) getYoutubeApiKey содержал запрещённый env-фолбэк `|| process.env.YOUTUBE_API_KEY` после getProviderKey (HOTFIX-FINAL-2) — выключенный в кабинете ключ воскресал; (3) чат при недоступной статистике всё равно звал AI и списывал ✦, отвечая шаблоном.
- [З1 Фикс] youtubeDataService: env-воскрешение выпилено (env-запас уже внутри getProviderKey через envMap.youtube); handleYoutubeApiError: постоянные ошибки (keyInvalid/accessNotConfigured/ipRefererBlocked/forbidden) → disableProviderKey('youtube') = isActive=false в БД + стоп кэша + TG-алерт (механика HOTFIX-FINAL-2); quotaExceeded/dailyLimitExceeded — ключ НЕ выводится (сброс после 10:00 МСК), троттленный TG-алерт ≤1/6ч; no_api_key → тот же троттленный алерт «добавьте ключ в Кабинет → API Ключи». Карточка «Требуется ключ: …» в ApiKeysTab (requiredFor.youtube) подтверждена живьём — работает, ключ просто не был подключён в прод-БД/кабинете.
- [З1 Чат] omegaController: YT-ссылка + статистика недоступна + нет явного действия (обложка/драфт/best time) → честный отказ «Не удалось получить данные YouTube: <причина>» БЕЗ вызова AI и БЕЗ списания (refund фактической цены, honest:true, source:youtube-unavailable, карточка со statsError). Гейт qaOmegaVideo2 §8: несуществующий id → honest-отказ + statsError + trialTokens неизменны (10 → 10), живой API views>0, mapYoutubeError различает quotaExceeded/keyInvalid.
- [З2 Диагностика yt-frames] Прогон yt-dlp 2026.08.19 (pinned) по Shorts из лога: probe OK, download OK, 9 кадров за 18с — код и Shorts-форматы здоровы локально. На проде (Render, datacenter IP) вероятен anti-bot («Sign in to confirm…») — раньше это было невидимо: фолбэк писал reason только в getLastYtFramesSkip(), в лог шёл лишь [MEM] yt-frames:start. Теперь КАЖДЫЙ фолбэк логируется: `[yt-frames] fallback <id>: <reason>` (skip() в ytFrames) + переходы цепочки в coverGenerator (yt-frames → thumbnail → AI с reason). Фолбэк-цепочка и kill-switch не тронуты.
- [ГЕЙТЫ] qa-launch 25/25 ✅ (вкл. qaOmegaVideo2 §8 новые). scripts/yt-data-shots.mjs ALL GREEN: watch-ссылка → карточка «✅ Реальные данные» + просмотры 3 800 939 (DOM-факт), shorts+si → 4 124 110 на 390px, честный отказ с причиной в ленте и amber-блоке карточки, кабинет «Проверить» битого ключа → тост «HTTP 400: Ключ недействителен — проверьте его в Google Cloud → Credentials». Скрины: reports/yt-data-fix/. Найдено гейтом: innerText отражает CSS uppercase (лейбл «ПРОСМОТРЫ») и useCountUp анимирует число — проверки по regex /просмотры/i + пауза 2.5с; селектор «Проверить» ловил «Проверить ЮKassa» — скоуп в модалку.
- [РЕГРЕССИЯ] build 0 ошибок (index 576КБ, фронт не тронут); i18n-parity 0-diff (новых ключей нет — серверные строки); прод / /creative-hub /login /owner → 200, 0 редиректов.
- [НЕ ТРОНУТО] COVERS-FRAMES/YT-FRAMES-логика кадров, SMART-TTL, OMEGA-VIDEO списание/refund механика, платежи/ЮKassa, боты, qa-харнес, инвентарь вечен. ЮKassa-тест в кабинете НЕ запускался (approve-зона; однажды селектор коснулся — отказ «Не заполнено поле», платежей не создавалось).
- [ЖДЁТ ВЛАДЕЛЬЦА] Прод: проверить/перевыпустить ключ youtube в Кабинет → API Ключи (или Render env YOUTUBE_API_KEY) — после деплоя keyGuard пришлёт TG-алерт с фактической причиной, если ключ мёртв.
- [GIT] fix/yt-data от origin/main. Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/yt-data. Merge — только по ✅ владельца в TG.
