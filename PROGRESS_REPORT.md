...
## 2026-09-04 — Б4-ДОП-2: АНТИГАЛЛЮЦИНАЦИИ OMEGA + ГЕЙТ UI-КАЧЕСТВА (ветка fix/b4-dop2)
- [З6 ЧЕСТНОСТЬ] `backend/ai/omega/honestyGuard.js`: HONESTY_PROMPT_BLOCK (запрет выдумывать числа/статистику/имена/статусы, цифры только из БД/API, нет источника → честный отказ) подключён в системные промпты aiService.js и contextEngine.js; детектор выдумок looksLikeFabrication/isHonestReply; детектор data-вопросов isDataQuestion + noDataReply (RU/EN, «нет данных» + куда смотреть). Шорт-циркуит в omegaController.chat: data-вопрос без реальных источников (и без YouTube-ссылки, для неё статистика фетчится реальным API) → сразу честный отказ, модель не вызывается, квота не тратится.
- [З6 ТЕСТ ПРОВОКАЦИЙ] `scripts/omega-honesty-test.mjs`: статика (блок подключён) + юнит детектора (6 выдумок флагаются, 5 честных проходят) + 12 провокаций ловятся шорт-циркуитом + LIVE против :18080 с Mongo — 12/12 честных отказов, ни одной выдуманной цифры.
- [З7 ГЕЙТ] `docs/UI_QUALITY_GATE.md` — чек-лист «идеальный кабинет» из 8 пунктов зафиксирован как обязательный гейт сдачи всех UI-батчей (Esc / focus-trap / тултипы у обрезанного / только дизайн-система / reduced-motion / матрица 360–1920 × RU/EN / first screen < 2с / inline empty-state).
- [З7 ПРОГОН] Плашка view-as, редактор аддонов, кабинет админа. Найденные дефекты исправлены: ①② у модалок админа (5 самодельных), AI-анализа аддонов и ModalShell не было Esc/focus-trap → общий хук `frontend/src/hooks/useModalA11y.js` (Esc + Tab-цикл + возврат фокуса на триггер, role=dialog/aria-modal); ③ у обрезанного текста view-as плашки не было title — добавлен; селектор теста qa-view-as [role=status] коллидировал с тостами react-hot-toast (strict-mode → ложные ❌) → плашка получила data-testid=view-as-banner.
- [ГЕЙТ СТАТУС] ① Esc ✅ (useModalA11y во всех модалках трёх экранов) ② focus-trap ✅ ③ тултипы ✅ (UsersManager EllipsisText + title у плашки) ④ стили по токенам дизайн-системы ✅ ⑤ reduced-motion ✅ (globals.css/index.css) ⑥ матрица: ui-audit admin+/owner?tab=addonsManage 20/20 OK (0 скроллов/налезаний/ключей), qa-view-as матрица плашки 5 ширин × RU/EN ✅ ⑦ first screen: admin 1.44–1.58с ✅, owner-addons 1.95–1.96с ✅ ⑧ empty-state inline ✅ («Пользователей нет», «Постов пока нет» — CTA на месте контента).
- [СНИМКИ] reports/ui-audit-b4dop2/ (admin + addonsManage × 5 ширин × RU/EN, в т.ч. 360/1280 RU), reports/ui-audit-b4dop2/addons-editor/ (редактор с реальными данными, 1280 RU), reports/view-as/ (плашка 360/1280 RU+EN и вся матрица).
- [TEST] npm run build 0 ошибок ✅; тест провокаций 12/12 честных (live) ✅; i18n-parity чист ✅; qaSecurityFlow вся матрица ✅; qaAddonsFlow зелёный (entitlement/снапшоты целы) ✅; qa-view-as e2e зелёный ✅.
- [ПОСТОРОННЕЕ, НЕ ИСПРАВЛЕНО] vite preview на этой машине слушает только [::1] без --host 127.0.0.1 — curl/аудиты по localhost не достучаться; ui-audit owner-страниц без UI_AUDIT_MOCK_AUTH=1 уходит в редирект (токены .tmp-ui-polish/token-owner.json протухли для продового API); редактор аддонов в mock-auth показывает пустую витрину (API стабится) — с реальными данными снято отдельно.
- [НЕ ТРОНУТО] Платежи/ЮKassa, PlanConfig и тарифы, боты, лендинг, модалка обновлений, рубильники, гард-матрица Б5, rate-limit, view-as логика (только data-testid/title), entitlement/снапшоты аддонов.
- [GIT] ветка fix/b4-dop2, 8 коммитов, 12 файлов, +315/−26 (diff от 5443d8d3). Compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/b4-dop2
