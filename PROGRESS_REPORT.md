...
## 2026-08-30 — PERSIST VIEW-AS (ветка fix/view-as-persist)
- [ЗАДАЧИ 1–4] view_as персистится в localStorage ОТДЕЛЬНО от реальной роли (AuthContext overlay: user.realRole='owner' + user.role=view_as; JWT не трогается, user_profile не затирается — фикс role-switch-flash сохранён, маркер role_switch_at оставлен). Плашка «Ты смотришь как …» + «Выйти» (ViewAsBadge, фиксирована сверху, safe-area, i18n RU/EN, выход без reload через navigate('/owner')). Owner-only фетчи подавляются под view-as (useDashboardData по эффективной роли) — консоль чистая, 0 × 401 во всех 5 ролях
- [ЗАДАЧА 3.5–6 — ПАРИТЕТ USERS] Новый shared/UsersManager.jsx (707 строк): таблица пользователей с поиском/фильтрами/сортировкой/масс-выбором, 5 модалок (Add/Edit/Delete/Confirm бан-разбан/Extend ±дни), toast. AdminDashboardPage → <UsersManager onUsersLoaded> (stats-карточки остались на странице, −674 строки); owner → вкладка «Клиенты» (ClientsTab) = обёртка над тем же компонентом (−298 строк). Продление тарифа идёт через /owner/control/extend-subscription (authorize owner+admin) — тот же сервис extendSubscriptionDays, что у TG-бота «продли» и OverviewTab; qaSecurityFlow: staff → 403, белый список ролей цел
- [БАГ ПО ПУТИ] /api/campaigns в server.js регистрировался ПОСЛЕ app.use('/api', fallbackRoutes) → вечный 404 (в т.ч. на проде). AdvertiserDashboardPage переведён на /api/ads/campaigns (тот же запрос Campaign)
- [CHECKS] esbuild затронутых OK; npm run build 0 ошибок; scripts/qa-view-as.mjs — ВСЕ ЗЕЛЁНЫЕ: owner→admin/staff/advertiser/creator/business (маршрут+плашка+навигация+reload-persist+«Выйти»→/owner, консоль без error, API без 401/403), EN-плашка, реальный creator на /owner|/admin|/staff → /unauthorized (гард не ослаблен); qaSecurityFlow — вся матрица зелёная; матрица плашка+«Клиенты» × 360/428/768/1280/1920 × RU/EN: 0 горизонтальных скроллов (по пути починено обрезание текста селектов фильтров на 360px — flex-wrap + w-full на мобильных)
- [GIT] Ветка fix/view-as-persist; compare: https://github.com/yuri0941/ai-viral-studio/compare/main...fix/view-as-persist; diff --stat: 15 файлов, +1160/−978; скрины: reports/view-as/ (banner 360/1280 RU+EN, persist-after-reload, clients 5 ширин × RU/EN)

## 2026-08-27 — Б3 CLIENT-JOURNEY-QA FINAL (ветка fix/client-journey-qa)
