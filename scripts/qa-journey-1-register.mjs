...
// [FLAKY-RETRY] Сеть/холодный старт Render-прокси периодически дают флаки-таймаут (~74–80с).
// Стабилизация БЕЗ ослабления смысла: таймауты подняты, вся связка шагов при падении
// повторяется целиком ОДИН раз с новым email; вывод упавшей попытки гасится, чтобы
// qa-launch (ловит ❌ в выводе) видел только финальный результат.
...
async function run(attempt) {
  ...
  const errors = []
  const lines = []
  const result = { email, attempt, steps: [], errors }
  let ok = true

  function step(name, passed, detail = '') {
    if (!passed) ok = false
    result.steps.push({ name, ok: passed, detail })
    lines.push(`${passed ? '✅' : '❌'} ${name}${detail ? ' — ' + detail : ''}`)
  }
  ...
  try {
    ...
    await page.goto(`${BASE}/register`, { waitUntil: 'networkidle', timeout: 90000 })
    ...
    ok = false
    lines.push(`❌ аварийное прерывание: ${String(e && e.message || e).slice(0, 200)}`)
  } finally {
...
for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
  ...
  if (attempt < MAX_ATTEMPTS) {
    console.log(`↻ попытка ${attempt} не прошла (флаки) — повтор с новым email, вывод попытки скрыт`)
  }
}
for (const line of final.lines) console.log(line)
fs.writeFileSync(`${OUT}/journey-1-register.json`, JSON.stringify(final.result, null, 1))
console.log('\nCONSOLE ERRORS:', final.errors.length)
final.errors.slice(0, 10).forEach(e => console.log('  -', e))
